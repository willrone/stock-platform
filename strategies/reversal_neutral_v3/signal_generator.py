#!/usr/bin/env python3
"""
Reversal Neutral V3 - 反转因子市场中性策略（稳健版）

V3 改进：
- Top N 从 10 增加到 15，分散风险
- 最大回撤从 -46% 降到 -32%
- 2023 年亏损从 -11% 降到 -2%
...
class ReversalNeutralV3:
    """反转因子市场中性策略 V3（稳健版）"""
    
    VERSION = "3.0.0"
    MODEL_NAME = "reversal_neutral_v3"
    
    DEFAULT_CONFIG = {
        'top_n': 15,                    # V3: 增加到 15，分散风险
        'train_months': 6,
    ...
    def load_data(self, data_dir: str, start_date: str = None, end_date: str = None):
        ...
        print(f"[ReversalNeutralV3] 数据加载完成:")
    ...
    def train(self, train_end_date: str = None):
        ...
        print(f"[ReversalNeutralV3] 训练完成: {len(train_data):,} 样本")
    ...
    def save(self, save_dir: str):
        ...
        print(f"[ReversalNeutralV3] 已保存到: {save_path}")
    ...
    def load(self, load_dir: str):
        ...
        print(f"[ReversalNeutralV3] 已加载: {load_path}")


if __name__ == '__main__':
    import argparse
    
    parser = argparse.ArgumentParser(description='Reversal Neutral V3')
    parser.add_argument('--data-dir', required=True)
    parser.add_argument('--action', choices=['train', 'signal', 'backtest'], default='signal')
    parser.add_argument('--date', help='信号日期')
    parser.add_argument('--start-date', help='回测开始')
    parser.add_argument('--end-date', help='回测结束')
    parser.add_argument('--save-dir')
    parser.add_argument('--load-dir')
    
    args = parser.parse_args()
    
    generator = ReversalNeutralV3()
