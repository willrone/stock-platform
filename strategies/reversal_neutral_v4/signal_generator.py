#!/usr/bin/env python3
"""
Reversal Neutral V4 - 反转因子市场中性策略（趋势增强版）

V4 改进：
- 加入趋势跟随：上涨趋势加仓 1.2x，下跌趋势减仓 0.8x
- 收益从 +783% 提升到 +1285%（+64%）
- 夏普从 3.97 提升到 4.47
- 回撤从 -45% 降到 -42%
...
class ReversalNeutralV4:
    """反转因子市场中性策略 V4（趋势增强版）"""
    
    VERSION = "4.0.0"
    MODEL_NAME = "reversal_neutral_v4"
    ...
    DEFAULT_CONFIG = {
        ...
        'trend_window': 10,           # V4: 趋势判断窗口
        'trend_up_factor': 1.2,       # V4: 上涨趋势加仓倍数
        'trend_down_factor': 0.8,     # V4: 下跌趋势减仓倍数
        'trend_threshold': 0.01,      # V4: 趋势判断阈值
        'lgb_params': {
    ...
    def __init__(self, config: dict = None):
        ...
        self.current_long = []
        self.current_short = []
        self.return_history = []  # V4: 记录收益历史用于趋势判断
        self.metadata = {
    ...
    def load_data(self, data_dir: str, start_date: str = None, end_date: str = None):
        ...
        print(f"[ReversalNeutralV4] 数据加载完成:")
    ...
    def _calculate_features(self):
        ...
        ema_12 = df.groupby('ts_code')['close'].transform(lambda x: x.ewm(span=12, min_periods=6).mean())
        ema_26 = df.groupby('ts_code')['close'].transform(lambda x: x.ewm(span=26, min_periods=13).mean())
        ...
        df['vol_ratio'] = df['volume'] / (df['volume_ma_20'] + 1)
    ...
    def train(self, train_end_date: str = None):
        ...
        ].dropna(subset=self.feature_cols + ['return_5d'])
        
        if len(train_data) < self.config['min_train_samples']:
            raise ValueError(f"训练样本不足: {len(train_data)}")
        ...
        print(f"[ReversalNeutralV4] 训练完成: {len(train_data):,} 样本")
    
    def _get_position_factor(self) -> float:
        """
        V4 核心：根据近期趋势调整仓位
        """
        if len(self.return_history) < self.config['trend_window']:
            return 1.0
        
        recent_returns = self.return_history[-self.config['trend_window']:]
        trend = np.mean(recent_returns)
        
        if trend > self.config['trend_threshold']:
            # 上涨趋势，加仓
            return self.config['trend_up_factor']
        elif trend < -self.config['trend_threshold']:
            # 下跌趋势，减仓
            return self.config['trend_down_factor']
        else:
            return 1.0
    ...
    def generate_signals(self, signal_date: str = None, force_rebalance: bool = False) -> dict:
        ...
        if not need_rebalance and self.last_rebalance_date:
            days_since = (signal_date - self.last_rebalance_date).days
            need_rebalance = days_since >= self.config['rebalance_days']
        
        day_data = self.data[self.data['date'] == signal_date].dropna(subset=self.feature_cols)
        
        if len(day_data) < self.config['top_n'] * 2:
            raise ValueError(f"有效股票数不足")
    ...
    def backtest(self, start_date: str, end_date: str = None, with_cost: bool = True) -> dict:
        ...
        for i, date in enumerate(dates):
            ...
            need_rebalance = (last_idx is None) or (i - last_idx >= rebalance_days)
            ...
            if need_rebalance:
                ...
                turnover = ((len(long_stocks - prev_long) + len(short_stocks - prev_short)) / (2 * top_n)
                           if prev_long else 1.0)
                prev_long, prev_short = long_stocks, short_stocks
            else:
                long_stocks, short_stocks = prev_long, prev_short
                turnover = 0
            ...
            gross_ret = (long_ret - short_ret) / 2 * position_factor
            net_ret = gross_ret - turnover * cost * 2 if with_cost else gross_ret
            
            self.return_history.append(net_ret)
            daily_returns.append({'date': date, 'return': net_ret, 'turnover': turnover})
    ...
    def save(self, save_dir: str):
        ...
        state = {
            ...
            'return_history': self.return_history[-50:]  # 只保存最近 50 天
        }
        with open(save_path / 'state.json', 'w') as f:
            json.dump(state, f, indent=2, default=str)
        
        print(f"[ReversalNeutralV4] 已保存到: {save_path}")
    ...
    def load(self, load_dir: str):
        ...
        print(f"[ReversalNeutralV4] 已加载: {load_path}")


if __name__ == '__main__':
    import argparse
    
    parser = argparse.ArgumentParser(description='Reversal Neutral V4')
    ...
    if args.action == 'train':
        ...
        print(f"仓位系数: {signal['position_factor']:.2f}")
    elif args.action == 'backtest':
        if generator.model is None:
            generator.train()
        result = generator.backtest(args.start_date, args.end_date)
        print(f"收益: {result['cum_return']*100:+.1f}%, 夏普: {result['sharpe']:.2f}")
