...
def add_market_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    daily_stats.columns = ['date', 'market_return', 'market_std', 'stock_count',
                           'market_return_1d', 'market_return_5d', 'market_vol']
    ...
    daily_stats['high_vol_regime'] = (daily_stats['market_vol'] > daily_stats['market_vol_ma']).astype(int)
    
    # 市场动量
    daily_stats['market_momentum'] = daily_stats['market_return_1d'].rolling(20).sum()
    
    # 合并回原数据
    market_cols = ['date', 'market_return', 'market_trend', 'high_vol_regime', 'market_momentum']
    data = data.merge(daily_stats[market_cols], on='date', how='left')
    ...
    # 相对收益
    data['excess_return'] = data['future_return'] - data['market_return']
...
def get_feature_cols():
    base = [
        ...
        'vol_ratio', 'vol_trend', 'vol_price_corr',
...
def rolling_train_backtest(data):
    ...
    # 滚动窗口设置
    train_months = 24  # 训练窗口 24 个月
    test_months = 3    # 测试窗口 3 个月
    ...
    for top_n in [30, 50, 100]:
        ...
        print(f"\n  Top {top_n}: 天数={total_days}, 胜率={win_rate:.1%}, "
              f"日均={avg_daily*100:.2f}%, 累计={total_return*100:.1f}%, "
        ...
        for year in sorted(daily_top['year'].unique()):
            ...
            if len(year_returns) > 0:
                ...
                print(f"    {year}: 年收益={y_total*100:.1f}%, 夏普={y_sharpe:.2f}")
    
    # 策略2: 多空对冲
    print("\n【策略2】多空对冲 (做多Top N + 做空Bottom N)")
    for n in [30, 50]:
        daily_long = full_result.groupby('date_str').apply(
            lambda x: x.nlargest(n, 'prob')['return'].mean()
        )
        daily_short = full_result.groupby('date_str').apply(
            lambda x: x.nsmallest(n, 'prob')['return'].mean()
        )
        
        # 多空收益 = 多头收益 - 空头收益 - 成本
        daily_ls = daily_long - daily_short - 4 * COST  # 双边各两次
        
        total_days = len(daily_ls)
        win_rate = (daily_ls > 0).mean()
        avg_daily = daily_ls.mean()
        total_return = (1 + daily_ls).prod() - 1
        sharpe = avg_daily / (daily_ls.std() + 1e-10) * np.sqrt(252)
        cumsum = daily_ls.cumsum()
        max_dd = (cumsum - cumsum.cummax()).min()
        
        print(f"\n  多空 {n}: 天数={total_days}, 胜率={win_rate:.1%}, "
              f"日均={avg_daily*100:.2f}%, 累计={total_return*100:.1f}%, "
              f"夏普={sharpe:.2f}, 回撤={max_dd*100:.1f}%")
        
        # 分年度
        daily_ls_df = pd.DataFrame({'date': daily_ls.index, 'return': daily_ls.values})
        daily_ls_df['year'] = pd.to_datetime(daily_ls_df['date']).dt.year
        for year in sorted(daily_ls_df['year'].unique()):
            year_returns = daily_ls_df[daily_ls_df['year'] == year]['return']
            if len(year_returns) > 0:
                y_total = (1 + year_returns).prod() - 1
                y_sharpe = year_returns.mean() / (year_returns.std() + 1e-10) * np.sqrt(252)
                print(f"    {year}: 年收益={y_total*100:.1f}%, 夏普={y_sharpe:.2f}")
    
    # 策略3: 市场择时 (牛市做多，熊市空仓或对冲)
    print("\n【策略3】市场择时 (趋势向上时做多，否则空仓)")
    for top_n in [30, 50]:
        daily_top = full_result.groupby('date_str').apply(
            lambda x: x.nlargest(top_n, 'prob')
        ).reset_index(drop=True)
...
if __name__ == '__main__':
    ...
    result = rolling_train_backtest(data)
