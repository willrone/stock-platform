#!/usr/bin/env python3
"""
双模型集成测试：LightGBM + XGBoost（不用 CatBoost）
"""
import pandas as pd
import numpy as np
...
def main():
    print("="*60)
    print("双模型集成测试：LightGBM + XGBoost")
    print("="*60)
    ...
    print(f"LightGBM 验证 AUC: {roc_auc_score(y_val, pred_lgb_val):.4f}")
    print(f"LightGBM 测试 AUC: {roc_auc_score(y_test, pred_lgb_test):.4f}")
    ...
    print(f"XGBoost 验证 AUC: {roc_auc_score(y_val, pred_xgb_val):.4f}")
    print(f"XGBoost 测试 AUC: {roc_auc_score(y_test, pred_xgb_test):.4f}")
    ...
    def objective(trial):
        ...
        pred = w * pred_lgb_val + (1 - w) * pred_xgb_val
    ...
    print(f"优化后权重: LGB={best_w:.3f}, XGB={1-best_w:.3f}")
    ...
    for name, weights in weight_schemes.items():
        pred = weights[0] * pred_lgb_test + weights[1] * pred_xgb_test
        auc = roc_auc_score(y_test, pred)
        
        test_df['prob'] = pred
        bt = backtest(test_df, top_n=5)
        
        results.append({
            'scheme': name,
            'weights': f"[{weights[0]:.2f}, {weights[1]:.2f}]",
            'auc': auc,
            'sharpe': bt['sharpe'],
        ...
        print(f"\n{name}:")
        print(f"  权重: LGB={weights[0]:.2f}, XGB={weights[1]:.2f}")
        print(f"  AUC: {auc:.4f}")
        print(f"  夏普比率: {bt['sharpe']:.2f}")
        print(f"  年化收益: {bt['annual_return']*100:.1f}%")
        print(f"  最大回撤: {bt['max_drawdown']*100:.1f}%")
    ...
    print(f"  夏普: {baseline['sharpe']:.2f}")
    print(f"  回撤: {baseline['max_drawdown']*100:.1f}%")
    
    print(f"\n最佳双模型集成 ({best['scheme']}):")
    print(f"  AUC: {best['auc']:.4f} ({'+' if best['auc'] > baseline['auc'] else ''}{(best['auc']-baseline['auc'])*100:.2f}%)")
    print(f"  夏普: {best['sharpe']:.2f} ({'+' if best['sharpe'] > baseline['sharpe'] else ''}{best['sharpe']-baseline['sharpe']:.2f})")
