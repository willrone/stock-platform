...
def compute_cross_sectional_features(df_all: pd.DataFrame) -> pd.DataFrame:
    ...
    df['market_volatility'] = df.groupby('date')['return_1d'].transform('std')
    ...
    # 相对强度
    df['relative_strength'] = df['return_20d'] / (df['market_return'].rolling(20).mean() + 1e-10)
...
def compute_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ========== 动量因子 ==========
    df['momentum_short'] = df['return_5d'] - df['return_20d']
    df['momentum_long'] = df['return_20d'] - df['return_60d']
    ...
    # 标签：绝对收益 > 0.15%（略低于 v3 的 0.2%）
    df['label'] = (df['future_return'] > 0.0015).astype(int)
    
    return df

def load_data(n_stocks=400, start_date='2018-01-01', end_date='2025-01-01'):
    """加载数据（增加股票数和时间范围）"""
...
def get_feature_cols():
    ...
    return [
        ...
        # 市场情绪
        'market_up_ratio', 'market_return', 'market_volatility',
        # MA
...
def train_with_optuna(X_train, y_train, X_val, y_val, n_trials=40):
    """使用 Optuna 进行超参优化"""
...
def train_and_evaluate(data, use_optuna=True, n_trials=40):
    """训练和评估"""
...
if __name__ == '__main__':
    print("="*60)
    print("LightGBM 股票预测模型 v4 - 修复版")
    print("="*60)
    
    data = load_data(n_stocks=400, start_date='2018-01-01', end_date='2025-01-01')
    
    model, result = train_and_evaluate(data, use_optuna=True, n_trials=40)
    
    model.save_model('/Users/ronghui/Documents/GitHub/willrone/experiments/lgbm_model_v4_fixed.txt')
    print("\n模型已保存到: lgbm_model_v4_fixed.txt")
    
    result.to_csv('/Users/ronghui/Documents/GitHub/willrone/experiments/lgbm_v4_fixed_results.csv', index=False)
    print("结果已保存到: lgbm_v4_fixed_results.csv")
