...
def test_basic_strategies():
    ...
    for strategy_name in basic_strategies:
        try:
            ...
            print(
                f"✓ {strategy_name}: 买入信号 {len(buy_signals)}, 卖出信号 {len(sell_signals)}"
            )
...
def test_advanced_strategies():
    ...
    strategies_to_test = [
        ...
        (
            "配对交易",
            PairsTradingStrategy,
            {"lookback_period": 20, "entry_threshold": 2.0},
        ),
        (
            "均值回归",
            MeanReversionStrategy,
        ...
        (
            "协整",
            CointegrationStrategy,
            {"lookback_period": 60, "entry_threshold": 2.0},
        ),
    ...
    for name, StrategyClass, config in strategies_to_test:
        try:
            ...
            min_data = (
                130
                if name in ["协整", "价值因子", "多因子"]
                else (50 if name in ["布林带", "均值回归", "配对交易"] else 70)
            )
            data = generate_mock_price_data(datetime(2023, 1, 1), 300)
            data.attrs["stock_code"] = "000001.SZ"
            ...
            if len(buy_signals) == 0 and len(sell_signals) == 0:
                ...
                print(
                    f"✓ {name}: 买入信号 {len(buy_signals)}, 卖出信号 {len(sell_signals)}"
                )
