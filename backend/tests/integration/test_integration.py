...
class TestIntegration:
    ...
    def sample_task_request(self):
        ...
        return {
            ...
            "model_id": "xgboost_v1",
            "prediction_config": {"horizon": "short_term", "confidence_level": 0.95},
        }
    ...
    def test_data_synchronization_flow(self, client):
        ...
        sync_request = {"stock_codes": ["000001.SZ"], "force_update": False}
        ...
        if response.status_code == 200:
            ...
            if sync_data.get("success"):
                assert (
                    "synced_files" in sync_data["data"]
                    or "total_files" in sync_data["data"]
                )
    ...
    def test_stock_data_retrieval_flow(self, client):
        ...
        response = client.get(
            "/api/v1/stocks/000001.SZ/indicators",
            params={
    ...
    def test_error_handling_mechanisms(self, client):
        ...
        if response.status_code == 200:
            ...
            assert (
                ...
                or len(data.get("data", {}).get("indicators", {})) == 0
            )
    ...
    async def test_websocket_task_subscription(self):
        ...
        with TestClient(app) as client:
            with client.websocket_connect("/ws") as websocket:
                ...
                websocket.send_json({"type": "subscribe:task", "task_id": task_id})
                ...
                # 取消订阅
                websocket.send_json({"type": "unsubscribe:task", "task_id": task_id})
    ...
    def test_data_consistency(self, client, sample_task_request):
        ...
        for i in range(1, len(responses)):
            ...
            assert (
                responses[i]["data"]["task_name"] == responses[0]["data"]["task_name"]
            )
