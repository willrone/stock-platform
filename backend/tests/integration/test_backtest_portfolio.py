...
from app.services.backtest import BacktestConfig, BacktestExecutor, StrategyFactory
...
class TestPortfolioBacktestIntegration:
    ...
    def test_portfolio_signal_generation(self, sample_stock_data):
        ...
        config = {
            "strategies": [
                {"name": "rsi", "weight": 0.5, "config": {"rsi_period": 14}},
                {
                    "name": "macd",
                    "weight": 0.5,
        ...
        if signals:
            assert (
                "integration_method" in signals[0].metadata
                or "source_signals" in signals[0].metadata
            )
    ...
    def test_single_strategy_equivalence(self):
        ...
        # 创建组合策略（只包含1个策略，权重=1）
        portfolio_config = {
            "strategies": [{"name": "rsi", "weight": 1.0, "config": single_config}]
        }
        portfolio_strategy = StrategyFactory.create_strategy(
    ...
    def test_portfolio_backtest_execution(self, mock_data_loader, sample_stock_data):
        ...
        mock_loader.load_multiple_stocks.return_value = {"000001.SZ": sample_stock_data}
    ...
    def test_portfolio_add_remove_strategy(self):
        """测试动态添加和移除策略"""
        config = {
            "strategies": [{"name": "rsi", "weight": 1.0, "config": {"rsi_period": 14}}]
        }
