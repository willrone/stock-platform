...
class TestBacktestDataAdapterProperties:
    ...
    def test_backtest_overview_data_completeness(
        self, num_trades, num_snapshots, initial_cash, final_value_multiplier
    ):
        """
    ...
    def test_extended_risk_metrics_calculation_accuracy(
        self, portfolio_length, volatility_level
    ):
        """
        ...
        daily_returns = np.random.normal(
            0.0005, volatility_level / np.sqrt(252), portfolio_length
        )
        portfolio_values = initial_cash * np.exp(np.cumsum(daily_returns))

        portfolio_history = []
        for _i, (date, value) in enumerate(zip(dates, portfolio_values)):
            snapshot = {
                "date": date.isoformat(),
        ...
        volatility_error = (
            abs(calculated_volatility - expected_volatility) / expected_volatility
        )
    ...
    @settings(max_examples=50)
    def test_monthly_returns_analysis_consistency(
        self, num_months, monthly_return_mean, monthly_return_std
    ):
        """
        ...
        for i, monthly_data in enumerate(monthly_returns):
            ...
            # 验证收益率的合理性
            assert (
                -0.5 < monthly_data.monthly_return < 0.5
            ), f"月度收益率 {monthly_data.monthly_return} 应该在合理范围内"

            # 验证累积收益的单调性（允许小幅波动）
            if i > 0:
                monthly_returns[i - 1].cumulative_return
    ...
    def test_position_analysis_completeness(self, num_stocks, trades_per_stock):
        ...
        assert (
            len(position_analysis) == num_stocks
        ), f"应该分析 {num_stocks} 只股票，实际分析了 {len(position_analysis)} 只"
        ...
        for position_data in position_analysis:
            ...
            assert not np.isnan(position_data.win_rate)
            assert (
                0 <= position_data.win_rate <= 1
            ), f"胜率 {position_data.win_rate} 应该在0-1之间"

            # 验证交易统计的一致性
            assert (
                position_data.trade_count == trades_per_stock
            ), f"交易次数应该是 {trades_per_stock}"
            assert (
                position_data.winning_trades + position_data.losing_trades
                <= position_data.trade_count
            )

            # 验证胜率计算的准确性
            if position_data.trade_count > 0:
                expected_win_rate = (
                    position_data.winning_trades / position_data.trade_count
                )
                win_rate_error = abs(position_data.win_rate - expected_win_rate)
                assert (
                    win_rate_error < 0.001
                ), f"胜率计算错误: 期望 {expected_win_rate}, 实际 {position_data.win_rate}"
