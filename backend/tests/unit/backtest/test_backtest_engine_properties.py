...
class TestBacktestEngineAccuracy:
    ...
    def test_strategy_signal_generation_accuracy(
        self, strategy_name, short_window, long_window, initial_cash
    ):
        """
        ...
        try:
            ...
            # 验证指标数据类型和长度
            for _name, indicator in indicators.items():
    ...
    def test_portfolio_management_accuracy(
        self, initial_cash, commission_rate, max_position_size
    ):
        """
        功能: production-ready-implementation, 属性 3: 回测引擎准确性
        ...
        if trade:  # 如果有足够资金执行交易
            ...
            expected_value = (
                portfolio_manager.cash + position.quantity * current_prices["000001.SZ"]
            )
    ...
    def test_backtest_execution_accuracy(self, data_length, strategy_name):
        ...
        extended_data["high"] = np.maximum(
            extended_data["high"], extended_data["close"]
        )
        extended_data["low"] = np.minimum(extended_data["low"], extended_data["close"])

        # 保存扩展数据
        extended_data_path = os.path.join(
            self.temp_dir, "daily", "000001.SZ", "extended.parquet"
        )
