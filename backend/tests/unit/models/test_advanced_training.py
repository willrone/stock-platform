...
class TestEnsembleConfig:
    ...
    def test_ensemble_config_defaults(self):
        """测试集成配置默认值"""
        config = EnsembleConfig(method=EnsembleMethod.WEIGHTED, base_models=["model1"])
...
class TestOnlineLearningManager:
    ...
    async def test_should_retrain_detection(self):
        ...
        assert should_retrain is True  # 性能下降超过阈值

        # 测试性能稳定的情况
        history["accuracies"] = [0.85, 0.84, 0.86, 0.85, 0.87]  # 性能稳定
        should_retrain = await manager._should_retrain("test_model", current_metrics)

        assert should_retrain is False  # 性能稳定，不需要重训练
...
if __name__ == "__main__":
    ...
    async def run_basic_tests():
        ...
        # 测试集成配置
        config = EnsembleConfig(
            method=EnsembleMethod.VOTING, base_models=["model1", "model2"]
        )
        ...
        # 测试集成管理器
        mock_training_service = Mock()
        EnsembleModelManager(mock_training_service)
        print("✓ 集成模型管理器创建成功")

        # 测试在线学习管理器
        OnlineLearningManager(mock_training_service)
        print("✓ 在线学习管理器创建成功")
