...
class TestLabelCalculation:
    ...
    def test_process_stock_data_with_prediction_horizon(self, sample_data):
        ...
        assert (
            processed["label"].iloc[-prediction_horizon:].isna().all()
            or (processed["label"].iloc[-prediction_horizon:] == 0).all()
        ), "最后N行标签应该是NaN或0"
    ...
    def test_process_stock_data_multiindex(self, multiindex_data):
        ...
        if isinstance(processed.index, pd.MultiIndex):
            ...
            future_prices = processed.groupby(level=0)["$close"].shift(
                -prediction_horizon
            )
        else:
            # 如果已经是单层索引，直接shift
    ...
    def test_label_calculation_different_horizons(self, sample_data):
        ...
        for horizon in [1, 5, 10]:
            ...
            assert (
                last_n.isna().all() or (last_n == 0).all()
...
class TestMissingValueHandling:
    ...
    def data_with_missing_values(self):
        ...
        data.loc[data.index[:3], "indicator1"] = (
            np.nan
        )  # 技术指标开头缺失（计算窗口不足）
...
class TestRobustFeatureScaler:
    ...
    def test_robust_scaler_fit_transform(self, sample_features):
        ...
        # 验证标签没有被标准化
        assert np.allclose(
            scaled_data["label"].values, sample_features["label"].values
        ), "标签不应该被标准化"
    ...
    def test_robust_scaler_transform(self, sample_features):
        ...
        assert not np.allclose(
            test_scaled["feature1"].values, test_data["feature1"].values
        ), "测试集特征应该被转换"
    ...
    def test_robust_scaler_without_sklearn(self, sample_features, monkeypatch):
        ...
        try:
            ...
            # 验证数据没有被修改（因为跳过了标准化）
            assert np.allclose(
                result["feature1"].values,
                sample_features["feature1"].values,
                equal_nan=True,
            ), "sklearn不可用时数据应该保持不变"
        finally:
            # 恢复原始模块
...
class TestOutlierHandler:
    ...
    def test_outlier_handler_winsorize(self, data_with_outliers):
        ...
        processed = handler.handle_label_outliers(
            data_with_outliers.copy(), label_col="label"
        )
    ...
    def test_outlier_handler_no_label_column(self, data_with_outliers):
        ...
        processed = handler.handle_label_outliers(
            data_no_label.copy(), label_col="label"
        )
    ...
    def test_outlier_handler_extreme_returns(self):
        ...
        assert processed["label"].max() <= max(
            upper_bound, data["label"].max()
        ), "极端值应该被Winsorize截断"
