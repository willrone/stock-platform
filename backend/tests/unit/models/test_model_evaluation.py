...
class TestModelVersioning:
    ...
    def test_model_comparison(self):
        """测试模型比较逻辑"""
        # 创建两个模型的指标
        metrics1 = {"sharpe_ratio": 1.2, "accuracy": 0.75, "max_drawdown": -0.15}

        metrics2 = {"sharpe_ratio": 1.5, "accuracy": 0.80, "max_drawdown": -0.10}
        ...
        best_by_drawdown = min(
            [metrics1, metrics2], key=lambda x: abs(x["max_drawdown"])
        )
...
class TestFinancialMetricsCalculator:
    ...
    def test_calculate_max_drawdown(self):
        ...
        positive_dd = FinancialMetricsCalculator.calculate_max_drawdown(
            positive_returns
        )
