            ...
            features_data.append(
                {
                    ...
                    "open": current_price
                    * draw(st.floats(min_value=0.99, max_value=1.01)),
                    "high": current_price
                    * draw(st.floats(min_value=1.0, max_value=1.02)),
                    "low": current_price
                    * draw(st.floats(min_value=0.98, max_value=1.0)),
                    "close": current_price,
                    "volume": draw(st.integers(min_value=1000000, max_value=5000000)),
                    "ma_5": current_price
                    * draw(st.floats(min_value=0.99, max_value=1.01)),
                    "ma_10": current_price
                    * draw(st.floats(min_value=0.98, max_value=1.02)),
                    "ma_20": current_price
                    * draw(st.floats(min_value=0.97, max_value=1.03)),
                    "ma_60": current_price
                    * draw(st.floats(min_value=0.96, max_value=1.04)),
                    ...
                    * draw(st.floats(min_value=1.01, max_value=1.03)),
                    "bb_lower": current_price
                    * draw(st.floats(min_value=0.97, max_value=0.99)),
                }
            )
...
class MockModelVersionManager:
    ...
    def save_model_version(
        self, model_id, model, model_type, parameters, metrics, training_data
    ):
        """保存模型版本"""
        # 生成版本信息
...
class TestModelTrainingResultsSaving:
    ...
    def test_model_training_results_saving_property(
        self, features_data, config, metrics
    ):
        """
        ...
        **验证：需求 3.3**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 生成模型ID
            model_id = (
                f"test_model_{hash(str(features_data.iloc[0].to_dict())) % 10000}"
            )
    ...
    @given(
        ...
        model_type=st.sampled_from(
            ["xgboost", "lstm", "transformer", "timesnet", "patchtst", "informer"]
        ),
        metrics=model_metrics_strategy(),
    )
    ...
    @given(metrics_list=st.lists(model_metrics_strategy(), min_size=2, max_size=5))
    ...
    def test_best_model_selection_property(self, metrics_list):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 按夏普比率选择最佳模型
            best_by_sharpe = max(
                all_versions, key=lambda v: v["metrics"]["sharpe_ratio"]
            )
            expected_best_sharpe = max(metrics_list, key=lambda m: m["sharpe_ratio"])
            assert (
                best_by_sharpe["metrics"]["sharpe_ratio"]
                == expected_best_sharpe["sharpe_ratio"]
            )
            ...
            assert (
                best_by_accuracy["metrics"]["accuracy"]
                == expected_best_accuracy["accuracy"]
            )

            # 按最大回撤选择最佳模型（绝对值最小）
            best_by_drawdown = min(
                all_versions, key=lambda v: abs(v["metrics"]["max_drawdown"])
            )
            expected_best_drawdown = min(
                metrics_list, key=lambda m: abs(m["max_drawdown"])
            )
            assert (
                best_by_drawdown["metrics"]["max_drawdown"]
                == expected_best_drawdown["max_drawdown"]
            )
    ...
    def test_training_data_consistency_property(self, training_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            MockModelVersionManager(temp_dir)
