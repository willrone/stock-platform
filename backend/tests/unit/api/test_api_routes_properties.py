...
def date_ranges(draw):
    ...
    start_date = draw(st.datetimes(min_value=datetime(2020, 1, 1), max_value=end_date))
...
class TestAPIRoutesProperties:
    ...
    async def test_technical_indicators_api_calls_real_service(
        ...
        **功能: data-management-implementation, 属性 1: API路由真实服务调用**
        ...
        with (
            patch("app.core.container.get_data_service") as mock_get_data_service,
            patch(
                "app.core.container.get_indicators_service"
            ) as mock_get_indicators_service,
        ):
            ...
            mock_indicators_service.calculate_indicators.return_value = (
                mock_indicator_results
            )
