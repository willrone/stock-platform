...
class TestPositionAnalyzer:
    ...
    @pytest.mark.asyncio
    async def test_analyze_position_performance_basic(
        self, analyzer, sample_trade_history
    ):
        """测试基本的持仓表现分析"""
        result = await analyzer.analyze_position_performance(sample_trade_history, [])
    ...
    async def test_stock_performance_analysis(self, analyzer, sample_trade_history):
        ...
        for stock in stock_performance:
            ...
            assert isinstance(
                stock.get("total_return", stock.get("avg_return_per_trade", 0)),
                (int, float),
            )
    ...
    async def test_trade_count_calculation(self, analyzer, sample_trade_history):
        ...
        if stock_000001:
            ...
            # 或者验证其他指标存在
            assert (
                "total_return" in stock_000001 or "avg_return_per_trade" in stock_000001
            )
