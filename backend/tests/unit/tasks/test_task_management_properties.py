...
class TestTaskManagementIntegrity:
    ...
    def test_task_creation_integrity(self, task_name, task_type, user_id, config):
        ...
        try:
            ...
            task = self.task_repository.create_task(
                task_name, task_type, user_id, config
            )
    ...
    @settings(max_examples=100)
    def test_task_status_update_integrity(
        self, initial_status, target_status, progress
    ):
        """
        功能: production-ready-implementation, 属性 2: 任务管理完整性
    ...
    def test_user_task_management_integrity(self, user_id, task_count):
        ...
        if task_count > 0:
            ...
            # 删除任务
            delete_result = self.task_repository.delete_task(
                completed_task.task_id, user_id
            )
    ...
    def test_task_statistics_integrity(self, task_count, days_back):
        ...
        for _task, status in created_tasks:
            status_value = status.value
            expected_status_counts[status_value] = (
                expected_status_counts.get(status_value, 0) + 1
            )
...
class TestTaskQueueReliability:
    ...
    def test_task_queue_reliability(self, task_count, priority):
        ...
        while (
            len(self.test_results) < task_count
...
class TestNotificationServiceReliability:
    ...
    async def test_notification_service_reliability(
        ...
        # 验证用户自动订阅
        subscriptions = await self.notification_service.get_user_task_subscriptions(
            user_id
        )
    ...
    async def test_subscription_management_reliability(self, user_id, task_ids):
        ...
        # 验证订阅
        user_subscriptions = (
            await self.notification_service.get_user_task_subscriptions(user_id)
        )
        ...
        if len(task_ids) > 1:
            ...
            await self.notification_service.unsubscribe_user_from_task(
                user_id, unsubscribe_task
            )
            ...
            unsubscribe_subscribers = (
                await self.notification_service.get_task_subscribers(unsubscribe_task)
            )
            assert user_id not in unsubscribe_subscribers

    @given(notification_count=st.integers(min_value=1, max_value=20))
    @settings(max_examples=30)
    def test_notification_statistics_integrity(self, notification_count):
        ...
        # 模拟发送通知（通过直接更新统计）
        for _i in range(notification_count):
