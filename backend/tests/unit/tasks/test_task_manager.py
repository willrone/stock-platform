...
class TestTaskManager:
    ...
    def test_update_task_status(self):
        ...
        success = self.task_manager.update_task(
            TaskUpdateRequest(task_id=task_id, status=TaskStatus.RUNNING, progress=25.0)
        )
    ...
    def test_query_tasks(self):
        ...
        self.task_manager.update_task(
            TaskUpdateRequest(task_ids[0], status=TaskStatus.RUNNING)
        )
        self.task_manager.update_task(
            TaskUpdateRequest(task_ids[1], status=TaskStatus.COMPLETED)
        )
        ...
        # 查询运行中的任务
        running_tasks = self.task_manager.query_tasks(
            TaskQuery(status=TaskStatus.RUNNING)
        )
        assert len(running_tasks) == 1
        assert running_tasks[0].status == TaskStatus.RUNNING

        # 查询已完成的任务
        completed_tasks = self.task_manager.query_tasks(
        ...
        # 查询待处理的任务
        pending_tasks = self.task_manager.query_tasks(
    ...
    def test_query_tasks_with_stock_filter(self):
        ...
        # 按股票代码过滤
        filtered_tasks = self.task_manager.query_tasks(
            TaskQuery(stock_code="000001.SZ")
        )
        assert len(filtered_tasks) == 1
        assert (
            "000001.SZ" in filtered_tasks[0].name
            or "股票过滤测试1" == filtered_tasks[0].name
        )
    ...
    def test_task_statistics(self):
        ...
        self.task_manager.update_task(
            TaskUpdateRequest(task_ids[2], status=TaskStatus.FAILED)
        )
    ...
    def test_task_callbacks(self):
        ...
        # 更新状态和进度
        self.task_manager.update_task(
            TaskUpdateRequest(task_id=task_id, status=TaskStatus.RUNNING, progress=50.0)
        )
    ...
    def test_nonexistent_task_operations(self):
        ...
        # 更新不存在的任务
        success = self.task_manager.update_task(
            TaskUpdateRequest(task_id=nonexistent_id, status=TaskStatus.RUNNING)
        )
        assert not success
