...
def task_create_request_strategy(draw):
    ...
    stock_codes = draw(
        st.lists(stock_code_strategy(), min_size=1, max_size=5, unique=True)
    )
    indicators = draw(
        st.lists(
            st.sampled_from(["MA5", "MA10", "RSI", "MACD", "BOLLINGER"]),
            min_size=1,
            max_size=3,
            unique=True,
        )
    ...
    models = draw(
        st.lists(
            st.sampled_from(["LSTM", "XGBoost", "Transformer"]),
            min_size=1,
            max_size=2,
            unique=True,
        )
    ...
    def test_task_status_transition_property(self, request, status_sequence):
        ...
        for i, new_status in enumerate(status_sequence):
            ...
            update_request = TaskUpdateRequest(
                task_id=task_id, status=new_status, progress=progress
            )
    ...
    def test_task_query_consistency_property(self, requests, query_status):
        ...
        if query_status and created_task_ids:
            ...
            for task_id in created_task_ids[: len(created_task_ids) // 2]:
                self.task_manager.update_task(
                    TaskUpdateRequest(task_id=task_id, status=query_status)
                )

        # 执行查询
        query = TaskQuery(status=query_status, limit=50, offset=0)
        ...
        for result in results:
            ...
            if query_status:
                ...
                ), f"查询结果状态应该匹配过滤条件: {result.status} == {query_status}"
    ...
    def test_concurrent_task_operations(self):
        ...
        def create_task_worker(worker_id):
            try:
                ...
                self.task_manager.update_task(
                    TaskUpdateRequest(
                        task_id=task_id, status=TaskStatus.RUNNING, progress=50.0
                    )
                )
