...
def stock_data_strategy(draw):
    ...
    for i in range(days_count):
        ...
        # 确保价格逻辑正确
        open_price = draw(
            st.floats(min_value=close_price * 0.95, max_value=close_price * 1.05)
        )
...
class TestTechnicalIndicatorProperties:
        ...
        try:
            ...
            for result in results:
                ...
                for indicator_name, value in result.indicators.items():
                    assert isinstance(
                        value, (int, float)
                    ), f"指标值应该是数值类型: {indicator_name}"
                    assert not (
                        isinstance(value, float) and (value != value)
                    ), f"指标值不应该是NaN: {indicator_name}"  # 检查NaN
                    ...
                    if indicator_name == "RSI":
                        ...
                        "MA"
                    ) and not indicator_name.startswith("MACD"):
                        # 只有移动平均线（MA5, MA10等）应该大于0，MACD相关指标可以为负值
                        assert (
                            value > 0
                        ), f"移动平均线应该大于0: {indicator_name}={value}"
                    elif indicator_name.startswith("BOLLINGER"):
                        # 布林带指标应该大于0
                        assert (
                            value > 0
                        ), f"布林带指标应该大于0: {indicator_name}={value}"
    ...
    @given(
        ...
        indicators=st.lists(
            st.sampled_from(["MA5", "MA10", "RSI"]), min_size=1, max_size=3, unique=True
        ),
    )
            ...
            async def async_test():
                with tempfile.TemporaryDirectory() as temp_dir:
                    data_service = SimpleStockDataService(
                        data_path=temp_dir, remote_url="http://192.168.3.62"
                    )
                    ...
                    # 执行批量计算
                    response = await self.calculator.calculate_batch_indicators(
                        request, data_service
                    )
                    ...
                    assert isinstance(
                        response.failed_stocks, list
                    ), "failed_stocks应该是列表"
                    assert isinstance(response.message, str), "message应该是字符串"

                    # 验证处理结果
                    total_processed = len(response.results) + len(
                        response.failed_stocks
                    )
                            ...
                            for result in results:
                                ...
                                for requested_indicator in indicators:
                                    ...
                                    has_related_indicator = any(
                                        requested_indicator in name
                                        for name in result_indicator_names
                                    )
                                    if (
                                        not has_related_indicator and len(results) > 20
                                    ):  # 只有在有足够数据时才要求
                                        # 某些指标可能需要更多数据才能计算
                                        pass
