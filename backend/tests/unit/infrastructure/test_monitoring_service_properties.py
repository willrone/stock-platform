...
def error_data(draw):
    ...
    return {
        ...
        "error_type": draw(
            st.sampled_from(
                ["ConnectionError", "TimeoutError", "ValidationError", "DataError"]
            )
        ),
...
class TestMonitoringServiceProperties:
    ...
    async def test_health_check_accuracy(self, service_name):
        ...
        **验证: 需求 5.1, 5.2**
        ...
        if service_name == "data_service":
            ...
            self.mock_data_service.check_remote_service_status.return_value = (
                DataServiceStatus(
            ...
            self.mock_indicators_service.calculate_moving_average.return_value = [
                None,
                None,
                2.0,
            ]
    ...
    async def test_health_check_failure_detection(self, service_name):
        ...
        **验证: 需求 5.1, 5.2**
        ...
        if service_name == "data_service":
            ...
            self.mock_indicators_service.calculate_moving_average.side_effect = (
                Exception("计算失败")
            )
        elif service_name == "parquet_manager":
        ...
        **验证: 需求 5.1, 5.3**
        ...
        for perf_data in perf_data_list:
            test_monitoring_service.record_request(
                service_name, perf_data["response_time"], perf_data["success"]
            )
    ...
    async def test_anomaly_detection_accuracy(self):
        ...
        **验证: 需求 5.2, 5.4**
        ...
        for i in range(10):
            response_time = (
                8000.0 if i < 7 else 1000.0
            )  # 前7个响应很慢，平均值会超过5000ms
