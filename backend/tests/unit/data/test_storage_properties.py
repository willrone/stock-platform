...
def stock_data_list_strategy(draw):
    ...
    for i in range(days_count):
        ...
        open_price = draw(
            st.floats(min_value=close_price * 0.98, max_value=close_price * 1.02)
        )
...
class TestStorageFormatConsistency:
    ...
    def test_parquet_storage_consistency_property(self, stock_data_list):
        ...
        # 保存数据
        success = self.parquet_manager.save_stock_data(
            stock_data_list, merge_with_existing=False
        )
        ...
        # 增量保存
        success = self.parquet_manager.save_stock_data(
            [new_data], merge_with_existing=True
        )
        assert success, "增量保存应该成功"

        # 验证增量数据
        extended_end_date = new_date
        all_loaded_data = self.parquet_manager.load_stock_data(
            stock_code, start_date, extended_end_date
        )
        assert (
            len(all_loaded_data) == len(stock_data_list) + 1
        ), "增量更新后数据数量应该正确"
    ...
    @given(
        ...
        indicators=st.lists(
            st.sampled_from(["MA5", "RSI", "MACD"]), min_size=1, max_size=3, unique=True
        ),
    )
    @settings(max_examples=10, deadline=None)
    def test_database_storage_consistency_property(
        self, task_name, stock_codes, indicators
    ):
        """
        属性测试：数据库存储格式一致性
        ...
        for stock_code in stock_codes:
            ...
            with self.db_manager.get_connection() as conn:
                ...
                """,
                    (
                        task_result.task_id,
                        task_result.stock_code,
                        task_result.prediction_date,
                        task_result.prediction_value,
                        task_result.confidence,
                        task_result.model_name,
                        task_result.indicators_used,
                        task_result.backtest_metrics,
            ...
            # 验证任务结果
            result_row = self.db_manager.fetch_one(
            ...
            assert (
                abs(result_row["prediction_value"] - task_result.prediction_value)
                < 0.01
            ), "预测值应该一致"
            assert (
                abs(result_row["confidence"] - task_result.confidence) < 0.01
            ), "置信度应该一致"
