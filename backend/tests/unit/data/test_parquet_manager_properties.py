...
from app.models.file_management import FileFilters, FilterCriteria, IntegrityStatus
...
class TestParquetManagerProperties:
    ...
    async def test_file_save_and_list_consistency(self, stock_data):
        ...
        **验证: 需求 2.1, 2.2, 2.3, 2.4, 2.5**
        ...
        for file_info in file_list:
            ...
            # 验证文件完整性
            validation_result = self.manager.validate_file_integrity(
                file_info.file_path
            )
            assert (
                validation_result.is_valid
                or validation_result.integrity_status != IntegrityStatus.CORRUPTED
            )
    ...
    async def test_comprehensive_stats_accuracy(self, stock_data):
        ...
        **功能: data-management-implementation, 属性 2: Parquet文件管理完整性**
        ...
        assert (
            abs(stats.average_file_size - expected_avg) < 0.01
        ), "平均文件大小计算错误"
    ...
    async def test_file_deletion_consistency(self, stock_data, num_files_to_delete):
        ...
        **功能: data-management-implementation, 属性 2: Parquet文件管理完整性**
        ...
        # 选择要删除的文件
        files_to_delete = [
            f.file_path for f in file_list[: min(num_files_to_delete, len(file_list))]
        ]
        ...
        # 验证删除结果
        assert len(deletion_result.deleted_files) + len(
            deletion_result.failed_files
        ) == len(files_to_delete)
        ...
        # 验证失败的文件仍然存在
        for _failed_file, _ in deletion_result.failed_files:
    ...
    @pytest.mark.asyncio
    @given(
        st.integers(min_value=1, max_value=5), st.integers(min_value=1, max_value=20)
    )
    ...
    async def test_pagination_consistency(self, num_stocks, records_per_stock):
        ...
        **功能: data-management-implementation, 属性 2: Parquet文件管理完整性**
        ...
        # 创建多个股票的数据
        all_stock_data = []
        for _i in range(num_stocks):
            stock_code = "{i+1:06d}.SZ"
            base_date = datetime(2023, 1, 1)
        ...
        page1 = self.manager.get_detailed_file_list(
            FileFilters(limit=page_size, offset=0)
        )
        page2 = self.manager.get_detailed_file_list(
            FileFilters(limit=page_size, offset=page_size)
        )
