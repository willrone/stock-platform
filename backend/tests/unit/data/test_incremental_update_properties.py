    ...
    async def run_test():
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_data = data_service.generate_mock_data(
                stock_code, first_start, first_end
            )
            ...
            # 测试获取现有数据
            data1 = await data_service.get_stock_data(
                stock_code, first_start, first_end
            )
            ...
            missing_ranges = data_service.identify_missing_date_ranges(
                stock_code, second_start, second_end
            )
...
@given(stock_code=stock_code_strategy(), date_ranges=overlapping_date_ranges_strategy())
        ...
        async def async_test():
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if data1 is not None and len(data1) > 0:
                    ...
                    # 第二次获取数据（可能重叠或扩展）
                    data2 = await data_service.get_stock_data(
                        stock_code, second_start, second_end
                    )
                    ...
                    if data2 is not None and len(data2) > 0:
                        ...
                        # 验证数据覆盖了请求的范围
                        assert (
                            data_start.date() <= second_start.date()
                        ), "数据应该覆盖请求的开始日期"
                        assert (
                            data_end.date() >= second_end.date()
                        ), "数据应该覆盖请求的结束日期"
                        ...
                        for i in range(len(sorted_dates) - 1):
                            ...
                            assert (
                                days_diff <= 3
                            ), f"数据日期间隔过大: {current_date} -> {next_date}"
        ...
        async def async_test():
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                data = await data_service.get_stock_data(
                    stock_code, base_start, base_end
                )
                ...
                if data is not None and len(data) > 0:
                    ...
                    for req_start, req_end in test_cases:
                        ...
                        for missing_start, missing_end in missing_ranges:
                            ...
                            assert (
                                req_start <= missing_end
                            ), "缺失范围应该与请求范围有交集"
                            assert (
                                missing_start <= req_end
                            ), "缺失范围应该与请求范围有交集"
    ...
    async def run_test():
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if data1 is not None and len(data1) > 0:
                ...
                start_time = time.time()
                data3 = await data_service.get_stock_data(
                    stock_code, start_date, extended_end
                )
