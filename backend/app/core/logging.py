...
def log_audit(action: str, user_id: str = None, resource: str = None, **kwargs):
    ...
    logger.bind(log_type="audit").info(
        f"AUDIT | {json.dumps(audit_data)}"
    )


def log_access(method: str, path: str, status_code: int, duration: float, 
    ...
    logger.bind(log_type="access").info(
        f"ACCESS | {json.dumps(access_data)}"
    )


def log_data_sync(operation: str, stock_code: str = None, status: str = "success", 
                  records: int = 0, **kwargs):
    """记录数据同步日志"""
    sync_data = {
        "operation": operation,
    ...
    logger.bind(log_type="data_sync").info(
        f"DATA_SYNC | {json.dumps(sync_data)}"
    )
...
def log_task(task_id: str, task_type: str, status: str, **kwargs):
    ...
    logger.bind(log_type="task").info(
        f"TASK | {json.dumps(task_data)}"    )