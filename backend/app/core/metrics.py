...
http_requests_total = Counter(
    "http_requests_total", "HTTP请求总数", ["method", "endpoint", "status"]
)

# HTTP请求持续时间直方图
http_request_duration_seconds = Histogram(
    "http_request_duration_seconds", "HTTP请求持续时间（秒）", ["method", "endpoint"]
)
...
# 数据库连接数
database_connections_active = Gauge("database_connections_active", "活跃数据库连接数")

# 任务队列大小
task_queue_size = Gauge("task_queue_size", "任务队列中的任务数量")

# 预测任务计数器
prediction_tasks_total = Counter("prediction_tasks_total", "预测任务总数", ["status"])

# 模型预测时间
model_prediction_duration_seconds = Histogram(
    "model_prediction_duration_seconds", "模型预测时间（秒）", ["model_type"]
)

# 数据同步计数器
data_sync_total = Counter("data_sync_total", "数据同步总数", ["source", "status"])

# 错误计数器
errors_total = Counter("errors_total", "错误总数", ["error_type", "endpoint"])

# 系统信息
app_info = Info("app_info", "应用程序信息")

# 内存使用量
memory_usage_bytes = Gauge("memory_usage_bytes", "内存使用量（字节）")

# CPU使用率
cpu_usage_percent = Gauge("cpu_usage_percent", "CPU使用率（百分比）")
...
class MetricsCollector:
    ...
    def record_request(
        ...
        try:
            ...
            http_requests_total.labels(
                method=method, endpoint=clean_endpoint, status=str(status_code)
            ).inc()
    ...
    def record_error(self, error_type: str, endpoint: str):
        ...
        try:
            clean_endpoint = self._clean_endpoint(endpoint)
            errors_total.labels(error_type=error_type, endpoint=clean_endpoint).inc()
        except Exception as e:
