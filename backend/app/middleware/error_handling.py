...
class ErrorHandlingMiddleware(BaseHTTPMiddleware):
    ...
    async def _handle_validation_error(
        ...
        return JSONResponse(
            status_code=400,
            content=StandardResponse(
                success=False, message=f"数据验证失败: {str(exc)}", data=None
            ).model_dump(mode="json"),
        )
    ...
    async def _handle_connection_error(
        ...
        return JSONResponse(
            status_code=503,
            content=StandardResponse(
                success=False, message="服务暂时不可用，请稍后重试", data=None
            ).model_dump(mode="json"),
        )
    ...
    async def _handle_timeout_error(
        ...
        return JSONResponse(
            status_code=504,
            content=StandardResponse(
                success=False, message="请求超时，请稍后重试", data=None
            ).model_dump(mode="json"),
        )
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            logger.info(
                f"请求完成: {request_info['method']} {request_info['url']} - {response_info}"
            )
            ...
            logger.error(
                f"请求失败: {request_info['method']} {request_info['url']} - {error_info}"
            )
            raise
