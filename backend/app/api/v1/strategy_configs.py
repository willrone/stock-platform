...
async def get_strategy_config(config_id: str):
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            return StandardResponse(
                success=True, message="获取配置详情成功", data=config.to_dict()
            )
    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                logger.debug(
                    f"准备保存配置对象: config_id={config_id}, parameters类型={type(cleaned_parameters)}"
                )
                session.add(config)
                ...
                return StandardResponse(
                    success=True, message="保存配置成功", data=config.to_dict()
                )
            except HTTPException:
                ...
                await session.rollback()
                logger.error(
                    f"数据库操作失败: {str(db_error)}, 错误类型: {type(db_error).__name__}",
                    exc_info=True,
                )
                # 检查是否是表不存在错误
...
async def update_strategy_config(config_id: str, request: StrategyConfigUpdate):
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            return StandardResponse(
                success=True, message="更新配置成功", data=config.to_dict()
            )
    except HTTPException:
        raise
    except Exception as e:
