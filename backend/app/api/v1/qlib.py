...
async def create_qlib_model_config(request: QlibModelConfigRequest):
    ...
    try:
        ...
        config = await provider.create_qlib_model_config(
            model_type=request.model_type, hyperparameters=request.hyperparameters
        )
...
async def clear_cache():
    ...
    try:
        ...
        return StandardResponse(success=True, message="缓存清空成功", data={})
...
async def get_model_hyperparameters(model_name: str):
    ...
    try:
        ...
        return StandardResponse(
            success=True,
            message="模型超参数规格获取成功",
            data={"model_name": model_name, "hyperparameters": specs_data},
        )
                ...
                recommended_models.append(
                    {
                        ...
                        "suitability_score": len(recommendations)
                        - recommendations.index(model_name),  # 简单的适合度评分
                    }
                )
...
async def get_training_tips(model_name: str):
    ...
    try:
        ...
        return StandardResponse(
            success=True,
            message="训练建议获取成功",
            data={"model_name": model_name, "recommendations": recommendations},
        )
