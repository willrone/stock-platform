...
async def get_average_resources(
    ...
    try:
        ...
        if avg_usage is None:
            return {"success": True, "data": None, "message": "暂无历史数据"}
...
async def update_resource_thresholds(
    ...
    try:
        ...
        return {"success": True, "message": "资源阈值已更新"}
    except Exception as e:
...
async def start_monitoring(interval: float = Query(30.0, description="监控间隔（秒）")):
    ...
    try:
        ...
        return {"success": True, "message": f"资源监控已启动，间隔 {interval} 秒"}
    except Exception as e:
...
async def create_deployment(
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"deployment_id": deployment_id, "message": "部署已创建"},
        }
    except Exception as e:
...
async def get_active_deployment():
    ...
    try:
        ...
        return {"success": True, "data": active_deployment}
    except Exception as e:
...
async def validate_model_compatibility(
    ...
    try:
        ...
        if target_model_path:
            target_metadata = compatibility_validator.extract_model_metadata(
                target_model_path
            )
...
async def extract_model_metadata(model_path: str):
    ...
    try:
        ...
        return {"success": True, "data": metadata.to_dict()}
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        ...
        if not can_deploy:
            ...
            if deployment_strategy in ["blue_green", "canary"]:
                deployment_recommendations.append(
                    f"推荐使用 {deployment_strategy} 策略进行安全部署"
                )
        else:
            ...
            tests.append(
                {
                    ...
                    "description": info.get("description", ""),
                    "has_baseline": name
                    in health_monitor.performance_tester.baseline_metrics,
                }
            )

        return {"success": True, "data": {"tests": tests, "count": len(tests)}}
    except Exception as e:
...
async def get_performance_test_history(
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"history": history, "count": len(history), "test_name": test_name},
        }
    except Exception as e:
...
async def validate_deployment_readiness(
    ...
    try:
        ...
        # 1. 兼容性验证
        compatibility_result = compatibility_validator.validate_compatibility(
            model_path
        )
        ...
        if run_health_checks:
            ...
            if health_result["overall_status"] != "healthy":
                ...
                validation_results["issues"].append(
                    f"健康检查状态: {health_result['overall_status']}"
                )
