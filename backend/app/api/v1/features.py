...
from app.services.explainability.technical_analyzer import technical_analyzer, TechnicalIndicator
...
@router.get("/indicators/available", response_model=StandardResponse, summary="获取可用技术指标")
        ...
        for stock_code in request.stock_codes:
            try:
                ...
                results[stock_code] = {
                    ...
                    "computed_features": list(feature_data.keys()) if feature_data else []
                }
        ...
        return StandardResponse(
            ...
            data={
                ...
                "summary": {
                    ...
                    "success_rate": len(successful_stocks) / len(request.stock_codes) * 100
                }
            }
        )
...
@router.get("/technical-indicators/{stock_code}", response_model=StandardResponse, summary="计算技术指标")
async def compute_technical_indicators(
    ...
    try:
        ...
        # 过滤指定的指标
        if indicator_list:
            available_indicators = [col for col in indicators_data.columns if any(ind in col for ind in indicator_list)]
        ...
        for column in indicators_data.columns:
            result_data[column] = {
                ...
                'latest_value': float(indicators_data[column].dropna().iloc[-1]) if not indicators_data[column].dropna().empty else None,
...
@router.get("/metadata/{feature_name}", response_model=StandardResponse, summary="获取特征元数据")
...
async def configure_technical_indicator(config: TechnicalIndicatorConfig):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, 
                detail=f"无效的技术指标类型: {config.indicator_type}"
            )
...
@router.delete("/features/{feature_name}", response_model=StandardResponse, summary="删除特征")
