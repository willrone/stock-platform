...
async def get_version(version_id: str):
    ...
    try:
        ...
        # 获取血缘信息
        lineages = data_versioning_integration.version_manager.get_lineage_chain(version_id)
...
@router.get("/versions/compare/{version_id1}/{version_id2}", response_model=StandardResponse, summary="比较版本")
async def compare_versions(version_id1: str, version_id2: str):
    """比较两个版本"""
    try:
        comparison = data_versioning_integration.get_version_comparison(version_id1, version_id2)
...
@router.get("/stocks/{stock_code}/versions", response_model=StandardResponse, summary="获取股票数据版本")
...
@router.get("/stocks/{stock_code}/lineage", response_model=StandardResponse, summary="获取股票特征血缘")
async def get_stock_feature_lineage(stock_code: str):
    """获取股票的特征血缘"""
    try:
        lineage_info = data_versioning_integration.get_feature_lineage_for_stock(stock_code)
...
@router.get("/lineage/features/{feature_id}", response_model=StandardResponse, summary="获取特征血缘")
async def get_feature_lineage(feature_id: str):
    """获取特征血缘"""
    try:
        lineage = data_versioning_integration.lineage_tracker.get_feature_lineage(feature_id)
...
@router.get("/lineage/models/{model_id}", response_model=StandardResponse, summary="获取模型血缘")
...
@router.post("/lineage/model-training", response_model=StandardResponse, summary="追踪模型训练")
...
async def list_snapshots(
    ...
    """获取快照列表"""
    try:
        snapshots = data_versioning_integration.version_manager.list_snapshots(created_by=created_by)
...
@router.delete("/versions/{version_id}", response_model=StandardResponse, summary="删除版本")
async def delete_version(version_id: str, remove_file: bool = Query(False, description="是否删除文件")):
    """删除版本"""
    try:
        success = data_versioning_integration.version_manager.delete_version(version_id, remove_file=remove_file)
