...
class BacktestWebSocketManager:
    ...
    def get_connection_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_backtests": len(
                backtest_progress_monitor.get_all_active_backtests()
            ),
            "timestamp": datetime.utcnow().isoformat(),
        }
    ...
    try:
        while True:
            ...
            try:
                ...
                await backtest_ws_manager.send_to_connection(
                    connection_id,
                    {
                        "type": "error",
                        "message": "无效的JSON格式",
...
async def get_backtest_progress(task_id: str, session: Session = Depends(get_db)):
    ...
    try:
        ...
        if not progress_data:
            return {"success": True, "message": "当前没有进度数据", "data": None}
