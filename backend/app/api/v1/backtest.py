    ...
    try:
        ...
        strategy_descriptions = {
            ...
            "rsi": {
                ...
                "parameters": {
                    ...
                    "trend_ma_period": {
                        "type": "int",
                        "default": 50,
                        "description": "趋势判断均线周期（用于判断趋势方向）",
                        "min": 20,
                        "max": 200,
                    },
                    "enable_trend_alignment": {
                        "type": "boolean",
                        "default": True,
                        "description": "启用趋势对齐（在上升趋势中只在回调时买入，在下降趋势中只在反弹时卖出）",
                    },
                    "enable_divergence": {
                        "type": "boolean",
                        "default": True,
                        "description": "启用背离检测（检测价格与RSI的背离作为反转信号）",
                    },
                    "enable_crossover": {
                        "type": "boolean",
                        "default": True,
                        "description": "启用RSI穿越信号（等待RSI从超买超卖区域穿越回来，而不是仅仅在超买超卖区域就交易）",
                    },
                    "uptrend_buy_threshold": {
                        "type": "int",
                        "default": 40,
                        "description": "上升趋势中的买入阈值（RSI回调到此值以上时买入）",
                        "min": 30,
                        "max": 60,
                    },
                    "downtrend_sell_threshold": {
                        "type": "int",
                        "default": 60,
                        "description": "下降趋势中的卖出阈值（RSI反弹到此值以下时卖出）",
                        "min": 40,
                        "max": 70,
                    },
                },
            ...
            "multi_factor": {
                ...
                "parameters": {
                    "factors": {
                        "type": "json",
                        "default": ["value", "momentum", "low_volatility"],
                        "description": "使用的因子列表",
                    },
                    "factor_weights": {
                        "type": "json",
        ...
        for strategy_key in all_strategies:
            if strategy_key in strategy_descriptions:
                result.append(
                    {"key": strategy_key, **strategy_descriptions[strategy_key]}
                )
            else:
                result.append(
                    {
...
async def run_backtest(request: BacktestRequest):
    ...
    try:
        ...
        result = {
            ...
            "period": {
                "start_date": backtest_report.get(
                    "start_date", request.start_date.isoformat()
                ),
                "end_date": backtest_report.get(
