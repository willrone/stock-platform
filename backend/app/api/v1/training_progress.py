...
async def get_training_tasks(
    ...
    try:
        ...
        if model_type:
            tasks = [task for task in tasks if task.metadata.get("model_type") == model_type]
...
@router.get("/tasks/{task_id}/progress", response_model=StandardResponse, summary="获取训练进度")
...
@router.get("/tasks/{task_id}/metrics", response_model=StandardResponse, summary="获取训练指标")
async def get_training_metrics(task_id: str):
    ...
    try:
        ...
        if hasattr(task, 'training_history') and task.training_history:
            ...
            # 提取损失和准确率历史
            loss_history = [epoch.get('loss', 0) for epoch in history if 'loss' in epoch]
            val_loss_history = [epoch.get('val_loss', 0) for epoch in history if 'val_loss' in epoch]
            acc_history = [epoch.get('accuracy', 0) for epoch in history if 'accuracy' in epoch]
            val_acc_history = [epoch.get('val_accuracy', 0) for epoch in history if 'val_accuracy' in epoch]
...
@router.post("/tasks/{task_id}/control", response_model=StandardResponse, summary="控制训练任务")
...
@router.get("/models/{model_id}/training-history", response_model=StandardResponse, summary="获取模型训练历史")
...
async def get_training_stats():
    ...
    try:
        ...
        for task in all_tasks:
            ...
            stats["model_type_distribution"][model_type] = stats["model_type_distribution"].get(model_type, 0) + 1
        ...
        if completed_tasks:
            durations = [(task.completed_at - task.created_at).total_seconds() for task in completed_tasks]
        ...
        while True:
            ...
            if task:
                ...
                if task.status in ["completed", "failed", "cancelled"]:
                    ...
                    await manager.send_personal_message(json.dumps(final_data), websocket)
                    break
            else:
                # 任务不存在
                error_data = {
