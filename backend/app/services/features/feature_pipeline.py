...
class FeaturePipeline:
    ...
    async def on_data_sync_complete(
        ...
        try:
            # 使相关缓存失效
            await self.feature_store.invalidate_cache(stock_codes=[stock_code])
    ...
    async def _register_builtin_features(self):
        ...
        logger.info(
            f"内置特征注册完成: {len(technical_indicators) + len(fundamental_features)} 个特征"
        )
    ...
    async def _calculate_stock_features(
        self, stock_code: str, stock_data: pd.DataFrame, feature_names: List[str]
    ) -> pd.DataFrame:
        """计算单只股票的特征"""
        # 首先计算所有技术指标
        indicators_df = await self.indicator_calculator.calculate_all_indicators(
            stock_data
        )

        # 合并原始数据和技术指标
        if not indicators_df.empty:
            combined_data = stock_data.merge(
    ...
    async def _calculate_obv(self, data: pd.DataFrame) -> pd.Series:
        ...
        volume_direction = (
            data["volume"]
            .where(price_change > 0, -data["volume"])
            .where(price_change != 0, 0)
        )
    ...
    async def _calculate_kdj_k(self, data: pd.DataFrame) -> pd.Series:
        ...
        for i in range(len(data)):
            if i == 0 or pd.isna(rsv.iloc[i]):
                ...
                prev_k = (
                    k_values.iloc[i - 1]
                    if not pd.isna(k_values.iloc[i - 1])
                    else rsv.iloc[i]
                )
    ...
    async def _calculate_kdj_d(self, data: pd.DataFrame) -> pd.Series:
        ...
        for i in range(len(data)):
            if i == 0 or pd.isna(k_values.iloc[i]):
                d_values.iloc[i] = (
                    k_values.iloc[i] if not pd.isna(k_values.iloc[i]) else None
                )
            else:
                prev_d = (
                    d_values.iloc[i - 1]
                    if not pd.isna(d_values.iloc[i - 1])
                    else k_values.iloc[i]
                )
