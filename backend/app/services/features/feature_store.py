...
class FeatureCacheEntry:
    ...
    def is_valid_for(self, stock_codes: List[str], date_range: Tuple[datetime, datetime]) -> bool:
        """检查缓存是否对指定参数有效"""
        # 检查股票代码是否匹配
...
class FeatureStore:
                    ...
                    try:
                        ...
                        if cache_file.exists():
                            ...
                            if available_features:
                                logger.info(f"命中特征缓存: {cache_key}, 特征数: {len(available_features)}")
    ...
    async def invalidate_cache(
        ...
        for cache_key, cache_entry in self._cache_index.items():
            ...
            # 检查股票代码
            if stock_codes and any(code in cache_entry.stock_codes for code in stock_codes):
                should_remove = True
            
            # 检查特征名称
            if feature_names and any(name in cache_entry.feature_names for name in feature_names):
                should_remove = True
    ...
    async def get_cache_stats(self) -> Dict[str, Any]:
        ...
        expired_entries = sum(1 for entry in self._cache_index.values() if entry.is_expired())
        total_size = 0
