...
class DataSyncEventManager:
    ...
    def register_listener(
        self,
        event_type: DataSyncEventType,
        callback: Callable[[DataSyncEvent], Any]
    ):
        """注册事件监听器"""
                ...
                if tasks:
                    ...
                    for i, result in enumerate(results):
                        if isinstance(result, Exception):
                            listener_name = listeners[i].__name__ if i < len(listeners) else "unknown"
...
def register_feature_pipeline_callbacks():
    ...
    event_manager.register_listener(DataSyncEventType.INCREMENTAL_UPDATE, on_incremental_update)
