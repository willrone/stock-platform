...
def _extract_training_history(model: Any, config: Any, training_history: list) -> None:
    ...
    try:
        ...
        for eval_name, eval_results in evals_result.items():
            ...
            logger.info(
                f"从模型获取到真实训练历史: {len(losses)} 轮 " f"(来源: {eval_name}, 指标: {loss_key})"
            )
            break
                    ...
                    try:
                        ...
                        for lgb_ds, ds_name in ds_l:
                            ...
                            if x_data is not None:
                                x_arr = (
                                    _np.array(x_data)
                                    if not isinstance(x_data, _np.ndarray)
                                    else x_data
                                )
                                nan_pct = (
                                    _np.isnan(x_arr).sum() / x_arr.size * 100
                                    if x_arr.size > 0
                                    else 0
                                )
                                nonzero_cols = (
                        ...
                        if x_data is not None:
                            ...
                            var_cols = (
                                sum(
                                    ...
                                    for c in range(x_arr.shape[1])
                                    if len(
                                        _np.unique(x_arr[~_np.isnan(x_arr[:, c]), c])
                                    )
                                    > 1
                                )
                                if x_arr.ndim == 2
