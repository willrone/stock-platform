...
class BaseCustomModel(Model):
    ...
    def _prepare_data(self, dataset: pd.DataFrame) -> Tuple["torch.Tensor", "torch.Tensor"]:
        """准备训练数据"""
        if dataset.empty:
...
if PYTORCH_AVAILABLE:
    ...
    class InformerNet(nn.Module):
        ...
        def __init__(self, input_dim: int, d_model: int, n_heads: int, 
                     e_layers: int, d_layers: int, factor: int):
            super().__init__()
    ...
    class TimesNetModel(nn.Module):
        ...
        def __init__(self, input_dim: int, seq_len: int, d_model: int, 
                     d_ff: int, num_kernels: int, top_k: int):
            super().__init__()
        ...
        def forward(self, x):
            ...
            # 合并卷积输出
            x = torch.stack(conv_outputs, dim=1).mean(dim=1)  # (batch_size, d_model, seq_len, d_model)
    ...
    class PatchTSTNet(nn.Module):
        ...
        def __init__(self, input_dim: int, seq_len: int, patch_len: int, 
                     stride: int, d_model: int, n_heads: int, num_layers: int):
            super().__init__()
        ...
        def forward(self, x):
            ...
            patches = torch.stack(patches, dim=1)  # (batch_size, num_patches, patch_len * input_dim)
