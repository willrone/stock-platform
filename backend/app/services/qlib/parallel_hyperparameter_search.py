...
class ParallelHyperparameterSearch:
    ...
    def parallel_grid_search(
        ...
        logger.info(
            f"开始并行网格搜索，试验次数: {len(trials_params)}, 工作进程数: {self.config.max_workers}"
        )
    ...
    def _execute_parallel_search(
        ...
        with ProcessPoolExecutor(max_workers=self.config.max_workers) as executor:
            ...
            futures = {
                executor.submit(
                    _train_and_evaluate_worker, train_fn, params, trial_id
                ): trial_id
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        failed_trials += 1
                        logger.warning(
                            f"试验 {trial_id} 失败: {result.get('error', 'Unknown error')}"
                        )
    ...
    def _generate_random_trials(
        self, param_space: Dict[str, HyperparameterSpace], n_trials: int
    ) -> List[Dict[str, Any]]:
        """生成随机超参数组合"""
        import random
...
def _train_and_evaluate_worker(
    ...
    try:
        ...
        # 提取得分
        score = metrics.get(
            "score", metrics.get("accuracy", metrics.get("f1_score", 0.0))
        )
