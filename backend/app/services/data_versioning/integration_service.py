...
from app.services.data_versioning.lineage_tracker import (
    DataLineageTracker, NodeType, TransformationType, data_lineage_tracker
)
...
class DataVersioningIntegrationService:
    ...
    def create_processed_data_version(
        ...
        version_name = f"processed_{stock_code}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def create_feature_version(
        ...
        version_name = f"feature_{feature_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def get_feature_lineage_for_stock(self, stock_code: str) -> Dict[str, Any]:
        ...
        lineage_info = {
            'stock_code': stock_code,
            'raw_data_versions': [v.to_dict() for v in versions if v.data_type == DataType.RAW],
            'processed_versions': [v.to_dict() for v in versions if v.data_type == DataType.PROCESSED],
        ...
        for feature_version in feature_versions:
            feature_lineage = self.lineage_tracker.get_feature_lineage(feature_version.version_id)
            if feature_lineage:
    ...
    def get_version_comparison(self, version_id1: str, version_id2: str) -> Dict[str, Any]:
        """比较两个版本"""
    ...
    def cleanup_old_versions(
        ...
        for version in self.version_manager.list_versions():
            if version.created_at < cutoff_date:
                # 如果保留标记版本，跳过有特殊标签的版本
                if keep_tagged_versions and any(tag in ["important", "baseline", "production"] for tag in version.tags):
                    continue
