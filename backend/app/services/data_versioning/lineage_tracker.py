...
class LineageAnalyzer:
    ...
    def get_lineage_path(
        ...
        try:
            paths = list(
                nx.all_simple_paths(self.nx_graph, source_node_id, target_node_id)
            )
            return paths
    ...
    def find_root_nodes(self) -> List[str]:
        """查找根节点（没有前驱的节点）"""
        return [
            node for node in self.nx_graph.nodes() if self.nx_graph.in_degree(node) == 0
        ]
    ...
    def get_graph_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "strongly_connected_components": len(
                list(nx.strongly_connected_components(self.nx_graph))
            ),
            "weakly_connected_components": len(
                list(nx.weakly_connected_components(self.nx_graph))
            ),
        }
...
class DataLineageTracker:
    ...
    def create_edge(
        ...
        edge_id = (
            f"edge_{source_node_id}_{target_node_id}_{int(datetime.now().timestamp())}"
        )
            ...
            if lineage_file.exists():
                ...
                for edge_data in data.get("edges", []):
                    edge = LineageEdge(
                        ...
                        transformation_config=edge_data.get(
