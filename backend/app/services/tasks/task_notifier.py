    ...
    async def _check_and_notify(self):
        ...
        try:
            ...
            for task in all_tasks:
                ...
                task_update_time = task.completed_at or task.started_at or task.created_at
                time_changed = not last_check or (task_update_time and task_update_time > last_check)
    ...
    async def _notify_backtest_update(self, task):
        ...
        try:
            ...
            if progress_data:
                ...
                await backtest_ws_manager.send_progress_update(task.task_id, progress_data)
            else:
                # 如果仍然没有，发送基本进度消息
                message = {
                ...
                await backtest_ws_manager.send_to_task_subscribers(task.task_id, message)
            
            logger.debug(f"已发送回测任务状态更新: {task.task_id}, 状态: {task.status}, 进度: {task.progress}%")
            ...
            if task.status == TaskStatus.RUNNING.value:
                ...
                if progress_data:
                    ...
                    if task.result and isinstance(task.result, dict):
                        ...
                        logger.info(f"从数据库读取进度数据: task_id={task.task_id}, progress_data={db_progress_data}, result={task.result}")
                    
                    # 同步详细数据
                    if db_progress_data:
                        progress_data.processed_trading_days = db_progress_data.get('processed_days', 0)
                        progress_data.total_trading_days = db_progress_data.get('total_days', 0)
                        progress_data.current_date = db_progress_data.get('current_date')
                        progress_data.total_signals_generated = db_progress_data.get('total_signals', 0)
                        progress_data.total_trades_executed = db_progress_data.get('total_trades', 0)
                        progress_data.current_portfolio_value = db_progress_data.get('portfolio_value', 0.0)
                        logger.info(f"同步进度数据完成: processed_days={progress_data.processed_trading_days}, total_days={progress_data.total_trading_days}, signals={progress_data.total_signals_generated}, trades={progress_data.total_trades_executed}, portfolio={progress_data.current_portfolio_value}")
                    else:
                        logger.warning(f"数据库中没有进度数据: task_id={task.task_id}, result={task.result}, result_type={type(task.result)}")
                    ...
                    if task.progress >= 30:  # 如果进度 >= 30%，说明前几个阶段已完成
                        ...
                        await backtest_progress_monitor.update_stage(
                            task.task_id, "initialization", 
                            progress=100, 
                            status="completed"
                        )
                        # 数据加载已完成
                        await backtest_progress_monitor.update_stage(
                            task.task_id, "data_loading", 
                            progress=100, 
                            status="completed"
                        )
                        # 策略设置已完成
                        await backtest_progress_monitor.update_stage(
                            task.task_id, "strategy_setup", 
                            progress=100, 
                            status="completed"
                        )
                        ...
                        if task.progress < 90:
                            ...
                            await backtest_progress_monitor.update_stage(
                                task.task_id, "backtest_execution", 
                            ...
                            if task.progress < 95:
                                await backtest_progress_monitor.update_stage(
                                    task.task_id, "metrics_calculation", 
