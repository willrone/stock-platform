...
class TaskCleanupService:
    ...
    async def start(self):
        ...
        logger.info(f"任务清理服务已启动，清理间隔: {self.cleanup_interval_minutes}分钟，任务超时: {self.task_timeout_minutes}分钟")
        ...
        try:
            ...
            if self.auto_cleanup:
                ...
                for task in stuck_tasks:
                    logger.warning(f"发现卡住任务: {task['task_id']} ({task['task_name']}) - {task['status']} - {task['progress']}%")
    ...
    async def manual_cleanup(self, task_timeout_minutes: Optional[int] = None) -> dict:
        ...
        try:
            ...
            logger.info(f"手动清理完成: 处理 {result['total_stuck']} 个任务，修复 {len(result['fixed_tasks'])} 个")
            return result
