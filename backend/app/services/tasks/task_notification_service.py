...
class TaskNotificationService:
            ...
            if subscribers:
                ...
                for user_id in subscribers:
                    await self.websocket_manager.notify_user(
                        user_id, "task_progress", message.data
                    )
        ...
        try:
            ...
            if target_users:
                ...
                for user_id in target_users:
                    await self.websocket_manager.notify_user(
                        user_id, "system_notification", notification_data
                    )
            else:
                # 广播给所有用户
    ...
    async def _send_task_notification(
        ...
        try:
            ...
            if subscribers:
                for user_id in subscribers:
                    await self.websocket_manager.notify_user(
                        user_id, "task_notification", message_data
                    )

            # 总是发送给任务创建者
            await self.websocket_manager.notify_user(
                notification.user_id, "task_notification", message_data
            )
    ...
    async def _send_task_completion_notification(
        ...
        try:
            completion_type = (
                "task_completed"
                if notification.status == TaskStatus.COMPLETED.value
                else "task_failed"
            )
