...
class TaskExecutor:
    ...
    def execute_task(self, queued_task: QueuedTask, 
        ...
        if queued_task.estimated_duration:
            context.estimated_end_time = context.start_time + timedelta(seconds=queued_task.estimated_duration)
        ...
        future = self.thread_pool.submit(
            ...
            handler, queued_task, context
        )
        
        logger.info(f"任务开始执行: {queued_task.task_id}, 执行器: {self.executor_id}")
        return future
    
    def _execute_task_with_context(self, handler: Callable, queued_task: QueuedTask, 
    ...
    def get_running_tasks(self) -> List[Dict[str, Any]]:
        ...
        for task_id, context in self.running_tasks.items():
            task_info = {
                ...
                "estimated_end_time": context.estimated_end_time.isoformat() if context.estimated_end_time else None,
                "running_duration": (datetime.utcnow() - context.start_time).total_seconds()
            }
...
class TaskScheduler:
    ...
    def enqueue_task(self, task_id: str, task_type: TaskType, config: Dict[str, Any],
                    user_id: str, priority: TaskPriority = TaskPriority.NORMAL,
        ...
        try:
            ...
            logger.info(f"任务加入队列: {task_id}, 优先级: {priority.name}, 队列大小: {self.stats['queue_size']}")
            return True
            ...
            try:
                ...
                try:
                    future = available_executor.execute_task(
                        queued_task, 
                        progress_callback=self._create_progress_callback(queued_task.task_id)
                    )
    ...
    def get_queue_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "available_executors": len([e for e in self.executors if e.can_accept_task()]),
...
class TaskQueueManager:
    ...
    def get_scheduler(self, name: str = None) -> TaskScheduler:
        ...
        if not scheduler:
            raise TaskError(
                message=f"调度器不存在: {scheduler_name}",
                severity=ErrorSeverity.MEDIUM
            )
