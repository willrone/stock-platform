...
class ProgressTracker:
    ...
    def update_step(self, step_name: str, details: Optional[Dict[str, Any]] = None):
        ...
        logger.info(
            f"任务进度: {self.task_id}, 步骤: {step_name}, 进度: {progress_percentage:.1f}%"
        )
        return progress
        ...
        with set_log_context(task_id=task_id, user_id=queued_task.user_id):
            try:
                ...
                progress_tracker.update_step(
                    "初始化预测任务", {"stock_count": len(stock_codes)}
                )
                ...
                for i, stock_code in enumerate(stock_codes):
                    ...
                    try:
                        ...
                        failed_stocks.append(
                            {"stock_code": stock_code, "error": str(e)}
                        )
                        continue
    ...
    def execute(
        ...
        with set_log_context(task_id=task_id, user_id=queued_task.user_id):
            try:
                ...
                equity_curve = [
                    snapshot["portfolio_value"] for snapshot in portfolio_history
                ]
                ...
                task_result = {
                    ...
                    "portfolio": {
                        ...
                        "final_value": backtest_report.get("final_value", initial_cash),
                        "total_return": backtest_report.get("total_return", 0),
                        "annualized_return": backtest_report.get(
                            "annualized_return", 0
                        ),
                    },
                    "risk_metrics": {
    ...
    def validate_task_config(self, task_type: TaskType, config: Dict[str, Any]) -> bool:
        ...
        try:
            if task_type == TaskType.PREDICTION:
                ...
                if (
                    not isinstance(config["stock_codes"], list)
                    or len(config["stock_codes"]) == 0
                ):
    ...
    def execute(self, queued_task: QueuedTask, context: TaskExecutionContext):
        ...
        with set_log_context(task_id=task_id, user_id=queued_task.user_id):
            try:
                ...
                self.task_repository.update_task_status(
                    task_id, TaskStatus.RUNNING, result=result
                )
    ...
    def execute(
        ...
        with set_log_context(task_id=task_id, user_id=queued_task.user_id):
            try:
                ...
                logger.info(
                    f"解析超参优化任务配置: 策略={strategy_name}, 股票数={len(stock_codes)}, "
                ...
                try:
                    from app.services.backtest.optimization import (
                ...
                if result.get("success"):
                    self.task_repository.update_task_status(
                        task_id, TaskStatus.COMPLETED, result=result, progress=100.0
                    )
                else:
                    self.task_repository.update_task_status(
                        task_id,
