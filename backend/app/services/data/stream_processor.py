...
import pandas as pd
import numpy as np
from typing import AsyncIterator, Iterator, List, Dict, Any, Optional, Callable, Union
...
class ChunkedDataReader:
        ...
        try:
            ...
            for batch_start in range(0, total_rows, self.chunk_size):
                ...
                if memory_status['status'] == 'critical':
                    logger.warning(f"内存使用过高: {memory_status['current_mb']:.1f}MB，强制垃圾回收")
    ...
    async def read_csv_chunks(self, file_path: Path, **kwargs) -> AsyncIterator[pd.DataFrame]:
        """异步读取CSV文件分块"""
        try:
...
class StreamProcessor:
        ...
        try:
            ...
            async for chunk_df in chunk_iterator:
                ...
                try:
                    ...
                    # 更新内存峰值
                    memory_status = self.memory_monitor.check_memory()
                    stats.memory_peak_mb = max(stats.memory_peak_mb, memory_status['current_mb'])
                    
                    # 如果内存压力过大，提前写入部分结果
                    if (memory_status['status'] == 'warning' and 
                        len(processed_chunks) > 10):
            ...
            logger.info(f"流式处理完成: 处理 {stats.processed_records}/{stats.total_records} 条记录")
    ...
    async def _flush_chunks(self, chunks: List[pd.DataFrame], output_path: Optional[Path]):
        """刷新数据块到文件"""
