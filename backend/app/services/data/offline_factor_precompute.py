...
class OfflineFactorPrecomputeService:
    ...
    def get_stock_date_range(self, stock_code: str) -> Optional[Tuple[datetime, datetime]]:
        """
        获取股票的日期范围
    ...
    async def precompute_single_stock(
        ...
        try:
            # 1. 加载基础数据
            stock_data = self.data_loader.load_stock_data(stock_code, start_date, end_date)
            if stock_data.empty:
                logger.warning(f"股票 {stock_code} 无数据")
                return None
            
            # 2. 转换为Qlib格式（基础OHLCV数据）
            qlib_base = self.format_converter.convert_parquet_to_qlib(stock_data, stock_code)
            ...
            if self.alpha_calculator:
                alpha_factors = await self._compute_alpha158_factors_with_handler(qlib_base, stock_code)
                if not alpha_factors.empty:
                    qlib_base = self.format_converter.add_indicators_to_qlib(qlib_base, alpha_factors, stock_code)
            
            # 5. 计算技术指标（使用Pandas）
            technical_indicators = await self._compute_technical_indicators(stock_data, stock_code)
            if not technical_indicators.empty:
                qlib_base = self.format_converter.add_indicators_to_qlib(qlib_base, technical_indicators, stock_code)
            
            # 6. 计算基本面特征（使用Pandas）
            fundamental_features = await self._compute_fundamental_features(stock_data, stock_code)
            if not fundamental_features.empty:
                qlib_base = self.format_converter.add_indicators_to_qlib(qlib_base, fundamental_features, stock_code)
    ...
    async def _compute_qlib_base_indicators(
        ...
        try:
            ...
            return self.format_converter.add_indicators_to_qlib(qlib_data, indicators, stock_code)
    ...
    async def _compute_alpha158_factors_with_handler(
        ...
        try:
            ...
            # 获取日期范围
            dates = stock_data.index.get_level_values(1) if isinstance(stock_data.index, pd.MultiIndex) else stock_data.index
    ...
    async def _compute_technical_indicators(
        ...
        try:
            ...
            # 移动平均线（包括策略常用的周期）
            indicators['MA5'] = [v for v in self.indicator_calculator.calculate_moving_average(stock_data_list, 5)]
            indicators['MA10'] = [v for v in self.indicator_calculator.calculate_moving_average(stock_data_list, 10)]
            indicators['MA20'] = [v for v in self.indicator_calculator.calculate_moving_average(stock_data_list, 20)]
            indicators['MA30'] = [v for v in self.indicator_calculator.calculate_moving_average(stock_data_list, 30)]
            indicators['MA50'] = [v for v in self.indicator_calculator.calculate_moving_average(stock_data_list, 50)]  # RSI策略的trend_ma需要
            indicators['MA60'] = [v for v in self.indicator_calculator.calculate_moving_average(stock_data_list, 60)]
            ...
            # 布林带
            bollinger_result = self.indicator_calculator.calculate_bollinger_bands(stock_data_list)
            indicators['BOLLINGER_UPPER'] = [v for v in bollinger_result.get('upper', [])]
            indicators['BOLLINGER_MIDDLE'] = [v for v in bollinger_result.get('middle', [])]
            indicators['BOLLINGER_LOWER'] = [v for v in bollinger_result.get('lower', [])]
    ...
    async def _compute_fundamental_features(
        ...
        """
        try:
            features = self.feature_calculator.calculate_fundamental_features(stock_data.copy())
    ...
    async def precompute_all_stocks(
        ...
        try:
            ...
            for batch_start in range(0, total_stocks, self.batch_size):
                ...
                logger.info(f"处理批次 {batch_start // self.batch_size + 1}: 股票 {batch_start + 1}-{batch_end}/{total_stocks}")
                    ...
                    try:
                        if isinstance(result, Exception):
                            ...
                            if incremental and output_file.exists():
                                # 加载现有数据并合并
                                existing_data = self.format_converter.load_qlib_data(output_file, stock_code)
                                if not existing_data.empty:
                                    result = self.incremental_updater.merge_incremental_data(
                                        existing_data, result, stock_code
                                    )
                                    logger.debug(f"股票 {stock_code} 增量更新完成")
                            
                            # 保存到文件
                            self.format_converter.save_qlib_data(result, output_file, stock_code)
                ...
                if self.progress_callback:
                    self.progress_callback(progress, f"已处理 {batch_end}/{total_stocks} 只股票")
            ...
            try:
                ...
                if all_precomputed_data:
                    # 从合并数据获取日期范围
                    dates = combined_data.index.get_level_values(1) if isinstance(combined_data.index, pd.MultiIndex) else combined_data.index
                    date_range = {
                ...
                indicators = {
                    'technical': list(IndicatorRegistry.get_indicators_by_category(IndicatorCategory.TECHNICAL).keys()),
                    'alpha': list(IndicatorRegistry.get_indicators_by_category(IndicatorCategory.ALPHA).keys()),
                    'fundamental': list(IndicatorRegistry.get_indicators_by_category(IndicatorCategory.FUNDAMENTAL).keys()),
                    'base': list(IndicatorRegistry.get_indicators_by_category(IndicatorCategory.BASE).keys())
                }
