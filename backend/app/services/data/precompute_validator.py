...
class PrecomputeValidator:
    ...
    def validate_precomputed_data(
        ...
        try:
            ...
            # 检查MultiIndex结构
            if not isinstance(qlib_data.index, pd.MultiIndex) or qlib_data.index.nlevels != 2:
                return ValidationResult(
                    is_valid=False,
                    missing_stocks=[],
                    missing_dates=[],
            ...
            if expected_indicators:
                ...
                missing_indicators = [ind for ind in expected_indicators if ind not in actual_indicators]
            ...
            summary = '验证通过' if is_valid else '; '.join(summary_parts) if summary_parts else '未知问题'
    ...
    def _check_data_quality(self, qlib_data: pd.DataFrame) -> List[str]:
        ...
        try:
            ...
            high_missing_cols = missing_counts[missing_counts > len(qlib_data) * 0.1]  # 缺失超过10%
            ...
            if 'RSI14' in qlib_data.columns:
                invalid_rsi = ((qlib_data['RSI14'] < 0) | (qlib_data['RSI14'] > 100)).sum()
                if invalid_rsi > 0:
