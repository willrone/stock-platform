...
class ParquetManager:
    ...
    def save_stock_data(
        self, stock_data: List[StockData], merge_with_existing: bool = True
    ) -> bool:
        """
        保存股票数据到Parquet文件
    ...
    def load_stock_data(
        ...
        try:
            # 获取需要读取的文件列表
            file_paths = self._get_file_paths_for_date_range(
                    ...
                    try:
                        ...
                        try:
                            ...
                            if (
                                ...
                                and integrity_status != filters.integrity_status
                            ):
                                continue
    ...
    def get_comprehensive_stats(self) -> ComprehensiveStats:
        ...
        try:
            ...
            storage_efficiency = (
                total_records / (total_size / 1024 / 1024) if total_size > 0 else 0
            )  # 记录数/MB
    ...
    def filter_files(self, criteria: FilterCriteria) -> List[DetailedFileInfo]:
        ...
        try:
            ...
            if criteria.stock_codes:
                stock_dirs = [
                    search_path / code
                    for code in criteria.stock_codes
                    if (search_path / code).exists()
                ]
            else:
            ...
            for stock_dir in stock_dirs:
                ...
                for file_path in stock_dir.rglob("*.parquet"):
                    try:
                        ...
                        # 应用大小过滤
                        if (
                            criteria.min_file_size
                            and file_size < criteria.min_file_size
                        ):
                            continue
                        if (
                            criteria.max_file_size
                            and file_size > criteria.max_file_size
                        ):
                            continue
                        ...
                        integrity_status = self._check_file_integrity(file_path, df)
                        if (
                            criteria.integrity_status
                            and integrity_status != criteria.integrity_status
                        ):
                            continue
