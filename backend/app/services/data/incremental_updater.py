...
class IncrementalUpdater:
    ...
    def __init__(self):
        ...
        # 数据路径
        self.parquet_data_path = Path(settings.DATA_ROOT_PATH) / "parquet" / "stock_data"
        ...
        try:
            ...
            if parquet_mtime > qlib_mtime:
                ...
                if parquet_date_range and qlib_date_range:
                    # 检查是否有新日期
                    if (parquet_date_range[1] > qlib_date_range[1] or 
                        parquet_date_range[0] < qlib_date_range[0]):
                        return {
                            'action': 'update',
                            'parquet_mtime': parquet_mtime,
                            'qlib_mtime': qlib_mtime,
                            'date_range': parquet_date_range,
                            'new_dates': self._get_new_dates(parquet_date_range, qlib_date_range)
                        }
                    else:
                        # 数据被修改但日期范围没变，需要重新计算
                        return {
    ...
    def _get_date_range(self, data: pd.DataFrame) -> Optional[Tuple[datetime, datetime]]:
        """获取数据的日期范围"""
        if data.empty:
            return None
    ...
    def _get_date_range_from_qlib(self, qlib_data: pd.DataFrame) -> Optional[Tuple[datetime, datetime]]:
        """从Qlib数据获取日期范围"""
        if qlib_data.empty:
            return None
    ...
    def get_stocks_to_update(
        ...
        return [code for code, info in changes.items() if info['action'] in ['new', 'update']]
    ...
    def merge_incremental_data(
        ...
        try:
            ...
            # 合并数据（新数据覆盖旧数据）
            # 先删除旧数据中与新数据重叠的部分
            new_dates = new_data.index.get_level_values(1) if isinstance(new_data.index, pd.MultiIndex) else new_data.index
            existing_dates = existing_data.index.get_level_values(1) if isinstance(existing_data.index, pd.MultiIndex) else existing_data.index
            ...
            logger.debug(f"合并数据完成: {stock_code}, 旧数据: {len(existing_data)}, 新数据: {len(new_data)}, 合并后: {len(merged_data)}")
