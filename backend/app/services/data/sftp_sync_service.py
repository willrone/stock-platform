...
class SFTPSyncService:
    ...
    def __init__(
        ...
        # 缓存远端文件列表，避免重复查询
        self._remote_files_cache: Optional[Dict[str, str]] = None  # {stock_code: actual_file_path}
    ...
    def _find_remote_file(self, stock_code: str, sftp: paramiko.SFTPClient) -> Optional[str]:
        """
        查找远端文件路径
            ...
            for basename in possible_basenames:
                for cached_code, cached_path in cache.items():
                    ...
                    if basename in cached_path or basename in cached_code:
                        logger.debug(f"[{stock_code}] 通过模糊匹配找到文件: {cached_path} (匹配: {basename})")
                        return cached_path
    ...
    def sync_stock_file(self, stock_code: str, sftp: paramiko.SFTPClient, max_retries: int = 3) -> Tuple[bool, int, str]:
        """
        同步单个股票文件（带重试机制）
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    logger.warning(f"[{stock_code}] 尝试 {attempt + 1}/{max_retries}: {error_msg}")
                ...
                # 下载文件
                if attempt > 0:
                    logger.info(f"[{stock_code}] 重试 {attempt + 1}/{max_retries}: 正在同步 ({remote_file} -> {local_file})")
                else:
            ...
            except (ConnectionError, paramiko.SSHException, paramiko.socket.error, OSError) as e:
                ...
                logger.error(f"[{stock_code}] 尝试 {attempt + 1}/{max_retries}: {error_msg}", exc_info=True)
                last_error = error_msg
                if attempt < max_retries - 1:
                    import time
                    time.sleep(1)
    ...
    def sync_all_stocks(self, stock_codes: Optional[List[str]] = None) -> SyncResult:
        ...
        try:
            ...
            # 过滤出远端实际存在的股票代码
            available_stock_codes = [code for code in stock_codes if code in remote_files_cache]
            missing_stock_codes = [code for code in stock_codes if code not in remote_files_cache]
            ...
            for i, stock_code in enumerate(stock_codes, 1):
                try:
                    ...
                    if success:
                        ...
                        logger.warning(f"[{i}/{total_files}] {stock_code}: 失败 - {error_msg}")
                    ...
                    if i % 10 == 0:
                        ...
                        logger.info(f"同步进度: {i}/{total_files} ({synced_files} 成功, {len(failed_files)} 失败, 成功率: {success_rate:.1f}%) | "
            ...
            if 'total_original_files' in locals() and total_original_files > 0:
                ...
                original_success_rate = (synced_files / total_original_files * 100) if total_original_files > 0 else 0
                # 计算相对于可用文件的成功率
                available_success_rate = (synced_files / len(sync_available_stock_codes) * 100) if sync_available_stock_codes else 0
                ...
                message = f"同步完成: {synced_files}/{total_files} 成功 (成功率: {success_rate:.1f}%)"
            ...
            if 'total_original_files' in locals() and total_original_files > 0:
                logger.info(f"原始股票列表: {total_original_files} 个, 远端可用文件: {len(sync_available_stock_codes)} 个")
            logger.info(f"实际同步文件数: {total_files}, 成功: {synced_files}, 失败: {len(failed_files)}")
