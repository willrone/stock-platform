...
from app.services.infrastructure import connection_pool_manager, PoolConfig
...
class StockDataService:
    ...
    async def fetch_remote_data(
        ...
        try:
            ...
            # 检查远端服务返回的success字段
            if not data:
                logger.warning(f"远端服务返回空响应: {stock_code}")
                return None
            
            # 检查success字段
            if data.get("success") is False:
                error_msg = data.get("error", "未知错误")
                logger.warning(f"远端服务返回错误: {stock_code}, {error_msg}")
                return None
