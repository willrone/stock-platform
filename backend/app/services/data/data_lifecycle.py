...
class DataLifecycleManager:
    ...
    def run_cleanup(self, dry_run: bool = False) -> CleanupResult:
        ...
        result = CleanupResult(
            deleted_files=0, deleted_size=0, deleted_tasks=0, deleted_logs=0, errors=[]
        )
        ...
        try:
            if dry_run:
                ...
                cutoff_date = datetime.now() - timedelta(
                    days=self.retention_policy.parquet_retention_days
                )
    ...
    def _cleanup_old_tasks(self, dry_run: bool) -> Dict:
        ...
        try:
            # 计算截止日期
            task_cutoff = datetime.now() - timedelta(
                days=self.retention_policy.task_retention_days
            )
            failed_task_cutoff = datetime.now() - timedelta(
                days=self.retention_policy.failed_task_retention_days
            )
            ...
            if dry_run:
                ...
                result["deleted_tasks"] = (
                    normal_tasks[0]["count"] if normal_tasks else 0
    ...
    def _cleanup_log_files(self, dry_run: bool) -> Dict:
        ...
        try:
            cutoff_date = datetime.now() - timedelta(
                days=self.retention_policy.log_retention_days
            )
    ...
    def _cleanup_temp_files(self, dry_run: bool) -> Dict:
        ...
        try:
            cutoff_date = datetime.now() - timedelta(
                days=self.retention_policy.temp_file_retention_days
            )
    ...
    def _cleanup_model_files(self, dry_run: bool) -> Dict:
        ...
        try:
            cutoff_date = datetime.now() - timedelta(
                days=self.retention_policy.model_retention_days
            )
    ...
    def get_storage_usage(self) -> Dict:
        ...
        try:
            ...
            # 计算总计
            usage["total"]["size"] = sum(
                cat["size"]
                for cat in usage.values()
                if isinstance(cat, dict) and "size" in cat
            )
            usage["total"]["files"] = sum(
                cat["files"]
                for cat in usage.values()
        ...
        try:
            ...
            with self.db_manager.get_connection() as conn:
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?)
                """,
                    (config_key, str(config_value), "数据清理调度配置", datetime.now()),
                )
                conn.commit()
