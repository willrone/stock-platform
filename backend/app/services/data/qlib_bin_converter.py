...
class QlibBinConverter:
    ...
    def convert_parquet_to_bin(
        ...
        try:
            ...
            # 确保Qlib初始化
            try:
                qlib.init(region=REG_CN, provider_uri=str(qlib_data_path), auto_mount=False)
            except Exception:
                # 已初始化或重复初始化失败时继续
                pass
            ...
            if missing:
                ...
                return None
            data = data[[col_map[c] for c in ['open', 'high', 'low', 'close', 'volume']]]
            ...
            if calendar_file.exists():
                raw_lines = [l.strip() for l in calendar_file.read_text().splitlines() if l.strip()]
            ...
            if len(valid_dates) > 0:
                ...
                if inst_file.exists():
                    lines = [l for l in inst_file.read_text(encoding="utf-8").splitlines() if l.strip()]
                    updated = False
                    out_lines = []
                    for line in lines:
                        code = line.split()[0]
