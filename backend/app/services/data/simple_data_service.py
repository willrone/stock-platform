...
class SimpleDataService:
    ...
    async def _try_connect(self, base_url: str, path: str) -> Tuple[bool, Optional[httpx.Response], str]:
    ...
    async def check_remote_service_status(self) -> DataServiceStatus:
        ...
        try:
            ...
            if success and response:
                logger.info(f"数据服务健康检查成功，使用URL: {working_url}，响应时间: {response_time:.2f}ms")
                return DataServiceStatus(
            ...
            response_time = (datetime.now() - start_time).total_seconds() * 1000
            logger.error(f"远端数据服务健康检查异常: {e}，URL: {working_url}，响应时间: {response_time:.2f}ms", exc_info=True)
            return DataServiceStatus(
