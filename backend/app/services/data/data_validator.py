...
class DataValidator:
    ...
    def validate_stock_data(self, df: pd.DataFrame, stock_code: str) -> ValidationResult:
        """验证股票数据"""
        ...
        quality_score = self._calculate_quality_score(original_count, records_removed, issues)
    ...
    def _apply_validation_rule(self, df: pd.DataFrame, rule: ValidationRule, stock_code: str) -> Dict[str, Any]:
        """应用单个验证规则"""
        issues = []
        modified_count = 0
        
        if rule == ValidationRule.POSITIVE_PRICES:
            df, rule_issues, rule_modified = self._validate_positive_prices(df, stock_code)
        elif rule == ValidationRule.PRICE_RELATIONSHIPS:
            df, rule_issues, rule_modified = self._validate_price_relationships(df, stock_code)
            ...
            df, rule_issues, rule_modified = self._validate_date_continuity(df, stock_code)
        elif rule == ValidationRule.PRICE_VOLATILITY:
            df, rule_issues, rule_modified = self._validate_price_volatility(df, stock_code)
            ...
            df, rule_issues, rule_modified = self._validate_missing_values(df, stock_code)
    ...
    def _validate_positive_prices(self, df: pd.DataFrame, stock_code: str) -> Tuple[pd.DataFrame, List[Dict], int]:
        """验证价格为正数"""
        issues = []
        modified_count = 0
    ...
    def _validate_price_relationships(self, df: pd.DataFrame, stock_code: str) -> Tuple[pd.DataFrame, List[Dict], int]:
        """验证价格关系的合理性"""
        issues = []
        modified_count = 0
    ...
    def _validate_volume(self, df: pd.DataFrame, stock_code: str) -> Tuple[pd.DataFrame, List[Dict], int]:
        """验证成交量"""
        issues = []
        modified_count = 0
    ...
    def _validate_date_continuity(self, df: pd.DataFrame, stock_code: str) -> Tuple[pd.DataFrame, List[Dict], int]:
        """验证日期连续性"""
        issues = []
        modified_count = 0
        ...
        if 'date' in df.columns and len(df) > 1:
            ...
            if large_gaps.any():
                issues.append({
                    ...
                    'details': {
                        'max_gap_days': date_diff.max().days if date_diff.max() else 0
                    }
                })
        
        return df, issues, modified_count
    
    def _validate_price_volatility(self, df: pd.DataFrame, stock_code: str) -> Tuple[pd.DataFrame, List[Dict], int]:
        """验证价格波动合理性"""
        issues = []
        modified_count = 0
    ...
    def _validate_duplicates(self, df: pd.DataFrame, stock_code: str) -> Tuple[pd.DataFrame, List[Dict], int]:
        """验证重复记录"""
        issues = []
        modified_count = 0
    ...
    def _validate_missing_values(self, df: pd.DataFrame, stock_code: str) -> Tuple[pd.DataFrame, List[Dict], int]:
        """验证缺失值"""
        issues = []
        modified_count = 0
        ...
        for col in required_columns:
            if col in df.columns:
                ...
                if missing_count > 0:
                    severity = 'error' if missing_ratio > self.thresholds['missing_data_threshold'] else 'warning'
    ...
    def _detect_outliers(self, df: pd.DataFrame, stock_code: str) -> Tuple[pd.DataFrame, List[Dict], int]:
        """检测异常值"""
        issues = []
        modified_count = 0
    ...
    def _calculate_quality_score(self, original_count: int, removed_count: int, issues: List[Dict]) -> float:
        """计算数据质量评分"""
        if original_count == 0:
            return 0.0
        ...
        warning_penalty = sum(1 for issue in issues if issue['severity'] == 'warning') * 0.05
    ...
    def get_validation_summary(self, result: ValidationResult) -> Dict[str, Any]:
        ...
        return {
            ...
            'records_processed': len(result.cleaned_data) if result.cleaned_data is not None else 0,
            'records_removed': result.records_removed,
            'records_modified': result.records_modified,
            'issues_by_severity': {
                'error': len([i for i in result.issues_found if i['severity'] == 'error']),
                'warning': len([i for i in result.issues_found if i['severity'] == 'warning']),
                'info': len([i for i in result.issues_found if i['severity'] == 'info'])
            },
