...
class MetricsCollector:
    ...
    async def collect_metrics(self, environment: DeploymentEnvironment) -> Dict[str, float]:
        """收集所有指标"""
        metrics = {}
...
class DeploymentManager:
    ...
    def _register_default_checks(self):
        ...
        self.metrics_collector.register_collector('response_time_ms', response_time_collector)
    ...
    def _generate_deployment_id(self, config: DeploymentConfig) -> str:
        ...
        model_hash = hashlib.md5(f"{config.model_id}_{config.version}".encode()).hexdigest()[:8]
    ...
    async def _deploy_to_environment(self, environment: DeploymentEnvironment, config: DeploymentConfig):
        """部署到指定环境"""
        try:
            # 1. 复制模型文件
    ...
    async def _run_health_checks(self, environment: DeploymentEnvironment, config: DeploymentConfig) -> Dict[str, Any]:
        """运行健康检查"""
        ...
        while datetime.now() - start_time < timeout:
            try:
                ...
                environment.health_status = "healthy" if results.get('overall_healthy') else "unhealthy"
    ...
    async def _monitor_canary(self, canary_env: DeploymentEnvironment, config: DeploymentConfig):
        """监控金丝雀环境"""
        ...
        if final_success_rate < config.canary_success_threshold:
            raise RuntimeError(f"金丝雀成功率不达标: {final_success_rate} < {config.canary_success_threshold}")
        
        logger.info(f"金丝雀监控完成，成功率: {final_success_rate}")
    
    def _should_rollback(self, metrics: Dict[str, float], config: DeploymentConfig) -> bool:
        """检查是否应该回滚"""
        error_rate = metrics.get('error_rate', 0.0)
        response_time = metrics.get('response_time_ms', 0.0)
        
        if error_rate > config.rollback_threshold_error_rate:
            logger.warning(f"错误率超过阈值: {error_rate} > {config.rollback_threshold_error_rate}")
            return True
        
        if response_time > config.rollback_threshold_latency_ms:
            logger.warning(f"响应时间超过阈值: {response_time} > {config.rollback_threshold_latency_ms}")
            return True
    ...
    async def rollback(self, deployment_id: str, target_version: Optional[str] = None) -> str:
        """
        手动回滚部署
        ...
        if target_version:
            ...
            for d in self.deployments.values():
                if d.config.version == target_version and d.status == DeploymentStatus.DEPLOYED:
                    target_deployment = d
                    break
