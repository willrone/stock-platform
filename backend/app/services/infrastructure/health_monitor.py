...
class PerformanceTester:
    ...
    def _evaluate_performance(
        self, metrics: PerformanceMetrics, baseline: Optional[PerformanceMetrics]
    ) -> Tuple[bool, str]:
        """评估性能测试结果"""
        if not baseline:
            # 没有基准，只检查基本指标
        ...
        if metrics.response_time_ms > baseline.response_time_ms * 1.5:
            issues.append(
                f"响应时间退化: {metrics.response_time_ms:.0f}ms vs {baseline.response_time_ms:.0f}ms"
            )

        # 吞吐量不应低于基准的80%
        if metrics.throughput_rps < baseline.throughput_rps * 0.8:
            issues.append(
                f"吞吐量下降: {metrics.throughput_rps:.1f} vs {baseline.throughput_rps:.1f} RPS"
            )

        # 错误率不应超过基准的2倍
        if metrics.error_rate > baseline.error_rate * 2:
            issues.append(
                f"错误率上升: {metrics.error_rate:.2%} vs {baseline.error_rate:.2%}"
            )
        ...
        try:
            ...
            for current_users in range(1, max_users + 1, max(1, max_users // 10)):
                ...
                if test_result.metrics.error_rate > 0.1:
                    logger.warning(
                        f"负载测试提前停止 - 错误率过高: {test_result.metrics.error_rate:.2%}"
                    )
                    break
            ...
            message = (
                "负载测试通过"
                if passed
                else f"负载测试失败 - 错误率: {error_rate:.2%}, 响应时间: {avg_response_time:.0f}ms"
            )
    ...
    def get_test_history(
        self, test_name: Optional[str] = None, limit: int = 100
...
class HealthMonitor:
    ...
    def _register_default_checks(self):
        ...
        self.health_checker.register_check(
            "model_load", model_load_check, "检查模型是否可以正常加载"
        )
    ...
    def _register_default_tests(self):
            ...
            while time.time() - start_time < duration_seconds:
                try:
                    ...
                    for _ in range(concurrent_users):
                        tasks.append(
                            self._simulate_prediction_request(model_path, test_data)
                        )
        ...
        self.performance_tester.register_test(
            "model_prediction", model_prediction_test, "模型预测性能测试"
        )

    async def _simulate_prediction_request(
        self, model_path: str, test_data: np.ndarray
    ) -> float:
        """模拟预测请求"""
        start_time = time.time()
