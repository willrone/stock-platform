...
class ResourceMonitor:
    ...
    def __init__(
        self, thresholds: Optional[ResourceThresholds] = None, history_size: int = 1000
    ):
        """
        初始化资源监控器
    ...
    def get_average_usage(self, duration_minutes: int = 60) -> Optional[ResourceUsage]:
        """获取指定时间段内的平均资源使用情况"""
        ...
        if recent_usage[0].gpu_usage:
            ...
            for gpu_id in range(gpu_count):
                gpu_loads = [
                    u.gpu_usage[gpu_id]["load"]
                    for u in recent_usage
                    if u.gpu_usage and len(u.gpu_usage) > gpu_id
                ]
                gpu_memory_percents = [
                    u.gpu_usage[gpu_id]["memory_percent"]
                    for u in recent_usage
                    if u.gpu_usage and len(u.gpu_usage) > gpu_id
                ]
                ...
                if gpu_loads and gpu_memory_percents:
                    ...
                    avg_gpu_info["memory_percent"] = sum(gpu_memory_percents) / len(
                        gpu_memory_percents
                    )
    ...
    def is_resource_available(
        ...
        # 检查内存
        available_memory_gb = (
            current_usage.memory_total_gb - current_usage.memory_used_gb
        )
        ...
        if required_gpu_memory_gb > 0 and current_usage.gpu_usage:
            for gpu in current_usage.gpu_usage:
                ...
                available_gpu_memory = max(
                    available_gpu_memory, gpu_available_memory / 1024
                )  # 转换为GB
