...
class DataMonitoringService:
    ...
    async def check_service_health(self, service_name: str) -> ServiceHealthStatus:
        ...
        try:
            if service_name == "data_service":
                ...
                test_data = [
                    StockData(stock_code="TEST", date=datetime.now(), open=1.0, high=2.0, low=1.0, close=1.5, volume=1000),
                    StockData(stock_code="TEST", date=datetime.now(), open=1.5, high=2.5, low=1.5, close=2.0, volume=1000),
                    StockData(stock_code="TEST", date=datetime.now(), open=2.0, high=3.0, low=2.0, close=2.5, volume=1000)
                ]
    ...
    def record_request(self, service_name: str, response_time_ms: float, success: bool = True):
        """
        记录请求性能数据
    ...
    def get_performance_metrics(self, service_name: str) -> Optional[PerformanceMetrics]:
        ...
        success_rate = (request_count - error_count) / request_count if request_count > 0 else 0.0
    ...
    def get_error_statistics(self, hours: int = 24) -> List[ErrorStatistics]:
        ...
        # 过滤时间范围内的错误
        recent_errors = [
            log for log in self._error_logs
    ...
    def get_system_overview(self) -> Dict[str, Any]:
        ...
        # 收集各服务状态
        for service_name in ['data_service', 'indicators_service', 'parquet_manager', 'sync_engine']:
            if service_name in self._health_cache:
    ...
    def check_data_quality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if date_range:
                end_date = datetime.fromisoformat(date_range['end']) if isinstance(date_range['end'], str) else date_range['end']
