...
class EarlyStopping:
    ...
    def __call__(
        self, current_value: float, epoch: int, model_weights: Optional[Any] = None
    ) -> bool:
        """
        检查是否应该早停
    ...
    def update(
        ...
        if self.is_better(current_value, self.state.best_value):
            ...
            if self.config.verbose:
                logger.info(
                    f"早停监控: 在第 {epoch} 轮发现更好的 {self.config.monitor}: {current_value:.6f}"
                )
        else:
            self.state.wait += 1
            ...
            if self.config.verbose and self.state.wait > 0:
                logger.info(
                    f"早停监控: {self.config.monitor} 没有改进，等待 {self.state.wait}/{self.config.patience}"
                )
        ...
        if self.state.wait >= self.config.patience:
            ...
            if self.config.verbose:
                logger.info(
                    f"早停触发: 在第 {epoch} 轮停止训练，最佳 {self.config.monitor}: {self.state.best_value:.6f} (第 {self.state.best_epoch} 轮)"
                )
...
class OverfittingDetector:
    ...
    def __init__(self, patience: int = 5, threshold: float = 0.1, min_epochs: int = 10):
        """
        初始化过拟合检测器
    ...
    def update(self, train_loss: float, val_loss: float, epoch: int) -> bool:
        ...
        if self.overfitting_count >= self.patience:
            logger.warning(
                f"检测到过拟合: 训练损失 {recent_train_loss:.6f}, 验证损失 {recent_val_loss:.6f}"
            )
            return True
    ...
    def update(self, current_value: float, epoch: int) -> bool:
        ...
        if current_value < self.best_value:  # 假设越小越好
            ...
            if improvement > self.improvement_threshold:
                ...
                new_patience = min(
                    int(self.current_patience * self.patience_factor), self.max_patience
                )
                if new_patience > self.current_patience:
                    logger.info(
                        f"检测到显著改进，增加容忍轮数: {self.current_patience} -> {new_patience}"
                    )
...
class EarlyStoppingManager:
    ...
    def update(
        self, metrics: Dict[str, float], epoch: int, model_weights: Optional[Any] = None
    ) -> Dict[str, bool]:
        """
        更新所有早停策略
        ...
        for name, strategy in self.strategies.items():
            if strategy.config.monitor in metrics:
                should_stop = strategy.update(
                    metrics[strategy.config.monitor], epoch, model_weights
                )
...
def create_default_early_stopping() -> EarlyStoppingManager:
    ...
    # 添加自适应策略
    manager.add_adaptive_strategy(
        initial_patience=10, patience_factor=1.5, max_patience=50
    )
