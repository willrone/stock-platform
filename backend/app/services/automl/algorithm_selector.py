...
class AlgorithmSelector:
    ...
    async def analyze_data_characteristics(
        ...
        try:
            ...
            if time_column and time_column in data.columns:
                ...
                # 季节性分析
                seasonality_score = await self._analyze_seasonality(data, target_column, time_column)
                ...
                # 趋势分析
                trend_score = await self._analyze_trend(data, target_column, time_column)
            ...
            # 特征相关性分析
            feature_correlations = await self._analyze_feature_correlations(data, target_column)
    ...
    async def _analyze_nonlinearity(self, data: pd.DataFrame, target_column: str) -> float:
        """分析非线性程度"""
        try:
            # 选择数值特征
    ...
    async def _analyze_feature_correlations(
        ...
        try:
            ...
            return {
                ...
                "avg_target_correlation": np.mean(list(target_correlations.values())) if target_correlations else 0.0,
                "max_target_correlation": max(target_correlations.values()) if target_correlations else 0.0
            }
    ...
    async def recommend_algorithms(
        ...
        """
        try:
            characteristics = [DataCharacteristic(c) for c in data_characteristics.get("characteristics", [])]
            
            algorithm_scores = {}
            
            for algorithm, profile in self.algorithm_profiles.items():
                score = await self._calculate_algorithm_score(
                    algorithm, profile, characteristics, data_characteristics, constraints
                )
            ...
            for algorithm, score_info in sorted_algorithms[:top_k]:
                recommendation = AlgorithmRecommendation(
                    ...
                    initial_params=self._get_initial_params(algorithm, data_characteristics),
    ...
    async def _calculate_algorithm_score(
        ...
        # 估算性能和资源需求
        expected_performance = self._estimate_performance(algorithm, data_characteristics)
        training_time_estimate = self._estimate_training_time(algorithm, data_characteristics)
        resource_requirements = self._estimate_resource_requirements(algorithm, data_characteristics)
    ...
    def _get_initial_params(self, algorithm: str, data_characteristics: Dict[str, Any]) -> Dict[str, Any]:
        """获取初始参数建议"""
    ...
    def _estimate_performance(self, algorithm: str, data_characteristics: Dict[str, Any]) -> float:
        """估算预期性能"""
        # 基于算法类型和数据特征的简单性能估算
        base_performance = {
            "lightgbm": 0.85,
    ...
    def _estimate_training_time(self, algorithm: str, data_characteristics: Dict[str, Any]) -> float:
        """估算训练时间（分钟）"""
        n_samples = data_characteristics.get("n_samples", 1000)
        n_features = data_characteristics.get("n_features", 10)
    ...
    def _estimate_resource_requirements(self, algorithm: str, data_characteristics: Dict[str, Any]) -> Dict[str, Any]:
        """估算资源需求"""
        n_samples = data_characteristics.get("n_samples", 1000)
        n_features = data_characteristics.get("n_features", 10)
        ...
        # 基础资源需求
        base_requirements = {
            "linear_regression": {"cpu_cores": 1, "memory_gb": 0.5, "gpu_required": False},
        ...
        requirements = base_requirements.get(algorithm, {"cpu_cores": 2, "memory_gb": 2.0, "gpu_required": False}).copy()
