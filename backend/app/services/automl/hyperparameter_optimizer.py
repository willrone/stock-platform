        ...
        **kwargs,
        ...
        try:
            ...
            return {
                ...
                "n_trials": len(study.trials),
                "optimization_history": [
                    {"trial": i, "params": trial.params, "score": trial.value}
                    for i, trial in enumerate(study.trials)
                ],
            }
    ...
    async def _genetic_optimization(
        ...
        **kwargs,
        ...
        for generation in range(n_generations):
            ...
            # 更新最佳结果
            current_best_idx = (
                np.argmax(scores) if direction == "maximize" else np.argmin(scores)
            )
        ...
        **kwargs,
        ...
        for param_name, param_config in param_space.items():
            if param_config["type"] == "categorical":
                ...
                if param_config["type"] == "int":
                    sklearn_param_space[param_name] = list(
                        range(low, high + 1, max(1, (high - low) // n_points))
                    )
                else:
                    sklearn_param_space[param_name] = np.linspace(
                        low, high, n_points
                    ).tolist()
        ...
        for i, params in enumerate(param_grid):
            try:
                ...
                optimization_history.append(
                    {"trial": i, "params": params, "score": score}
                )
        ...
        **kwargs,
        ...
        for trial in range(n_trials):
            ...
            try:
                ...
                if (
                    early_stopping_rounds
                    and no_improvement_count >= early_stopping_rounds
                ):
