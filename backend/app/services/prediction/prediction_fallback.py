...
class PredictionFallbackEngine:
    ...
    def execute_fallback_prediction(self, stock_code: str, historical_data: pd.DataFrame,
                                  strategy: FallbackStrategy, 
    ...
    def _simple_linear_prediction(self, data: pd.DataFrame, params: Dict[str, Any]) -> Dict[str, Any]:
        """简单线性回归预测"""
    ...
    def _moving_average_prediction(self, data: pd.DataFrame, params: Dict[str, Any]) -> Dict[str, Any]:
        """移动平均预测"""
    ...
    def _momentum_prediction(self, data: pd.DataFrame, params: Dict[str, Any]) -> Dict[str, Any]:
        """动量预测"""
    ...
    def _mean_reversion_prediction(self, data: pd.DataFrame, params: Dict[str, Any]) -> Dict[str, Any]:
        """均值回归预测"""
        window = params.get('window', 20)
    ...
    def _random_walk_prediction(self, data: pd.DataFrame, params: Dict[str, Any]) -> Dict[str, Any]:
        """随机游走预测"""
    ...
    def _historical_average_prediction(self, data: pd.DataFrame, params: Dict[str, Any]) -> Dict[str, Any]:
        """历史平均预测"""
        ...
        # 预测方向
        predicted_direction = 1 if avg_return > 0.001 else (-1 if avg_return < -0.001 else 0)
        
        # 置信度基于收益稳定性
        return_std = returns.tail(window).std()
        confidence_score = max(0.1, min(0.5, abs(avg_return) / return_std if return_std > 0 else 0.1))
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if attempt < self.config.max_retries:
                    ...
                    logger.warning(f"执行失败，{delay:.2f}秒后重试 (尝试 {attempt + 1}/{self.config.max_retries}): {e}")
                    time.sleep(delay)
                else:
...
class PredictionErrorHandler:
    ...
    def handle_prediction_error(self, error: Exception, stock_code: str, 
        ...
        try:
            ...
            logger.info(f"错误处理完成: {stock_code}, 错误类型: {error_type}, 降级策略: {fallback_strategy.value}")
            return result
            
        except Exception as fallback_error:
            # 降级策略也失败了，返回最基本的预测
            logger.error(f"降级策略失败: {fallback_error}")
            return self._emergency_fallback(stock_code, historical_data, error, fallback_error)
    
    def _classify_error(self, error: Exception) -> str:
        """分类错误类型"""
        error_message = str(error).lower()
        
        if "model" in error_message and ("load" in error_message or "file" in error_message):
            return "model_load_error"
        elif "data" in error_message and ("insufficient" in error_message or "not enough" in error_message):
    ...
    def _select_fallback_strategy(self, error_type: str, 
                                context: Optional[Dict[str, Any]] = None) -> FallbackStrategy:
        """选择降级策略"""
        # 基于错误类型选择策略
        strategy = self.error_strategy_mapping.get(error_type, FallbackStrategy.SIMPLE_LINEAR)
    ...
    def _emergency_fallback(self, stock_code: str, historical_data: pd.DataFrame,
                          original_error: Exception, fallback_error: Exception) -> Dict[str, Any]:
        """紧急降级策略"""
        try:
