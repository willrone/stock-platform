...
class RiskAssessment:
    ...
    @staticmethod
    def assess_prediction_risk(historical_prices: pd.Series, predicted_price: float,
        ...
        # 计算各项风险指标
        var = RiskAssessment.calculate_var(returns, confidence_level)
        expected_shortfall = RiskAssessment.calculate_expected_shortfall(returns, confidence_level)
...
class PredictionEngine:
    ...
    def predict_single_stock(self, stock_code: str, config: PredictionConfig,
                           end_date: Optional[datetime] = None) -> PredictionOutput:
        """预测单只股票"""
        try:
            if end_date is None:
            ...
            logger.info(f"预测完成: {stock_code}, 预测价格: {prediction_result['predicted_price']:.2f}")
            return prediction_output
            
        except Exception as e:
            self.prediction_stats["errors"] += 1
            error_context = ErrorContext(stock_code=stock_code, model_id=config.model_id)
    ...
    def predict_multiple_stocks(self, stock_codes: List[str], config: PredictionConfig,
                              end_date: Optional[datetime] = None) -> List[PredictionOutput]:
        """批量预测多只股票"""
        predictions = []
        failed_stocks = []
    ...
    def _load_stock_data(self, stock_code: str, end_date: datetime, 
    ...
    def _extract_features(self, stock_code: str, data: pd.DataFrame, 
                         config: PredictionConfig) -> pd.DataFrame:
        """提取特征"""
        try:
            # 构建特征配置
            feature_config = FeatureConfig(
                technical_indicators=[
                    ...
                    'rsi_14', 'macd', 'bb_20', 'atr_14'
                ],
                statistical_features=[
                    'returns', 'volatility', 'momentum', 'price_ratios'
                ],
            ...
            # 提取特征
            features = self.feature_extractor.extract_features(stock_code, data, feature_config)
            
            # 如果指定了特定特征，进行过滤
            if config.features:
                available_features = [f for f in config.features if f in features.columns]
    ...
    def _execute_prediction(self, stock_code: str, features: pd.DataFrame, 
        ...
        try:
            ...
            if 'volatility_20d' in features.columns:
                volatility = features['volatility_20d'].iloc[-1] if not pd.isna(features['volatility_20d'].iloc[-1]) else 0.02
