...
class TechnicalIndicatorCalculator:
    ...
    def calculate_moving_average(
        self, data: List[StockData], period: int
    ) -> List[Optional[float]]:
        """计算移动平均线"""
        if len(data) < period:
    ...
    def calculate_stochastic(
        self, data: List[StockData], k_period: int = 14, d_period: int = 3
    ...
    def calculate_cci(
        self, data: List[StockData], period: int = 20
    ) -> List[Optional[float]]:
        """计算商品通道指数(CCI)"""
        if len(data) < period:
            return [None] * len(data)
    ...
    def calculate_incremental_indicators(
        ...
        updated_results = self.calculate_indicators(
            self._cached_data[stock_code], indicators
        )
    ...
    def calculate_bollinger_bands(
        self, data: List[StockData], period: int = 20, std_dev: float = 2.0
    ) -> Dict[str, List[Optional[float]]]:
        """计算布林带"""
        if len(data) < period:
            return {
    ...
    def calculate_indicators(
        self, data: List[StockData], indicators: List[str]
    ) -> List[TechnicalIndicatorResult]:
        """计算指定的技术指标 - 增强版本"""
        ...
        if "SMA" in indicators:
            calculated_indicators["SMA"] = self.calculate_moving_average(
                data, 20
            )  # 默认20日
    ...
    def validate_indicator_parameters(
        ...
        if indicator == "MACD":
            ...
            return (
                all(isinstance(p, int) and p > 0 for p in [fast, slow, signal])
                and fast < slow
            )
        ...
        if indicator == "BOLLINGER":
            ...
            return (
                ...
                and period > 0
                and isinstance(std_dev, (int, float))
                and std_dev > 0
            )
