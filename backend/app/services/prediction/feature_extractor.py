...
from app.core.error_handler import DataError, ErrorContext, ErrorSeverity
...
class StatisticalFeatures:
    ...
    def calculate_returns(
        prices: pd.Series, periods: List[int]
    ) -> Dict[str, pd.Series]:
        """计算不同周期的收益率"""
        features = {}
    ...
    def calculate_volatility(
        prices: pd.Series, windows: List[int]
    ) -> Dict[str, pd.Series]:
        """计算不同窗口的波动率"""
        returns = prices.pct_change()
        features = {}
        for window in windows:
            features[f"volatility_{window}d"] = returns.rolling(
                window=window
            ).std() * np.sqrt(252)
        return features
    ...
    def calculate_volume_features(
        data: pd.DataFrame, windows: List[int]
    ) -> Dict[str, pd.Series]:
        """计算成交量特征"""
        features = {}
        if "volume" in data.columns:
            ...
            for window in windows:
                ...
                features[f"volume_ratio_{window}d"] = (
                    volume / volume.rolling(window=window).mean()
                )
        return features
...
class FeatureExtractor:
        ...
        try:
            ...
            if self.cache and config.cache_enabled:
                ...
                cache_key = self.cache.get_cache_key(
                    stock_code, start_date, end_date, config
                )
            ...
            if config.custom_features:
                custom_features = self._extract_custom_features(
                    data, config.custom_features
                )
    ...
    def _extract_technical_indicators(
        self, data: pd.DataFrame, config: FeatureConfig
    ) -> pd.DataFrame:
        """提取技术指标特征"""
        features = pd.DataFrame(index=data.index)
        ...
        for indicator in config.technical_indicators:
            try:
                if indicator.startswith("sma_"):
                    ...
                    features[indicator] = self.technical_indicators.calculate_sma(
                        close, window
                    )

                elif indicator.startswith("ema_"):
                    window = int(indicator.split("_")[1])
                    features[indicator] = self.technical_indicators.calculate_ema(
                        close, window
                    )

                elif indicator.startswith("rsi_"):
                    window = int(indicator.split("_")[1])
                    features[indicator] = self.technical_indicators.calculate_rsi(
                        close, window
                    )
                    ...
                    window = int(indicator.split("_")[1])
                    bb_features = self.technical_indicators.calculate_bollinger_bands(
                        close, window
                    )
                    ...
                    stoch_features = self.technical_indicators.calculate_stochastic(
                        high, low, close, k_period
                    )
                    ...
                    window = int(indicator.split("_")[1])
                    features[indicator] = self.technical_indicators.calculate_atr(
                        high, low, close, window
                    )

                elif indicator.startswith("adx_"):
                    window = int(indicator.split("_")[1])
                    features[indicator] = self.technical_indicators.calculate_adx(
                        high, low, close, window
                    )

                elif indicator.startswith("cci_"):
                    window = int(indicator.split("_")[1])
                    features[indicator] = self.technical_indicators.calculate_cci(
                        high, low, close, window
                    )
        ...
        for feature_type in config.statistical_features:
            try:
                if feature_type == "returns":
                    return_features = self.statistical_features.calculate_returns(
                        close, [1, 5, 10, 20]
                    )
                    ...
                    vol_features = self.statistical_features.calculate_volatility(
                        close, config.time_windows
                    )
                    for key, series in vol_features.items():
                        features[key] = series

                elif feature_type == "momentum":
                    momentum_features = self.statistical_features.calculate_momentum(
                        close, config.time_windows
                    )
                    ...
                    ratio_features = self.statistical_features.calculate_price_ratios(
                        data
                    )
                    ...
                    volume_features = (
                        self.statistical_features.calculate_volume_features(
                            data, config.time_windows
                        )
                    )
                    ...
                    moment_features = (
                        self.statistical_features.calculate_statistical_moments(
                            close, config.time_windows
                        )
                    )
    ...
    def _extract_custom_features(
        self, data: pd.DataFrame, custom_features: List[str]
    ) -> pd.DataFrame:
        """提取自定义特征"""
        features = pd.DataFrame(index=data.index)
    ...
    def get_feature_importance(
        ...
        try:
            ...
            importance_scores = mutual_info_regression(
                aligned_features.fillna(0), aligned_target.fillna(0)
            )

            importance = pd.Series(
                importance_scores, index=aligned_features.columns
