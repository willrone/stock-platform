...
class RiskMetricsCalculator:
    ...
    @staticmethod
    def calculate_var(
        returns: pd.Series, confidence_levels: List[float]
    ) -> Dict[float, float]:
        """计算不同置信水平的VaR"""
        var_results = {}
    ...
    def calculate_volatility_metrics(
        returns: pd.Series, time_horizons: List[int]
    ) -> Dict[str, float]:
        """计算波动率指标"""
        clean_returns = returns.dropna()
        if len(clean_returns) == 0:
...
class RiskAssessmentService:
    ...
    def assess_prediction_risk(
        ...
        try:
            ...
            risk_metrics = self._calculate_risk_metrics(
                returns, historical_data["close"], config
            )
    ...
    def _calculate_risk_rating(
        self, risk_metrics: Dict[str, float], returns: pd.Series
    ) -> str:
        """计算风险评级"""
        try:
            # 基于多个指标计算风险评分
    ...
    def calculate_portfolio_risk(
        ...
        individual_risks: Dict[str, RiskAssessmentResult],
    ) -> Dict[str, float]:
        """计算投资组合风险"""
        try:
