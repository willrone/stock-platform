...
class PerformanceMonitor:
    ...
    def get_model_performance(
        ...
        return {
            ...
            "average_accuracy": statistics.mean([m.success_rate for m in history])
            if history
            else 0,
            "average_latency": statistics.mean([m.avg_latency_ms for m in history])
            if history
            else 0,
            "error_rate": statistics.mean([m.error_rate for m in history])
            if history
            else 0,
            "metrics_history": [m.to_dict() for m in history]
