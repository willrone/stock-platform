...
class MetricsCalculator:
    ...
    def calculate_metrics(self, model_id: str, model_version: str) -> Optional[PerformanceMetrics]:
        """计算性能指标"""
        with self.lock:
        ...
        # 计算吞吐量指标
        time_span = (recent_records[-1].timestamp - recent_records[0].timestamp).total_seconds()
        ...
        # 计算预测分布统计
        predictions = [r.prediction for r in successful_requests if r.prediction is not None]
        prediction_stats = {}
        if predictions:
        ...
        # 计算置信度统计
        confidences = [r.confidence for r in successful_requests if r.confidence is not None]
        avg_confidence = statistics.mean(confidences) if confidences else None
        low_confidence_rate = len([c for c in confidences if c < 0.7]) / len(confidences) if confidences else 0.0
    ...
    def check_metrics(self, metrics: PerformanceMetrics):
        ...
        with self.lock:
            for rule_name, rule in self.rules.items():
                ...
                # 检查条件
                should_alert = self._check_condition(metric_value, rule.condition, rule.threshold)
...
class PerformanceMonitor:
        ...
        while self.running:
            try:
                ...
                for key in model_keys:
                    ...
                    metrics = self.metrics_calculator.calculate_metrics(model_id, model_version)
                    ...
                    if metrics:
                        ...
                        logger.debug(f"计算指标: {model_id}_{model_version}, RPS: {metrics.requests_per_second:.2f}")
    ...
    def get_current_metrics(self, model_id: str, model_version: str) -> Optional[PerformanceMetrics]:
        """获取当前指标"""
    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_models_monitored': len(set(f"{m.model_id}_{m.model_version}" for m in recent_metrics)),
