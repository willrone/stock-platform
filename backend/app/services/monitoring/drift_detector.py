...
class StatisticalDriftDetector:
    ...
    def _wasserstein_distance(self, ref_data: np.ndarray, cur_data: np.ndarray) -> Tuple[float, None]:
        """Wasserstein距离"""
        distance = wasserstein_distance(ref_data, cur_data)
        return float(distance), None
    
    def _psi_score(self, ref_data: np.ndarray, cur_data: np.ndarray, bins: int = 10) -> Tuple[float, None]:
    ...
    def _jensen_shannon_divergence(self, ref_data: np.ndarray, cur_data: np.ndarray, bins: int = 10) -> Tuple[float, None]:
        """Jensen-Shannon散度"""
        # 创建分箱
...
class MultivariateDriftDetector:
    ...
    def fit_reference(self, reference_data: np.ndarray):
        ...
        logger.info(f"多变量漂移检测器已拟合，解释方差比: {self.pca.explained_variance_ratio_.sum():.3f}")
    ...
    def detect_drift(self, current_data: np.ndarray) -> Tuple[float, DriftSeverity]:
        ...
        # 计算重构误差
        ref_error = np.mean(np.sum((self.reference_reconstructed - self.scaler.transform(
            self.scaler.inverse_transform(self.reference_reconstructed)
        ))**2, axis=1))
        
        cur_error = np.mean(np.sum((reconstructed_current - scaled_current)**2, axis=1))
...
class DriftDetector:
    ...
    def __init__(self, window_size: int = 1000, reference_window_days: int = 7):
        ...
        # 数据存储
        self.feature_data: Dict[str, deque] = defaultdict(lambda: deque(maxlen=window_size))
        self.prediction_data: Dict[str, deque] = defaultdict(lambda: deque(maxlen=window_size))
    ...
    def fit_reference_data(self, model_id: str, model_version: str):
        ...
        with self.lock:
            ...
            reference_samples = [
                sample for sample in self.feature_data[key]
        ...
        with self.lock:
            ...
            for i, feature_name in enumerate(feature_names):
                ...
                for method in methods:
                    try:
                        drift_score, p_value, severity = self.statistical_detector.detect_drift(
            ...
            if self.reference_fitted:
                try:
                    mv_drift_score, mv_severity = self.multivariate_detector.detect_drift(current_matrix)
                    ...
                    mv_result = DriftResult(
                        ...
                        details={
                            ...
                            'explained_variance': float(self.multivariate_detector.pca.explained_variance_ratio_.sum())
                        }
                    )
            ...
            # 计算总体漂移分数和严重程度
            if feature_results:
                drift_scores = [r.drift_score for r in feature_results if r.drift_score is not None]
                ...
                overall_severity = [k for k, v in severity_levels.items() if v == max_severity_level][0]
            else:
    ...
    def _generate_summary(self, results: List[DriftResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            'features_with_drift': len([r for r in results if r.severity != DriftSeverity.NONE])
        }
    
    def _generate_recommendations(self, results: List[DriftResult], overall_severity: DriftSeverity) -> List[str]:
        """生成建议"""
        recommendations = []
        ...
        # 针对特定特征的建议
        high_drift_features = [r.feature_name for r in results if r.severity in [DriftSeverity.HIGH, DriftSeverity.CRITICAL]]
    ...
    def get_latest_report(self, model_id: str, model_version: str) -> Optional[DriftReport]:
        """获取最新的漂移报告"""
