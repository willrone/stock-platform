...
class MetricCalculator:
    ...
    def _calculate_conversion_rate(self, events: List[MetricEvent]) -> Optional[float]:
        ...
        for user_id, user_event_list in user_events.items():
            has_numerator = any(
                e.event_name == self.definition.numerator_event for e in user_event_list
            )
            has_denominator = any(
                e.event_name == self.definition.denominator_event
    ...
    def _calculate_confidence_interval(
        self, events: List[MetricEvent], value: float, confidence_level: float = 0.95
        ...
        try:
            ...
            elif self.definition.metric_type in [
...
class BusinessMetricsCollector:
    ...
    def register_metric(self, metric_definition: MetricDefinition):
        ...
        with self.lock:
            ...
            self.metric_calculators[metric_definition.metric_id] = MetricCalculator(
                metric_definition
            )
    ...
    def calculate_all_metrics(
        ...
        for metric_id in self.metric_definitions:
            metric_value = self.calculate_metric(
                metric_id, experiment_id, variant_id, start_time, end_time
            )
            if metric_value:
        ...
        for metric_id, definition in self.metric_definitions.items():
            task = asyncio.create_task(
                self._metric_calculation_loop(
                    metric_id, definition.calculation_interval_seconds
                )
            )
    ...
    async def stop_real_time_calculation(self):
        ...
        if self.calculation_tasks:
            await asyncio.gather(
                *self.calculation_tasks.values(), return_exceptions=True
            )
