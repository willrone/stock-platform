...
class HashBasedSplitter:
    ...
    def assign_variant(
        self, user_id: str, experiment: ABExperiment
...
class StickySessionSplitter:
    ...
    def assign_variant(
        self, user_id: str, experiment: ABExperiment, session_id: Optional[str] = None
    ) -> Optional[TrafficVariant]:
        """基于会话的粘性分配"""
        with self.lock:
            # 检查是否已有分配
            if (
                ...
                and experiment.experiment_id in self.user_assignments[user_id]
            ):
...
class TrafficManager:
        ...
        with self.lock:
            ...
            if existing_assignment:
                variant = self._get_variant_by_id(
                    experiment, existing_assignment.variant_id
                )
                if variant:
            ...
            if experiment.split_method == TrafficSplitMethod.STICKY_SESSION:
                assigned_variant = splitter.assign_variant(
                    user_id, experiment, session_id
                )
            else:
            ...
            logger.debug(
                f"用户分配: {user_id} -> {experiment_id}:{assigned_variant.variant_id}"
            )
            return allocation
    ...
    def _get_variant_by_id(
        self, experiment: ABExperiment, variant_id: str
    ) -> Optional[TrafficVariant]:
        """根据ID获取变体"""
        for variant in experiment.variants:
            if variant.variant_id == variant_id:
    ...
    def _check_user_criteria(
        self, user_attributes: Dict[str, Any], experiment: ABExperiment
    ) -> bool:
        """检查用户是否满足实验条件"""
        # 检查包含条件
    ...
    def delete_experiment(self, experiment_id: str) -> bool:
        ...
        with self.lock:
            ...
            for user_id in list(self.user_assignments.keys()):
                self.user_assignments[user_id] = [
                    a
                    for a in self.user_assignments[user_id]
