...
from app.services.ab_testing.metrics_collector import (
    BusinessMetricsCollector,
    MetricType,
    MetricValue,
)
from app.services.ab_testing.traffic_manager import (
    ABExperiment,
    TrafficVariant,
    VariantType,
)
...
class StatisticalTestEngine:
    ...
    def t_test(
        ...
        power = self._calculate_power_t_test(
            len(control_data), len(treatment_data), cohens_d, self.alpha
        )

        is_significant = p_value < self.alpha
        interpretation = self._interpret_t_test_result(
            statistic, p_value, cohens_d, is_significant
        )
    ...
    def proportion_z_test(
        ...
        power = self._calculate_power_proportion_test(
            control_total, treatment_total, p1, p2, self.alpha
        )

        is_significant = p_value < self.alpha
        interpretation = self._interpret_proportion_test_result(
            z_statistic, p_value, cohens_h, is_significant
        )
    ...
    def chi_square_test(
        self, contingency_table: np.ndarray, confidence_level: float = 0.95
    ) -> StatisticalTestResult:
        """卡方检验"""
        ...
        is_significant = p_value < self.alpha
        interpretation = self._interpret_chi_square_result(
            chi2_stat, p_value, cramers_v, is_significant
        )
    ...
    def _interpret_t_test_result(
        self, statistic: float, p_value: float, effect_size: float, is_significant: bool
    ) -> str:
        """解释t检验结果"""
        if is_significant:
    ...
    def _interpret_mann_whitney_result(
        self, statistic: float, p_value: float, effect_size: float, is_significant: bool
    ) -> str:
        """解释Mann-Whitney检验结果"""
        if is_significant:
    ...
    def _interpret_chi_square_result(
        self, statistic: float, p_value: float, effect_size: float, is_significant: bool
    ) -> str:
        """解释卡方检验结果"""
        if is_significant:
...
class ExperimentAnalyzer:
    ...
    def analyze_experiment(
        ...
        if experiment.start_time and experiment.end_time:
            ...
            duration_days = (
                datetime.now() - experiment.start_time
            ).total_seconds() / 86400

        # 确定要分析的指标
        if metric_ids is None:
            metric_ids = [
                m.metric_id for m in self.metrics_collector.list_metric_definitions()
            ]
    ...
    def _perform_statistical_test(
        ...
        """执行统计检验"""
        # 根据指标类型选择合适的检验方法
        metric_def = self.metrics_collector.get_metric_definition(
            control_metric.metric_id
        )

        if not metric_def:
            # 默认使用t检验
            return self._mock_t_test_result(
                control_metric, treatment_metric, confidence_level
            )
        ...
        if metric_def.metric_type == MetricType.CONVERSION:
            ...
            treatment_successes = int(
                treatment_metric.value * treatment_metric.sample_size
            )

            return self.test_engine.proportion_z_test(
                control_successes,
                control_metric.sample_size,
                treatment_successes,
                treatment_metric.sample_size,
                confidence_level,
            )

        elif metric_def.metric_type in [
    ...
    def _mock_t_test_result(
        ...
        # 估算标准误差
        pooled_se = abs(mean_diff) / max(
            np.sqrt(control_metric.sample_size + treatment_metric.sample_size), 1
        )
        ...
        # 效应量
        pooled_std = pooled_se * np.sqrt(
            control_metric.sample_size + treatment_metric.sample_size
        )
        cohens_d = mean_diff / pooled_std if pooled_std > 0 else 0
    ...
    def _perform_power_analysis(
        self, experiment: ABExperiment, metric_analyses: Dict[str, Any]
    ) -> Dict[str, Any]:
        """执行统计功效分析"""
