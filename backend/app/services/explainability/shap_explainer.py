...
class ShapValues:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'base_value': float(self.base_value) if isinstance(self.base_value, (int, float, np.number)) else self.base_value.tolist(),
...
class ModelExplainer:
    ...
    def explain_batch(self, batch_data: np.ndarray, max_samples: int = 100) -> ShapValues:
        """
        批量解释
    ...
    def get_feature_importance(self, shap_values: ShapValues) -> Dict[str, float]:
        ...
        if self.feature_names:
            ...
            return {f"feature_{i}": importance for i, importance in enumerate(mean_abs_shap)}
...
class ShapExplainerManager:
    ...
    def generate_explanation_report(
        ...
        # 特征重要性排名
        feature_ranking = self.get_feature_importance_ranking(model_id, model_version, top_k=20)
