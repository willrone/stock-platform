...
class IndicatorAnalysisResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'indicator_importance': [imp.to_dict() for imp in self.indicator_importance],
...
class TechnicalIndicatorCalculator:
    ...
    @staticmethod
    def calculate_moving_average(prices: pd.Series, period: int = 20, ma_type: str = 'sma') -> pd.Series:
        """计算移动平均线"""
        if ma_type == 'sma':
...
class IndicatorImpactAnalyzer:
    ...
    def analyze_correlation(
        ...
        for column in indicators_data.columns:
            try:
                if method == 'pearson':
                    corr, p_value = stats.pearsonr(indicators_data[column].dropna(), 
                                                  target[indicators_data[column].dropna().index])
                elif method == 'spearman':
                    corr, p_value = stats.spearmanr(indicators_data[column].dropna(), 
                                                   target[indicators_data[column].dropna().index])
                else:
...
class TechnicalAnalyzer:
    ...
    def calculate_all_indicators(
        ...
        try:
            ...
            # 移动平均线
            indicators['sma_5'] = self.calculator.calculate_moving_average(close, 5, 'sma')
            indicators['sma_10'] = self.calculator.calculate_moving_average(close, 10, 'sma')
            indicators['sma_20'] = self.calculator.calculate_moving_average(close, 20, 'sma')
            indicators['sma_50'] = self.calculator.calculate_moving_average(close, 50, 'sma')
            indicators['ema_12'] = self.calculator.calculate_moving_average(close, 12, 'ema')
            indicators['ema_26'] = self.calculator.calculate_moving_average(close, 26, 'ema')
        ...
        try:
            ...
            for method in analysis_methods:
                if method == AnalysisMethod.CORRELATION:
                    results = self.impact_analyzer.analyze_correlation(indicators_data, target)
                elif method == AnalysisMethod.MUTUAL_INFO:
                    results = self.impact_analyzer.analyze_mutual_information(indicators_data, target)
                elif method == AnalysisMethod.FEATURE_IMPORTANCE:
                    results = self.impact_analyzer.analyze_feature_importance(indicators_data, target)
                elif method == AnalysisMethod.PERMUTATION:
                    results = self.impact_analyzer.analyze_permutation_importance(indicators_data, target)
                elif method == AnalysisMethod.STATISTICAL_TEST:
                    results = self.impact_analyzer.analyze_statistical_significance(indicators_data, target)
                else:
            ...
            # 聚合结果
            aggregated_importance = self._aggregate_importance_results(all_importance_results)
            
            # 生成摘要统计
            summary_stats = self._generate_summary_stats(aggregated_importance, indicators_data, target)
            ...
            with self.lock:
                ...
                if len(self.analysis_results) > self.max_results:
                    oldest_id = min(self.analysis_results.keys(),
    ...
    def _aggregate_importance_results(
        ...
        return sorted(aggregated_results, key=lambda x: x.importance_score, reverse=True)
    ...
    def _generate_recommendations(
        ...
        # 低重要性指标建议
        low_importance_indicators = [imp.indicator_name for imp in importance_results[-5:] if imp.importance_score < 0.1]
        if low_importance_indicators:
            recommendations.append(f"以下指标影响力较低，可考虑移除: {', '.join(low_importance_indicators)}")
        
        return recommendations
    
    def get_analysis_result(self, analysis_id: str) -> Optional[IndicatorAnalysisResult]:
        """获取分析结果"""
