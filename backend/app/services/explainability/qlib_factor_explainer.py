...
class QlibFactorMapper:
    ...
    @classmethod
    def parse_alpha158_factors(cls, factor_names: List[str]) -> Dict[str, FactorCategory]:
        """解析Alpha158因子名称"""
        factor_categories = {}
...
class FactorAttributionAnalyzer:
    ...
    def analyze_factor_attribution(
        ...
        try:
            ...
            if factor_weights is None:
                factor_weights = self._estimate_factor_weights(factor_values, model_predictions)
            ...
            # 计算贡献度百分比
            total_abs_contribution = sum(abs(c.contribution_score) for c in contributions)
            
            if total_abs_contribution > 0:
                for contribution in contributions:
                    contribution.contribution_percentage = (
                        abs(contribution.contribution_score) / total_abs_contribution * 100
                    )
    ...
    def _calculate_factor_significance(
        ...
        try:
            ...
            # 计算相关性的p值
            _, p_value = stats.pearsonr(aligned_data.iloc[:, 0], aligned_data.iloc[:, 1])
...
class FactorClusterAnalyzer:
    ...
    def analyze_factor_clusters(
        ...
        try:
            # 准备数据
            contribution_scores = np.array([fc.contribution_score for fc in factor_contributions])
            ...
            for cluster_id in range(self.n_clusters):
                ...
                cluster_factors = [factor_names[i] for i in range(len(factor_names)) if cluster_mask[i]]
                cluster_contributions = [contribution_scores[i] for i in range(len(contribution_scores)) if cluster_mask[i]]
...
class QlibFactorExplainer:
    ...
    def explain_prediction(
        ...
        try:
            ...
            # 聚合类别贡献度
            category_contributions = self.attribution_analyzer.aggregate_category_contributions(
                factor_contributions
            )
            
            # 计算总归因和未解释方差
            total_attribution = sum(fc.contribution_score for fc in factor_contributions)
            ...
            explanation = FactorExplanation(
                ...
                explanation_level=ExplanationLevel.STOCK if stock_code else ExplanationLevel.PORTFOLIO,
                base_return=base_return,
                ...
                metadata={
                    ...
                    'top_factor': factor_contributions[0].factor_name if factor_contributions else None,
                    'attribution_coverage': abs(total_attribution / (prediction - base_return)) if prediction != base_return else 0
                }
            )
    ...
    def explain_portfolio_prediction(
        ...
        try:
            ...
            for stock_code, weight in portfolio_weights.items():
                if stock_code in portfolio_factor_values and stock_code in portfolio_predictions:
                    # 获取个股的因子解释
                    ...
                    for fc in stock_explanation.factor_contributions:
                        ...
                        portfolio_factor_contributions[fc.factor_name]['contribution_score'] += (
                            fc.contribution_score * weight
                        )
                        portfolio_factor_contributions[fc.factor_name]['weighted_count'] += weight
                    
                    total_portfolio_prediction += portfolio_predictions[stock_code] * weight
            ...
            # 计算贡献度百分比
            total_abs_contribution = sum(abs(fc.contribution_score) for fc in factor_contributions)
            if total_abs_contribution > 0:
            ...
            # 排序
            factor_contributions.sort(key=lambda x: abs(x.contribution_score), reverse=True)
    ...
    def get_factor_importance_ranking(
        ...
        sorted_factors = sorted(
            [(name, score, factor_categories[name]) for name, score in avg_contributions.items()],
            key=lambda x: x[1],
            reverse=True
        )
    ...
    def get_category_importance_ranking(
        ...
        for category in category_contributions:
            avg_contributions[category] = category_contributions[category] / category_counts[category]
        
        # 排序
        sorted_categories = sorted(avg_contributions.items(), key=lambda x: x[1], reverse=True)
    ...
    def generate_factor_report(
        ...
        # 因子重要性排名
        factor_ranking = self.get_factor_importance_ranking(model_id, model_version, top_k=20)
        ...
        # 预测准确性分析（如果有实际收益率）
        accuracy_stats = {}
        predictions_with_actual = [e for e in explanations if e.actual_return is not None]
        ...
        if predictions_with_actual:
            ...
            mse = np.mean([(p - a) ** 2 for p, a in zip(predicted_returns, actual_returns)])
        ...
        for explanation in explanations:
            if explanation.predicted_return != explanation.base_return:
                coverage = abs(explanation.total_attribution / 
                             (explanation.predicted_return - explanation.base_return))
                attribution_coverages.append(coverage)
        
        avg_attribution_coverage = np.mean(attribution_coverages) if attribution_coverages else 0
    ...
    def analyze_factor_clusters(
        ...
        for explanation in explanations:
            ...
            factor_names.update(fc.factor_name for fc in explanation.factor_contributions)
        ...
        factor_values = pd.DataFrame(
            np.random.randn(100, len(factor_names)),
