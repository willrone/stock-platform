...
class ModelEvaluator:
    ...
    async def evaluate_model(
        ...
        for fold, (train_idx, test_idx) in enumerate(splits):
            ...
            # 计算收益率
            fold_returns = self.metrics_calculator.calculate_returns(predictions, prices_test)
        ...
        win_rate = np.sum(all_returns > 0) / len(all_returns) if len(all_returns) > 0 else 0
...
class ModelVersionManager:
    ...
    def get_best_model(self, model_id: str, metric: str = "sharpe_ratio") -> Optional[ModelVersion]:
        """
        获取最佳模型版本
