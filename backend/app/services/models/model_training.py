    ...
    async def _fallback_prepare_features(
        ...
        for stock_code in stock_codes:
            try:
                ...
                # 计算技术指标
                from ..prediction.technical_indicators import (
...
class ModelTrainingService:
    ...
    async def train_model(
        ...
        try:
            ...
            train_X, train_y, val_X, val_y = self._time_series_split(
                X, y, config.validation_split
            )
            ...
            logger.info(
                f"评估结果 - 准确率: {metrics.accuracy:.4f}, 夏普比率: {metrics.sharpe_ratio:.4f}"
            )
            logger.info(
                f"总收益: {metrics.total_return:.4f}, 最大回撤: {metrics.max_drawdown:.4f}"
            )
    ...
    def _prepare_training_data(
        ...
        for stock_code in features_df["stock_code"].unique():
            ...
            # 创建滑动窗口
            for i in range(
                len(stock_data) - config.sequence_length - config.prediction_horizon + 1
            ):
                # 输入序列
                X_seq = (
                    stock_data[feature_cols].iloc[i : i + config.sequence_length].values
                )

                # 目标值（未来收益率）
                current_price = stock_data[config.target_column].iloc[
                    i + config.sequence_length - 1
                ]
                future_price = stock_data[config.target_column].iloc[
