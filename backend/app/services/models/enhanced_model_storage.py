...
class EnhancedModelStorage:
        ...
        if db is None:
            async for session in get_db():
                return await self._create_model_with_lineage_impl(
                    session, model_info, training_config, data_sources,
                    feature_config, parent_models, tags
                )
        else:
            return await self._create_model_with_lineage_impl(
                db, model_info, training_config, data_sources,
                feature_config, parent_models, tags
            )
        ...
        if db is None:
            async for session in get_db():
                return await self._search_models_impl(
                    session, query, tags, model_types, status_list,
                    date_range, performance_threshold, limit, offset
                )
        else:
            return await self._search_models_impl(
                db, query, tags, model_types, status_list,
                date_range, performance_threshold, limit, offset
            )
    ...
    async def _search_models_impl(
        ...
        try:
            ...
            # 获取模型列表
            models_query = base_query.order_by(desc(ModelInfo.created_at)).limit(limit).offset(offset)
    ...
    async def _compare_models_impl(
        ...
        try:
            ...
            for metric in metrics:
                metric_values = []
                for model in models:
                    if model.performance_metrics and metric in model.performance_metrics:
    ...
    async def _get_model_recommendations_impl(
        ...
        try:
            # 计算当前配置的指纹
            data_fingerprint = self.lineage_tracker._calculate_data_fingerprint(data_sources)
            config_fingerprint = self.lineage_tracker._calculate_config_fingerprint(training_config)
            ...
            for model in similar_models[:limit]:
                if model.get("performance_metrics"):
                    recommendation = {
                        ...
                        "recommendation_reason": self._generate_recommendation_reason(model)
                    }
