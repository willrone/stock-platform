...
class ModelTrainingService:
    ...
    def train_model(self, model_name: str, model_type: ModelType, 
                   stock_codes: List[str], start_date: datetime, end_date: datetime,
                   config: TrainingConfig, created_by: str,
        ...
        try:
            ...
            X, y = self._prepare_training_data(stock_codes, start_date, end_date, config)
            ...
            cv_scores = self._cross_validate(model, X_train, y_train, config.cross_validation_folds)
            
            # 步骤7: 获取特征重要性
            if progress_callback:
                progress_callback(90, "分析特征重要性")
            
            feature_importance = trainer.get_feature_importance(model, X_train.columns.tolist())
            ...
            raise ModelError(
                ...
                context=ErrorContext(model_id=model_id if 'model_id' in locals() else None),
                original_exception=e
            )
    
    def _prepare_training_data(self, stock_codes: List[str], start_date: datetime,
                             end_date: datetime, config: TrainingConfig) -> Tuple[pd.DataFrame, pd.Series]:
        """准备训练数据"""
        # 创建模拟数据
        np.random.seed(42)
        n_samples = 1000
    ...
    def _cross_validate(self, model: Any, X: pd.DataFrame, y: pd.Series, cv: int) -> List[float]:
        """交叉验证"""
        try:
            ...
            for i in range(cv):
                ...
                # 训练和评估
                fold_model = TrainerFactory.create_trainer(ModelType.RANDOM_FOREST).create_model({})
    ...
    def _update_average_training_time(self, training_time: float):
        ...
        if total_successful == 1:
            ...
            new_avg = (current_avg * (total_successful - 1) + training_time) / total_successful
