...
class ModelEvaluator:
    ...
    def evaluate_model(
        ...
        try:
            ...
            evaluation_id = (
                f"eval_{model_id}_{evaluation_start.strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            # 业务指标评估
            business_metrics = self._evaluate_business_metrics(
                model, test_data, metadata
            )
            ...
            evaluation = ModelEvaluation(
                ...
                test_data_info={
                    ...
                    "date_range_days": (
                        test_data.index.max() - test_data.index.min()
                    ).days,
                },
            ...
            logger.info(
                f"模型评估完成: {model_id}, 评分: {overall_score:.3f}, 建议: {recommendation}"
            )
    ...
    def _prepare_test_data(
        self, metadata: ModelMetadata, evaluation_config: Optional[Dict[str, Any]]
    ) -> pd.DataFrame:
        """准备测试数据"""
        try:
            # 从配置获取测试参数
            ...
            if feature_columns:
                available_columns = [
                    col for col in feature_columns if col in test_data.columns
                ]
    ...
    def _evaluate_performance(
        self, model: Any, test_data: pd.DataFrame, metadata: ModelMetadata
    ) -> Dict[str, float]:
        """评估模型性能"""
        try:
            # 准备特征和目标
    ...
    def _calculate_overall_score(
        ...
        try:
            ...
            if "sharpe_ratio" in business_metrics:
                # 夏普比率标准化到0-1范围
                sharpe_score = max(
                    0, min(1, (business_metrics["sharpe_ratio"] + 1) / 3)
                )
...
class ModelDeploymentService:
    ...
    def deploy_model(
        ...
        try:
            ...
            deployment_id = (
                f"deploy_{model_id}_{deployment_start.strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            # 检查模型状态
            if (
                metadata.status not in [ModelStatus.TRAINED, ModelStatus.READY]
                and not force
            ):
                raise ModelError(
            ...
            try:
                ...
                self.model_storage.save_model(
                    *self.model_storage.load_model(model_id), overwrite=True
                )
    ...
    def _execute_deployment(self, deployment_record: DeploymentRecord):
        ...
        try:
            ...
            if deployment_record.config.deployment_type == "production":
                ...
                raise ValueError(
                    f"不支持的部署类型: {deployment_record.config.deployment_type}"
                )
    ...
    def rollback_deployment(
        self, deployment_id: str, user_id: str, reason: str = ""
    ) -> bool:
        """回滚部署"""
        try:
    ...
    def list_deployments(
        self, model_id: Optional[str] = None, status: Optional[DeploymentStatus] = None
