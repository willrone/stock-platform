...
class EnsembleModelManager:
    ...
    async def _create_weighted_ensemble(
        ...
        ensemble_model = {"type": "weighted", "models": base_models, "weights": weights}
    ...
    async def _predict_ensemble(
        self, ensemble_model: Dict[str, Any], X: np.ndarray
    ) -> np.ndarray:
        """集成模型预测"""
...
class OnlineLearningManager:
    ...
    async def update_model_online(
        ...
        if len(buffer["data"]) >= 32:  # 最少需要一个批次的数据
            ...
            metrics = await self._evaluate_online_model(
                updated_model, np.array(buffer["data"]), np.array(buffer["labels"])
            )
    ...
    async def _incremental_training(
        self, model: Any, X: np.ndarray, y: np.ndarray, model_id: str
    ) -> Any:
        """增量训练"""
        ...
        if isinstance(model, xgb.Booster):
            ...
            current_lr = config.learning_rate * (
                ...
                ** len(self.model_performance_history[model_id]["accuracies"])
            )
    ...
    async def _should_retrain(
        ...
        # 计算历史最佳准确率
        historical_best = (
            np.max(history["accuracies"][:-1])
            if len(history["accuracies"]) > 1
            else recent_avg
        )
