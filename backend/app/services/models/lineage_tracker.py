        ...
        if db is None:
            async for session in get_db():
                return await self._record_training_lineage_impl(
                    session, model_id, training_config, data_sources, 
                    feature_config, parent_models
                )
        else:
            return await self._record_training_lineage_impl(
                db, model_id, training_config, data_sources, 
                feature_config, parent_models
            )
    ...
    async def _record_training_lineage_impl(
        ...
        try:
            ...
            lineage_info = {
                ...
                "config_fingerprint": self._calculate_config_fingerprint(training_config),
