...
class ModelLifecycleManager:
    ...
    async def get_lifecycle_history(
        self, model_id: str, limit: int = 50, db: Optional[AsyncSession] = None
    ) -> List[Dict[str, Any]]:
        """
        获取模型生命周期历史
    ...
    async def _get_lifecycle_history_impl(
        self, db: AsyncSession, model_id: str, limit: int
    ) -> List[Dict[str, Any]]:
        """获取生命周期历史实现"""
        try:
            result = await db.execute(
    ...
    async def get_models_by_status(
        self, status: str, limit: int = 100, db: Optional[AsyncSession] = None
    ) -> List[Dict[str, Any]]:
        """
        根据状态获取模型列表
    ...
    async def auto_cleanup_old_models(
        self, days_threshold: int = 30, db: Optional[AsyncSession] = None
    ) -> int:
        """
        自动清理旧模型
    ...
    async def _auto_cleanup_old_models_impl(
        self, db: AsyncSession, days_threshold: int
    ) -> int:
        """自动清理旧模型实现"""
        try:
    ...
    async def _get_status_statistics_impl(self, db: AsyncSession) -> Dict[str, int]:
        """获取状态统计信息实现"""
        try:
            from sqlalchemy import func

            result = await db.execute(
                select(ModelInfo.status, func.count(ModelInfo.model_id)).group_by(
                    ModelInfo.status
                )
            )
