    ...
    async def generate_pdf_report(
        ...
        try:
            ...
            basic_info = [
                ...
                ["回测期间", f"{backtest_result.get('start_date', '')} 至 {backtest_result.get('end_date', '')}"],
            ...
            if not include_tables or "trade_summary" in include_tables:
                ...
                if trade_history:
                    ...
                    trade_table = Table(trade_data, colWidths=[1*inch, 0.8*inch, 0.8*inch, 1*inch, 1.2*inch, 1*inch])
                    ...
                    if len(trade_history) > 10:
                        story.append(Paragraph(f"注：仅显示前10条交易记录，总共{len(trade_history)}条", styles['Normal']))
                    story.append(Spacer(1, 20))
    ...
    async def generate_excel_report(
        ...
        try:
            ...
            with pd.ExcelWriter(filepath, engine='openpyxl') as writer:
                ...
                performance_data = {
                    '指标': [
                        '总收益率', '年化收益率', '波动率', '夏普比率', '最大回撤',
                        '胜率', '盈亏比', '总交易次数', '盈利交易', '亏损交易'
                    ],
                    '数值': [
                ...
                if portfolio_history:
                    ...
                    # 只重命名存在的列
                    existing_columns = {k: v for k, v in column_mapping.items() if k in portfolio_df.columns}
                    ...
                    if '组合价值' in portfolio_df.columns:
                        ...
                        portfolio_df['累积收益率'] = (portfolio_df['组合价值'] / initial_value - 1) * 100
