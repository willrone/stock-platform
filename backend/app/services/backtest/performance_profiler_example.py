...
async def example_parallel_efficiency_analysis():
    ...
    print(f"效率: {report['parallel_efficiency']['stock_processing']['efficiency_percent']:.1f}%")
...
async def example_memory_analysis():
    ...
    for snapshot in report['memory_snapshots']:
        print(f"  {snapshot['label']}: {snapshot['current_mb']:.2f}MB (峰值: {snapshot['peak_mb']:.2f}MB)")
