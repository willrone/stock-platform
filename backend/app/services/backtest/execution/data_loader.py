...
class DataLoader:
    ...
    def load_stock_data(self, stock_code: str, start_date: datetime, 
        ...
        try:
            # 1. 优先尝试从Qlib预计算目录加载
            precomputed_data = self._load_from_precomputed(stock_code, start_date, end_date)
            if precomputed_data is not None and not precomputed_data.empty:
                logger.info(f"从预计算结果加载: {stock_code}, 指标数: {len(precomputed_data.columns)}")
    ...
    def _load_from_parquet_and_calculate(
        ...
        logger.info(f"从Parquet加载股票数据: {stock_code}, 数据量: {len(data)}, 日期范围: {data.index[0]} - {data.index[-1]}")
        return data
    ...
    def load_multiple_stocks(self, stock_codes: List[str], start_date: datetime,
        ...
        if parallel and len(stock_codes) > 1 and self.max_workers:
            ...
            def load_single_stock(stock_code: str) -> Tuple[str, Optional[pd.DataFrame], Optional[str], bool]:
