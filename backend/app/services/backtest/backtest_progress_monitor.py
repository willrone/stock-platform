...
from app.services.infrastructure.websocket_manager import websocket_manager, WebSocketMessage
...
class BacktestProgressMonitor:
    ...
    async def update_execution_progress(self, task_id: str, processed_days: int,
        ...
        if progress_data.total_trading_days > 0:
            execution_progress = min(processed_days / progress_data.total_trading_days * 100, 100)
            await self.update_stage(task_id, "backtest_execution", 
                                  progress=execution_progress, status="running",
                                  details={
        ...
        # 每10天或每10%进度发送一次更新
        if processed_days % 10 == 0 or (processed_days % max(1, progress_data.total_trading_days // 10) == 0):
    ...
    async def add_warning(self, task_id: str, warning_message: str):
        ...
        # 发送警告通知
        await self._notify_progress_update(task_id, "warning_added", {
    ...
    async def set_error(self, task_id: str, error_message: str):
        ...
        # 发送错误通知
        await self._notify_progress_update(task_id, "backtest_failed", {
            "error": error_message
        })
    ...
    async def complete_backtest(self, task_id: str, final_results: Dict[str, Any] = None):
        ...
        # 发送完成通知
        await self._notify_progress_update(task_id, "backtest_completed", {
            "results": final_results or {}
        })
    ...
    async def cancel_backtest(self, task_id: str, reason: str = "用户取消"):
        ...
        # 发送取消通知
        await self._notify_progress_update(task_id, "backtest_cancelled", {
            "reason": reason
        })
    ...
    async def _notify_progress_update(self, task_id: str, update_type: str, 
