...
class SignalIntegrator:
    ...
    def integrate(self, 
                  ...
                  consistency_threshold: float = 0.6) -> List[TradingSignal]:
        """
        整合多个策略的信号
    ...
    def _integrate_stock_signals(self,
                                 stock_code: str,
                                 signals: List[TradingSignal],
                                 weights: Dict[str, float],
                                 consistency_threshold: float) -> Optional[TradingSignal]:
        """
        整合单个股票的所有信号
    ...
    def normalize_signal_strength(self, signals: List[TradingSignal]) -> List[TradingSignal]:
        """
        归一化信号强度到0-1范围
        ...
        normalized_signals = []
        for signal in signals:
            normalized_strength = (signal.strength - min_strength) / (max_strength - min_strength)
