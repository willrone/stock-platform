...
class ChartCacheService:
    ...
    async def get_cached_chart_data(
        self, task_id: str, chart_type: str
        ...
        async for session in get_async_session():
            try:
                ...
                async def _get_cache():
                    ...
                    if not cache_record:
                        self.logger.debug(
                            f"未找到缓存数据: task_id={task_id}, chart_type={chart_type}"
                        )
                        return None
                    ...
                    if cache_record.is_expired():
                        self.logger.info(
                            f"缓存已过期，删除记录: task_id={task_id}, chart_type={chart_type}"
                        )
            ...
            try:
                ...
                async def _cache_data():
                    ...
                    if existing_record:
                        ...
                        self.logger.info(
                            f"创建缓存: task_id={task_id}, chart_type={chart_type}"
                        )
            ...
            try:
                ...
                async def _invalidate():
                    if chart_type:
                        ...
                        self.logger.info(
                            f"删除特定缓存: task_id={task_id}, chart_type={chart_type}"
                        )
                    else:
                        # 删除任务的所有缓存
                        stmt = delete(BacktestChartCache).where(
    ...
    async def is_cache_valid(
        self, task_id: str, chart_type: str, data_hash: Optional[str] = None
    ) -> bool:
        """检查缓存是否有效"""
