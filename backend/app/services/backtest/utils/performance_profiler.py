...
class BacktestPerformanceProfiler:
    ...
    def end_stage(self, stage_name: str, details: Optional[Dict[str, Any]] = None) -> StageMetrics:
        """
        结束监控一个阶段
    ...
    def record_db_operation(self, operation: str, duration: float, details: Optional[Dict] = None):
        """
        记录数据库操作
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            'summary': {
                ...
                'signals_per_second': self.total_signals / total_time if total_time > 0 else 0,
                'trades_per_second': self.total_trades / total_time if total_time > 0 else 0,
                'days_per_second': self.total_trading_days / total_time if total_time > 0 else 0,
            },
            'stages': {
                name: {
    ...
    def print_summary(self):
        ...
        for stage_name, stage_data in report['stages'].items():
            print(f"  {stage_name}:")
            print(f"    耗时: {stage_data['duration']:.2f}秒 ({stage_data['percentage']:.1f}%)")
            print(f"    内存: {stage_data['memory_after_mb']:.2f}MB (峰值: {stage_data['memory_peak_mb']:.2f}MB)")
            print(f"    CPU: {stage_data['cpu_avg_percent']:.1f}%")
        
        if report['function_calls']:
            print("\n最耗时的函数调用 (Top 10):")
            for i, (func_name, func_data) in enumerate(list(report['function_calls'].items())[:10], 1):
                print(f"  {i}. {func_name}:")
...
class PerformanceContext:
    ...
    def __init__(self, profiler: BacktestPerformanceProfiler, stage_name: str, details: Optional[Dict] = None):
