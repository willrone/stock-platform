...
class StatisticalArbitrageStrategy(BaseStrategy):
    ...
    def validate_pair_correlation(self, data1: pd.DataFrame, data2: pd.DataFrame, 
...
class PairsTradingStrategy(StatisticalArbitrageStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        zscore = (close_prices - close_prices.rolling(window=self.lookback_period).mean()) / (
    ...
    def generate_signals(self, data: pd.DataFrame, current_date: datetime) -> List[TradingSignal]:
        ...
        try:
            ...
            if prev_zscore <= -self.zscore_threshold and current_zscore > -self.zscore_threshold:
            ...
            elif prev_zscore >= self.zscore_threshold and current_zscore < self.zscore_threshold:
                if relative_strength > 0.02:
                    strength = min(1.0, abs(current_zscore) / self.zscore_threshold)
                    signal = TradingSignal(
                        timestamp=current_date,
...
class CointegrationStrategy(StatisticalArbitrageStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        if half_life > 0:
            mean_reversion_strength = pd.Series(-np.log(2) / half_life, index=close_prices.index)
    ...
    def generate_signals(self, data: pd.DataFrame, current_date: datetime) -> List[TradingSignal]:
        ...
        try:
            ...
            if prev_zscore <= -self.entry_threshold and current_zscore > -self.entry_threshold:
                if mean_reversion < 0:
            ...
            elif prev_zscore >= self.entry_threshold and current_zscore < self.entry_threshold:
                if mean_reversion < 0:
                    strength = min(1.0, abs(current_zscore) / self.entry_threshold)
                    signal = TradingSignal(
                        timestamp=current_date,
...
class FactorStrategy(BaseStrategy):
    ...
    def apply_neutralization(self, scores: pd.Series, 
                            market_caps: Optional[pd.Series] = None,
                            industries: Optional[pd.Series] = None) -> pd.Series:
        """应用中性化处理"""
        ...
        if market_caps is not None and self.market_cap_neutral:
            ...
            if len(valid_idx) > 10:
                X = np.column_stack([np.ones(len(valid_idx)), log_mcap.loc[valid_idx].values])
        ...
        if industries is not None and self.industry_neutral:
            ...
            if len(valid_idx) > 10:
                ...
                for ind in unique_industries:
                    ...
                    if ind_mask.sum() > 5:
                        neutralized.loc[valid_idx[ind_mask]] -= neutralized.loc[valid_idx[ind_mask]].mean()
...
class ValueFactorStrategy(FactorStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        pe_estimate = pd.Series(1 / (returns.rolling(window=252).mean() + 0.001), index=close_prices.index)
        pe_estimate = pe_estimate.clip(-100, 100).fillna(15)
        
        pb_estimate = pd.Series(1 / (volatility + 0.01) * 0.5 + 1, index=close_prices.index)
        pb_estimate = pb_estimate.clip(0.1, 10).fillna(2)
        
        ps_estimate = pd.Series(1 / (volatility + 0.01) * 0.3 + 1.5, index=close_prices.index)
        ps_estimate = ps_estimate.clip(0.5, 15).fillna(3)
        
        ev_ebitda_estimate = pd.Series(1 / (volatility + 0.01) * 0.4 + 5, index=close_prices.index)
        ev_ebitda_estimate = ev_ebitda_estimate.clip(2, 30).fillna(10)
        
        value_score = pd.Series(0.0, index=close_prices.index)
        
        pe_normalized = (pe_estimate - pe_estimate.rolling(window=252).mean()) / (pe_estimate.rolling(window=252).std() + 0.01)
        pb_normalized = (pb_estimate - pb_estimate.rolling(window=252).mean()) / (pb_estimate.rolling(window=252).std() + 0.01)
        ps_normalized = (ps_estimate - ps_estimate.rolling(window=252).mean()) / (ps_estimate.rolling(window=252).std() + 0.01)
        ev_normalized = (ev_ebitda_estimate - ev_ebitda_estimate.rolling(window=252).mean()) / (ev_ebitda_estimate.rolling(window=252).std() + 0.01)
...
class MomentumFactorStrategy(FactorStrategy):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.lookback_period = max(config.get('lookback_period', 252), max(self.momentum_periods))
...
class LowVolatilityStrategy(FactorStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        historical_vol = daily_returns.rolling(window=self.volatility_window).std() * np.sqrt(252)
        
        risk_adjusted_return = daily_returns.rolling(window=self.volatility_period).mean() / (
            daily_returns.rolling(window=self.volatility_period).std() + 1e-8
        )
...
class MultiFactorStrategy(FactorStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        volatility_normalized = (volatility - volatility.rolling(window=252).mean()) / (volatility.rolling(window=252).std() + 0.001)
        ...
        momentum_normalized = (momentum_combined - momentum_combined.rolling(window=60).mean()) / (
        ...
        for i, factor_name in enumerate(self.factors):
            if factor_name in factor_scores:
                ...
                weight = self.factor_weights[i] if i < len(self.factor_weights) else 1.0 / len(self.factors)
            ...
            if prev_score <= 0 and current_score > 0:
                ...
                signal = TradingSignal(
                    ...
                    metadata={
                        ...
                        'value_score': float(indicators['value_score'].iloc[current_idx]) if not pd.isna(indicators['value_score'].iloc[current_idx]) else 0,
                        'momentum_score': float(indicators['momentum_score'].iloc[current_idx]) if not pd.isna(indicators['momentum_score'].iloc[current_idx]) else 0
                    }
                )
                signals.append(signal)
...
class AdvancedStrategyFactory:
    ...
    def get_available_strategies(cls) -> Dict[str, List[str]]:
        ...
        for name in cls._strategies.keys():
            ...
            elif name in ['value_factor', 'momentum_factor', 'low_volatility', 'multi_factor']:
