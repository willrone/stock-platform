...
class EnhancedMetricsCalculator:
    ...
    async def calculate_performance_metrics(
        ...
        try:
            ...
            # 基础收益指标
            basic_metrics = self._calculate_basic_metrics(portfolio_df, returns, initial_cash)
            
            # 风险调整收益指标
            risk_adjusted_metrics = self._calculate_risk_adjusted_metrics(returns, basic_metrics['annualized_return'])
            
            # 回撤指标
            drawdown_metrics = self._calculate_drawdown_metrics(portfolio_df['portfolio_value'])
    ...
    def _calculate_drawdown_metrics(self, portfolio_values: pd.Series) -> Dict[str, float]:
        ...
        # Calmar比率
        annualized_return = (portfolio_values.iloc[-1] / portfolio_values.iloc[0]) ** (252 / len(portfolio_values)) - 1
        calmar_ratio = (annualized_return - self.risk_free_rate) / abs(max_drawdown) if max_drawdown < 0 else 0
        ...
        return {
            ...
            'avg_drawdown_duration': float(np.mean(drawdown_durations)) if drawdown_durations else 0,
    ...
    def _calculate_rolling_metrics(self, returns: pd.Series, window: int = 252) -> Dict[str, Any]:
        ...
        return {
            'rolling_sharpe_mean': float(rolling_sharpe.mean()) if not rolling_sharpe.empty else 0,
            'rolling_sharpe_std': float(rolling_sharpe.std()) if not rolling_sharpe.empty else 0,
            'rolling_volatility_mean': float(rolling_volatility.mean()) if not rolling_volatility.empty else 0,
            'rolling_volatility_std': float(rolling_volatility.std()) if not rolling_volatility.empty else 0,
            'rolling_max_drawdown_mean': float(rolling_max_dd.mean()) if not rolling_max_dd.empty else 0
        }
    ...
    def calculate_sector_analysis(self, trade_history: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            for stock_code, perf in stock_performance.items():
                perf['win_rate'] = perf['win_count'] / perf['trade_count'] if perf['trade_count'] > 0 else 0
