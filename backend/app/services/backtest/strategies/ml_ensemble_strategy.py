...
class MLEnsembleLgbXgbRiskCtlStrategy(BaseStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        indicators["momentum_short"] = (
            indicators["return_5d"] - indicators["return_10d"]
        )
        indicators["momentum_long"] = (
            indicators["return_10d"] - indicators["return_20d"]
        )
        ...
        indicators["ma_alignment"] = (ma_5 > ma_10).astype(int) + (
            ma_10 > ma_20
        ).astype(int)
        ...
        indicators["vol_regime"] = indicators["volatility_5"] / (
            indicators["volatility_20"] + 1e-10
        )
        ...
        indicators["wick_upper"] = (high - np.maximum(close, open_)) / (
            high - low + 1e-10
        )
        indicators["wick_lower"] = (np.minimum(close, open_) - low) / (
            high - low + 1e-10
        )
        ...
        for window in [20, 60]:
            ...
            indicators[f"price_pos_{window}"] = (close - low_n) / (
                high_n - low_n + 1e-10
            )
    ...
    def generate_signals(
        ...
        # 计算仓位缩放
        last_return = (
            indicators.get("return_1d", pd.Series()).iloc[idx]
            if "return_1d" in indicators
            else 0
        )
        position_scale = self._calculate_position_scale(
            last_return if not pd.isna(last_return) else 0
        )
        ...
        if prob > self.prob_threshold and position_scale > 0:
            ...
            signals.append(
                TradingSignal(
                    ...
                    strength=(
                        min(1.0, max(0.1, (0.5 - prob) / 0.5))
                        if position_scale == 0
                        else min(1.0, max(0.1, (0.4 - prob) / 0.4))
                    ),
                    price=data["close"].iloc[idx],
                    timestamp=current_date,
                    reason=(
                        f"ML集成预测概率={prob:.3f}, 触发风控"
                        if position_scale == 0
                        else f"ML集成预测概率低={prob:.3f}"
                    ),
                )
            )
    ...
    def _calculate_ensemble_prob(
        ...
        try:
            ...
            # RSI 信号
            rsi = (
                indicators.get("rsi_14", pd.Series()).iloc[idx]
            ...
            # MACD 信号
            macd_hist = (
                indicators.get("macd_hist", pd.Series()).iloc[idx]
                if "macd_hist" in indicators
                else 0
            )
            macd_slope = (
                indicators.get("macd_hist_slope", pd.Series()).iloc[idx]
                if "macd_hist_slope" in indicators
                else 0
            )
            ...
            # 动量信号
            mom_short = (
                indicators.get("momentum_short", pd.Series()).iloc[idx]
            ...
            # 布林带位置
            bb_pos = (
                indicators.get("bb_position", pd.Series()).iloc[idx]
            ...
            # MA 排列
            ma_align = (
                indicators.get("ma_alignment", pd.Series()).iloc[idx]
    ...
    def precompute_all_signals(self, data: pd.DataFrame) -> Optional[pd.Series]:
        ...
        if data is None or len(data) < 60:
            logger.warning(
                f"ML策略预计算跳过: data is None={data is None}, len={len(data) if data is not None else 0}"
            )
            return None
        ...
        try:
            ...
            if missing_features:
                logger.debug(
                    f"缺失特征（已填0）: {missing_features[:5]}... 共{len(missing_features)}个"
                )
            ...
            avg_prob = score.mean()
            logger.info(
                f"ML策略预计算完成: BUY={buy_count}, SELL={sell_count}, None={none_count}, 平均概率={avg_prob:.4f}"
            )
        ...
        try:
            ...
            for _, row in features_df.iterrows():
                ...
                if p > self.prob_threshold:
                    ...
                    signal_type = SignalType.SELL
                    strength = ((1 - self.prob_threshold) - p) / (
                        1 - self.prob_threshold
                    )
                else:
                    continue  # 无信号
    ...
    def _calculate_cross_sectional_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "volatility_20" in result.columns:
            result["volatility_20_rank"] = result.groupby("date")["volatility_20"].rank(
                pct=True
            )
        ...
        if (
            "relative_strength_5d" in result.columns
            and "relative_strength_20d" in result.columns
        ):
