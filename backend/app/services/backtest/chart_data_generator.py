...
class ChartDataGenerator:
    ...
    async def generate_chart_data(self, backtest_result: Dict[str, Any], chart_type: str) -> Dict[str, Any]:
        """生成指定类型的图表数据"""
        try:
    ...
    async def _generate_equity_curve_data(self, backtest_result: Dict[str, Any]) -> Dict[str, Any]:
        """生成收益曲线图表数据"""
    ...
    async def _generate_drawdown_curve_data(self, backtest_result: Dict[str, Any]) -> Dict[str, Any]:
        """生成回撤曲线图表数据"""
        portfolio_history = backtest_result.get("portfolio_history", [])
        if not portfolio_history:
        ...
        max_drawdown_info = {
            ...
            'max_drawdown_date': df.loc[max_dd_idx, 'date'].isoformat() if pd.notna(df.loc[max_dd_idx, 'date']) else '',
    ...
    async def _generate_monthly_heatmap_data(self, backtest_result: Dict[str, Any]) -> Dict[str, Any]:
        """生成月度收益热力图数据"""
        portfolio_history = backtest_result.get("portfolio_history", [])
        if not portfolio_history:
    ...
    async def _generate_trade_distribution_data(self, backtest_result: Dict[str, Any]) -> Dict[str, Any]:
        """生成交易分布图表数据"""
        ...
        # 交易统计
        trade_stats = {
            'total_trades': len([t for t in trade_history if t.get('action') == 'SELL']),
    ...
    async def _generate_position_weights_data(self, backtest_result: Dict[str, Any]) -> Dict[str, Any]:
        """生成持仓权重图表数据"""
        trade_history = backtest_result.get("trade_history", [])
        if not trade_history:
    ...
    async def _generate_risk_metrics_data(self, backtest_result: Dict[str, Any]) -> Dict[str, Any]:
        """生成风险指标图表数据"""
        portfolio_history = backtest_result.get("portfolio_history", [])
        if not portfolio_history:
        ...
        risk_metrics = {
            ...
            'max_drawdown': float(backtest_result.get('max_drawdown', 0) * 100),  # 转换为百分比
        ...
        if rolling_window >= 5:
            rolling_volatility = returns.rolling(window=rolling_window).std() * np.sqrt(252) * 100
            ...
            for date, vol in rolling_volatility.dropna().items():
                sharpe = rolling_sharpe.loc[date] if pd.notna(rolling_sharpe.loc[date]) else 0
