...
class MonthlyAnalyzer:
    ...
    def __init__(self):
        ...
        self.quarter_names = {1: "Q1", 2: "Q2", 3: "Q3", 4: "Q4"}
    ...
    def _calculate_yearly_returns(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for date, yearly_return in yearly_returns.items():
            ...
            if len(year_data) > 1:
                ...
                yearly_sharpe = (
                    (yearly_return - 0.03) / yearly_volatility
    ...
    def _calculate_monthly_statistics(
        ...
        # 连续盈利/亏损月份
        max_consecutive_positive = self._calculate_max_consecutive_months(
            returns, lambda x: x > 0
        )
        max_consecutive_negative = self._calculate_max_consecutive_months(
            returns, lambda x: x < 0
        )
    ...
    async def calculate_rolling_monthly_metrics(
        self, portfolio_history: List[Dict], window_months: int = 12
    ) -> Dict[str, Any]:
        """
        计算滚动月度指标
        ...
        try:
            ...
            if len(monthly_returns) < window_months:
                logger.warning(
                    f"月度数据不足 {len(monthly_returns)}，无法计算 {window_months} 个月的滚动指标"
                )
                return {}
            ...
            # 滚动波动率
            rolling_volatility = monthly_returns.rolling(window_months).std() * np.sqrt(
                12
            )
