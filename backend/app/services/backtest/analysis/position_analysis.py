...
class PositionAnalyzer:
    ...
    async def analyze_position_performance(
        ...
        try:
            ...
            # 风险集中度分析
            concentration_analysis = self._analyze_concentration_risk(trade_history, portfolio_history)
    ...
    def _analyze_stock_performance(self, trade_history: List[Dict]) -> List[Dict[str, Any]]:
        """分析各股票表现"""
        ...
        for stock_code, stats in stock_stats.items():
            ...
            # 持仓期分析
            holding_periods = self._calculate_holding_periods_for_stock(buy_trades, sell_trades)
            ...
            if buy_trades and sell_trades:
                ...
                trade_frequency = total_trades / max(trading_span_days / 30, 1)  # 每月交易次数
            else:
                trade_frequency = 0
            ...
            price_improvement = (avg_sell_price - avg_buy_price) / avg_buy_price if avg_buy_price > 0 else 0
            ...
            stock_analysis = {
                ...
                'avg_return_per_trade': float(stats['total_pnl'] / total_trades) if total_trades > 0 else 0,
                'return_ratio': float(stats['total_pnl'] / stats['total_volume']) if stats['total_volume'] > 0 else 0,
                ...
                'commission_ratio': float(stats['total_commission'] / stats['total_volume']) if stats['total_volume'] > 0 else 0
            }
    ...
    def _analyze_position_weights(self, portfolio_history: List[Dict]) -> Dict[str, Any]:
        """分析持仓权重"""
        ...
        try:
            ...
            # 分析集中度
            concentration_metrics = self._calculate_concentration_metrics(weight_history)
    ...
    def _analyze_weight_changes(self, weight_history: List[Dict]) -> List[Dict[str, Any]]:
        """分析权重变化"""
        ...
        # 按日期和变化幅度排序
        weight_changes.sort(key=lambda x: (x['date'], abs(x['weight_change'])), reverse=True)
        
        return weight_changes[-100:]  # 返回最近100个显著变化
    
    def _calculate_concentration_metrics(self, weight_history: List[Dict]) -> Dict[str, Any]:
        """计算集中度指标"""
        ...
        for snapshot in weight_history:
            ...
            if weights:
                ...
                top_1_concentration = sorted_weights[0] if len(sorted_weights) >= 1 else 0
        ...
        if concentration_metrics:
            avg_metrics = {
                ...
                'avg_effective_stocks': float(np.mean([m['effective_stocks'] for m in concentration_metrics])),
                'avg_top_1_concentration': float(np.mean([m['top_1_concentration'] for m in concentration_metrics])),
                'avg_top_3_concentration': float(np.mean([m['top_3_concentration'] for m in concentration_metrics])),
                'avg_top_5_concentration': float(np.mean([m['top_5_concentration'] for m in concentration_metrics])),
                'avg_total_positions': float(np.mean([m['total_positions'] for m in concentration_metrics]))
            }
        else:
            avg_metrics = {}
    ...
    def _analyze_trading_patterns(self, trade_history: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            # 按时间排序
            sorted_trades = sorted(trade_history, key=lambda x: pd.to_datetime(x['timestamp']))
    ...
    def _analyze_trading_times(self, trades: List[Dict]) -> Dict[str, Any]:
        ...
        weekday_distribution = [
            {'weekday': weekday, 'weekday_name': weekday_names[weekday], 'count': count, 'percentage': count / len(trades)}
    ...
    def _analyze_trading_frequency(self, trades: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_monthly_trades': float(np.mean(monthly_frequency)) if monthly_frequency else 0,
            'max_monthly_trades': max(monthly_frequency) if monthly_frequency else 0,
            'total_trading_days': (timestamps[-1] - timestamps[0]).days if len(timestamps) >= 2 else 0
        }
    ...
    def _analyze_trading_success(self, trades: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_win_amount': float(np.mean([t['pnl'] for t in winning_trades])) if winning_trades else 0,
            'avg_loss_amount': float(np.mean([t['pnl'] for t in losing_trades])) if losing_trades else 0
        }
    ...
    def _analyze_holding_periods(self, trade_history: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            'short_term_positions': len([p for p in all_holding_periods if p <= 7]),  # 一周内
            'medium_term_positions': len([p for p in all_holding_periods if 7 < p <= 30]),  # 一周到一月
            'long_term_positions': len([p for p in all_holding_periods if p > 30])  # 超过一月
        }
    ...
    def _analyze_concentration_risk(
        ...
        try:
            ...
            # 基于持仓的集中度
            position_concentration = self._calculate_position_concentration(portfolio_history)
    ...
    def _calculate_trade_concentration(self, trade_history: List[Dict]) -> Dict[str, Any]:
        """计算交易集中度"""
        ...
        # 计算权重
        stock_weights = {
            stock: volume / total_volume 
            for stock, volume in stock_volumes.items()
        }
    ...
    def _calculate_position_concentration(self, portfolio_history: List[Dict]) -> Dict[str, Any]:
        """计算持仓集中度"""
