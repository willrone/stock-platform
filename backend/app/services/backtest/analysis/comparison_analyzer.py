...
class BacktestComparisonAnalyzer:
    ...
    async def analyze_comparison(
        ...
        try:
            ...
            # 计算相关性分析
            correlation_analysis = await self._calculate_correlation_analysis(
                comparison_results
            )

            # 风险收益散点图数据
            risk_return_scatter = await self._generate_risk_return_scatter(
                comparison_results
            )

            # 收益曲线对比数据
            equity_curves_comparison = await self._generate_equity_curves_comparison(
                comparison_results
            )

            # 排名分析
            ranking_analysis = await self._generate_ranking_analysis(
                comparison_results, comparison_metrics
            )
    ...
    async def _build_comparison_table(
        self, comparison_results: List[Dict[str, Any]], comparison_metrics: List[str]
    ) -> List[Dict[str, Any]]:
        """构建对比表格"""
        comparison_table = []
    ...
    async def _generate_risk_return_scatter(
        self, comparison_results: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """生成风险收益散点图数据"""
        scatter_data = []
    ...
    async def _generate_equity_curves_comparison(
        self, comparison_results: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """生成收益曲线对比数据"""
        equity_curves = []
        ...
        for result in comparison_results:
            ...
            if "date" in df.columns and "portfolio_value" in df.columns:
                ...
                df["cumulative_return"] = (
                    df["portfolio_value"] / initial_value - 1
                ) * 100
        ...
        return {"equity_curves": equity_curves, "total_strategies": len(equity_curves)}
    ...
    async def _generate_ranking_analysis(
        ...
        composite_ranking = sorted(
            composite_scores.values(), key=lambda x: x["composite_score"]
        )
