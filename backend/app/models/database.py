...
class TaskResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'backtest_metrics': json.loads(self.backtest_metrics) if self.backtest_metrics else {},
            'created_at': self.created_at.isoformat() if self.created_at else None
        }
...
class ModelMetadata:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'parameters': json.loads(self.parameters) if self.parameters else {},
            'training_data_info': json.loads(self.training_data_info) if self.training_data_info else {},
...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_task_results_stock_code ON task_results (stock_code)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_model_metadata_type ON model_metadata (model_type)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_model_metadata_active ON model_metadata (is_active)")
