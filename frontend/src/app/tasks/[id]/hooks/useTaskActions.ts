...
export function useTaskActions(taskId: string, currentTask: Task | null, onRefresh: () => Promise<void>) {
  ...
  const handleRebuild = () => {
    ...
    if (currentTask.task_type === 'backtest') {
      ...
      if (predConfig) {
        if (predConfig.horizon) params.set('horizon', predConfig.horizon.toString());
