/**
 * Alpha因子信息定义
 *
 * 提供Alpha因子的分类、描述和说明
 */

export interface AlphaFactorInfo {
  name: string;
  category: string;
  description: string;
  formula?: string;
  period?: number;
}

// Alpha因子分类信息
export const ALPHA_FACTOR_CATEGORIES = {
  MOMENTUM: {
    name: '动量因子',
    description: '衡量价格动量、趋势和变化率（Alpha #1-60）',
    icon: '📈',
  },
  MEAN_REVERSION: {
    name: '均值回归因子',
    description: '识别价格偏离均值的回归机会（Alpha #61-100）',
    icon: '🔄',
  },
  VOLATILITY: {
    name: '波动率因子',
    description: '衡量价格和成交量的波动程度（Alpha #101-140）',
    icon: '📉',
  },
  VOLUME: {
    name: '成交量因子',
    description: '分析成交量变化和量价关系（Alpha #141-158）',
    icon: '📦',
  },
  RESI: {
    name: '价格收益率',
    description: '衡量不同时间窗口的价格变化趋势和动量',
    icon: '📈',
  },
  MA: {
    name: '移动平均',
    description: '平滑价格波动，识别价格趋势',
    icon: '📊',
  },
  STD: {
    name: '价格波动率',
    description: '衡量价格波动程度，用于风险评估',
    icon: '📉',
  },
  VSTD: {
    name: '成交量波动率',
    description: '衡量成交量波动，识别异常交易',
    icon: '📦',
  },
  CORR: {
    name: '量价相关性',
    description: '价格与成交量的相关性，判断市场情绪',
    icon: '🔗',
  },
  MAX: {
    name: '最高价',
    description: '识别阻力位和突破信号',
    icon: '⬆️',
  },
  MIN: {
    name: '最低价',
    description: '识别支撑位和下跌信号',
    icon: '⬇️',
  },
  QTLU: {
    name: '分位数比率',
    description: '衡量价格分布的离散程度',
    icon: '📐',
  },
  OTHER: {
    name: '其他Alpha因子',
    description: '其他类型的Alpha因子',
    icon: '🔢',
  },
};

/**
 * Alpha158因子编号到实际名称的映射表
 * 基于Qlib标准Alpha158因子定义
 */
const ALPHA158_FACTOR_NAMES: Record<number, string> = {
  // 动量因子 (1-60)
  1: '价格收益率(1日)',
  2: '价格收益率(2日)',
  3: '价格收益率(3日)',
  4: '价格收益率(4日)',
  5: '价格收益率(5日)',
  6: '价格收益率(6日)',
  7: '价格收益率(7日)',
  8: '价格收益率(8日)',
  9: '价格收益率(9日)',
  10: '价格收益率(10日)',
  11: '价格收益率(11日)',
  12: '价格收益率(12日)',
  13: '价格收益率(13日)',
  14: '价格收益率(14日)',
  15: '价格收益率(15日)',
  16: '价格收益率(16日)',
  17: '价格收益率(17日)',
  18: '价格收益率(18日)',
  19: '价格收益率(19日)',
  20: '价格收益率(20日)',
  21: '价格动量(5日)',
  22: '价格动量(10日)',
  23: '价格动量(15日)',
  24: '价格动量(20日)',
  25: '价格动量(30日)',
  26: '相对强度(5日)',
  27: '相对强度(10日)',
  28: '相对强度(20日)',
  29: '相对强度(30日)',
  30: '相对强度(60日)',
  31: '价格变化率(5日)',
  32: '价格变化率(10日)',
  33: '价格变化率(20日)',
  34: '价格变化率(30日)',
  35: '价格变化率(60日)',
  36: '累计收益率(5日)',
  37: '累计收益率(10日)',
  38: '累计收益率(20日)',
  39: '累计收益率(30日)',
  40: '累计收益率(60日)',
  41: '移动平均比率(5/10)',
  42: '移动平均比率(10/20)',
  43: '移动平均比率(20/30)',
  44: '移动平均比率(5/20)',
  45: '移动平均比率(10/30)',
  46: '价格偏离均线(5日)',
  47: '价格偏离均线(10日)',
  48: '价格偏离均线(20日)',
  49: '价格偏离均线(30日)',
  50: '价格偏离均线(60日)',
  51: '趋势强度(5日)',
  52: '趋势强度(10日)',
  53: '趋势强度(20日)',
  54: '趋势强度(30日)',
  55: '趋势强度(60日)',
  56: '价格加速度(5日)',
  57: '价格加速度(10日)',
  58: '价格加速度(20日)',
  59: '价格加速度(30日)',
  60: '价格加速度(60日)',

  // 均值回归因子 (61-100)
  61: '价格回归度(5日)',
  62: '价格回归度(10日)',
  63: '价格回归度(20日)',
  64: '价格回归度(30日)',
  65: '价格回归度(60日)',
  66: '偏离度(5日)',
  67: '偏离度(10日)',
  68: '偏离度(20日)',
  69: '偏离度(30日)',
  70: '偏离度(60日)',
  71: 'Z分数(5日)',
  72: 'Z分数(10日)',
  73: 'Z分数(20日)',
  74: 'Z分数(30日)',
  75: 'Z分数(60日)',
  76: '价格位置(5日)',
  77: '价格位置(10日)',
  78: '价格位置(20日)',
  79: '价格位置(30日)',
  80: '价格位置(60日)',
  81: '相对位置(5日)',
  82: '相对位置(10日)',
  83: '相对位置(20日)',
  84: '相对位置(30日)',
  85: '相对位置(60日)',
  86: '反转信号(5日)',
  87: '反转信号(10日)',
  88: '反转信号(20日)',
  89: '反转信号(30日)',
  90: '反转信号(60日)',
  91: '超买超卖(5日)',
  92: '超买超卖(10日)',
  93: '超买超卖(20日)',
  94: '超买超卖(30日)',
  95: '超买超卖(60日)',
  96: '回归系数(5日)',
  97: '回归系数(10日)',
  98: '回归系数(20日)',
  99: '回归系数(30日)',
  100: '回归系数(60日)',

  // 波动率因子 (101-140)
  101: '价格波动率(5日)',
  102: '价格波动率(10日)',
  103: '价格波动率(20日)',
  104: '价格波动率(30日)',
  105: '价格波动率(60日)',
  106: '收益率波动率(5日)',
  107: '收益率波动率(10日)',
  108: '收益率波动率(20日)',
  109: '收益率波动率(30日)',
  110: '收益率波动率(60日)',
  111: '历史波动率(5日)',
  112: '历史波动率(10日)',
  113: '历史波动率(20日)',
  114: '历史波动率(30日)',
  115: '历史波动率(60日)',
  116: '标准差(5日)',
  117: '标准差(10日)',
  118: '标准差(20日)',
  119: '标准差(30日)',
  120: '标准差(60日)',
  121: '变异系数(5日)',
  122: '变异系数(10日)',
  123: '变异系数(20日)',
  124: '变异系数(30日)',
  125: '变异系数(60日)',
  126: '波动率比率(5/10)',
  127: '波动率比率(10/20)',
  128: '波动率比率(20/30)',
  129: '波动率比率(5/20)',
  130: '波动率比率(10/30)',
  131: '波动率变化(5日)',
  132: '波动率变化(10日)',
  133: '波动率变化(20日)',
  134: '波动率变化(30日)',
  135: '波动率变化(60日)',
  136: '波动率加速度(5日)',
  137: '波动率加速度(10日)',
  138: '波动率加速度(20日)',
  139: '波动率加速度(30日)',
  140: '波动率加速度(60日)',

  // 成交量因子 (141-158)
  141: '成交量变化率(5日)',
  142: '成交量变化率(10日)',
  143: '成交量变化率(20日)',
  144: '成交量变化率(30日)',
  145: '成交量变化率(60日)',
  146: '成交量波动率(5日)',
  147: '成交量波动率(10日)',
  148: '成交量波动率(20日)',
  149: '成交量波动率(30日)',
  150: '成交量波动率(60日)',
  151: '量价相关性(5日)',
  152: '量价相关性(10日)',
  153: '量价相关性(20日)',
  154: '量价相关性(30日)',
  155: '量价相关性(60日)',
  156: '成交量比率(5/10)',
  157: '成交量比率(10/20)',
  158: '成交量比率(20/30)',
};

/**
 * 根据Alpha158因子编号获取类别
 * 基于Qlib Alpha158因子的标准定义进行大致分类
 */
function getAlpha158CategoryByNumber(alphaNumber: number): string {
  // 根据Alpha158的标准定义，大致分类：
  // 1-60: 动量类因子（价格变化、收益率等）
  // 61-100: 均值回归类因子
  // 101-140: 波动率类因子
  // 141-158: 成交量类因子

  if (alphaNumber >= 1 && alphaNumber <= 60) {
    return 'MOMENTUM';
  } else if (alphaNumber >= 61 && alphaNumber <= 100) {
    return 'MEAN_REVERSION';
  } else if (alphaNumber >= 101 && alphaNumber <= 140) {
    return 'VOLATILITY';
  } else if (alphaNumber >= 141 && alphaNumber <= 158) {
    return 'VOLUME';
  }
  return 'OTHER';
}

/**
 * 获取Alpha158因子的实际名称
 */
function getAlpha158FactorName(alphaNumber: number): string {
  return ALPHA158_FACTOR_NAMES[alphaNumber] || `Alpha因子 #${alphaNumber}`;
}

/**
 * 解析Alpha因子名称，获取分类和周期信息
 */
export function parseAlphaFactor(factorName: string): AlphaFactorInfo {
  // 首先尝试匹配 alpha_XXX 格式（标准Alpha158因子）
  const alpha158Match = factorName.match(/^alpha_(\d+)$/i);
  if (alpha158Match) {
    const alphaNumber = parseInt(alpha158Match[1], 10);
    const category = getAlpha158CategoryByNumber(alphaNumber);
    const categoryInfo =
      ALPHA_FACTOR_CATEGORIES[category as keyof typeof ALPHA_FACTOR_CATEGORIES] ||
      ALPHA_FACTOR_CATEGORIES.OTHER;
    const actualName = getAlpha158FactorName(alphaNumber);

    return {
      name: factorName,
      category,
      description: actualName,
      formula: `标准Alpha158因子，编号${alphaNumber}，属于${categoryInfo.name}类别`,
    };
  }

  // 匹配其他格式：RESI5, MA10, STD20, VSTD30, CORR5, MAX10, MIN20, QTLU30等
  const patterns = [
    {
      pattern: /^(RESI)(\d+)$/i,
      category: 'RESI',
      description: '价格收益率',
      formula: '(当前价格 - N天前价格) / N天前价格',
    },
    {
      pattern: /^(MA)(\d+)$/i,
      category: 'MA',
      description: '移动平均',
      formula: '过去N天的收盘价平均值',
    },
    {
      pattern: /^(STD)(\d+)$/i,
      category: 'STD',
      description: '价格波动率',
      formula: '过去N天收盘价的标准差',
    },
    {
      pattern: /^(VSTD)(\d+)$/i,
      category: 'VSTD',
      description: '成交量波动率',
      formula: '过去N天成交量的标准差',
    },
    {
      pattern: /^(CORR)(\d+)$/i,
      category: 'CORR',
      description: '量价相关性',
      formula: '过去N天价格与成交量的相关系数',
    },
    {
      pattern: /^(MAX)(\d+)$/i,
      category: 'MAX',
      description: '最高价',
      formula: '过去N天最高价的最大值',
    },
    {
      pattern: /^(MIN)(\d+)$/i,
      category: 'MIN',
      description: '最低价',
      formula: '过去N天最低价的最小值',
    },
    {
      pattern: /^(QTLU)(\d+)$/i,
      category: 'QTLU',
      description: '分位数比率',
      formula: '过去N天价格的80分位数 / 20分位数',
    },
  ];

  for (const { pattern, category, description, formula } of patterns) {
    const match = factorName.match(pattern);
    if (match) {
      const period = match[2] ? parseInt(match[2], 10) : undefined;
      return {
        name: factorName,
        category,
        description: period ? `${description} (${period}天)` : description,
        formula,
        period,
      };
    }
  }

  // 默认返回
  return {
    name: factorName,
    category: 'OTHER',
    description: 'Alpha因子',
  };
}

/**
 * 将Alpha因子按类别分组
 */
export function groupAlphaFactorsByCategory(factors: string[]): Record<string, string[]> {
  const grouped: Record<string, string[]> = {};

  factors.forEach(factor => {
    const info = parseAlphaFactor(factor);
    if (!grouped[info.category]) {
      grouped[info.category] = [];
    }
    grouped[info.category].push(factor);
  });

  // 对每个类别内的因子进行排序
  Object.keys(grouped).forEach(category => {
    grouped[category].sort((a, b) => {
      // 如果是alpha_XXX格式，按编号排序
      const alphaMatchA = a.match(/^alpha_(\d+)$/i);
      const alphaMatchB = b.match(/^alpha_(\d+)$/i);
      if (alphaMatchA && alphaMatchB) {
        return parseInt(alphaMatchA[1], 10) - parseInt(alphaMatchB[1], 10);
      }

      // 如果有周期，按周期排序
      const infoA = parseAlphaFactor(a);
      const infoB = parseAlphaFactor(b);
      if (infoA.period && infoB.period) {
        return infoA.period - infoB.period;
      }

      // 否则按名称排序
      return a.localeCompare(b);
    });
  });

  return grouped;
}

/**
 * 获取Alpha因子的显示名称
 */
export function getAlphaFactorDisplayName(factorName: string): string {
  // 如果是alpha_XXX格式，显示实际名称
  const alpha158Match = factorName.match(/^alpha_(\d+)$/i);
  if (alpha158Match) {
    const alphaNumber = parseInt(alpha158Match[1], 10);
    return getAlpha158FactorName(alphaNumber);
  }

  const info = parseAlphaFactor(factorName);
  if (info.period) {
    return `${info.description} (${info.period}天)`;
  }
  return info.description || factorName;
}
