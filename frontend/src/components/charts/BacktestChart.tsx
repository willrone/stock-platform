...
export default function BacktestChart({ stockCode, backtestData }: BacktestChartProps) {
  ...
  const processBacktestData = () => {
    ...
    if (hasRealData) {
      ...
      // 转换交易记录格式
      const trades: TradeRecord[] = tradeHistory.map((trade: Record<string, unknown>) => ({
      ...
      return {
        dates:
          dates.length > 0
            ? dates
            : equityCurve.map((_: unknown, i: number) => {
                const date = new Date();
  ...
  useEffect(() => {
    ...
    if (equityChartRef.current) {
      ...
      const equityOption = {
        ...
        tooltip: {
          trigger: 'axis',
          formatter: function (params: { value: number; axisValue: string }[]) {
        ...
        <Card>
          <CardContent sx={{ p: { xs: 1, sm: 2 }, overflow: 'hidden' }}>
            <Box sx={{ overflowX: 'auto' }}>
              <Box
                ref={equityChartRef}
                sx={{ height: { xs: 250, sm: 300 }, width: '100%', minWidth: 350 }}
              />
            </Box>
          </CardContent>
        </Card>
