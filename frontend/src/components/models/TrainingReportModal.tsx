...
export const TrainingReportModal: React.FC<TrainingReportModalProps> = ({
  ...
  const downloadReport = () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `training_report_${report.model_id}_${
        ...
        {report ? (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            <Box>
              ...
              >
                {Object.entries(report.performance_metrics || report.final_metrics || {}).map(
                  ([key, value]) => {
                    // 格式化显示值
                ...
                <Box sx={{ height: 320, bgcolor: 'grey.50', borderRadius: 1, p: 2 }}>
                  <ReactECharts
                    option={{
                      ...
                      legend: {
                        data: ['训练损失', '验证损失', '训练准确率', '验证准确率'].filter(
                          (_, idx) => {
                            const hasLoss = report.training_history?.some(
                              h => h.train_loss !== undefined || h.val_loss !== undefined
                            );
                            const hasAccuracy = report.training_history?.some(
                              h => h.train_accuracy !== undefined || h.val_accuracy !== undefined
                            );
                            if (idx < 2) {
                              return hasLoss;
                            }
                      ...
                      xAxis: {
                        type: 'category',
                        boundaryGap: false,
                        data: report.training_history.map((h, idx) =>
                          h.epoch !== undefined ? `Epoch ${h.epoch}` : idx + 1
                        ),
                      },
                      yAxis: [
                        {
                          ...
                          name: '损失',
                          position: 'left',
                          show: report.training_history.some(
                            h => h.train_loss !== undefined || h.val_loss !== undefined
                          ),
                        },
                        {
                        ...
                        series: [
                          {
                            ...
                            data: (Array.isArray(report.feature_importance)
                              ? report.feature_importance
                                  .sort(
                                    (a: any, b: any) => (b.importance || b) - (a.importance || a)
                                  )
                                  .slice(0, 10)
                  ...
                  <Box sx={{ mt: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                      {(Array.isArray(report.feature_importance)
                        ...
                        const importance =
                          item.importance !== undefined ? item.importance : item[1] || 0;
                        const maxImportance = Array.isArray(report.feature_importance)
                          ? Math.max(
                              ...report.feature_importance.map((f: any) => f.importance || f)
                            )
                          : Math.max(
                              ...Object.values(report.feature_importance as Record<string, number>)
                            );
                    ...
                    <Box>
                      ...
                      >
                        {highCorrelationPairs.slice(0, 6).map((pair, idx) => (
                          <Typography
                            key={`${pair.feature1}-${pair.feature2}-${idx}`}
                            variant="body2"
                            color="text.secondary"
                          >
                  ...
                  <CardContent>
                    ...
                    >
                      {report.recommendations.map((rec, idx) => (
                        ...
                        >
                          <Box component="span" sx={{ color: 'primary.main', mt: 0.5 }}>
                            •
                          </Box>
                ...
                <AccordionDetails>
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    ...
                    <Box>
                      ...
                      <Typography variant="body2" component="span">
                        {report.training_data_info.start_date || '未知'} 至{' '}
                        {report.training_data_info.end_date || '未知'}
                      </Typography>
                    </Box>
