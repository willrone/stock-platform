          ...
          <TableBody>
            {models.map(model => (
              <TableRow key={model.model_id} hover>
                      ...
                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <LinearProgress
                            variant="determinate"
                            value={
                              trainingProgress[model.model_id]?.progress ||
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {trainingProgress[model.model_id]?.message ||
                            getStageText(
                              trainingProgress[model.model_id]?.stage || model.training_stage || ''
                            )}
                        </Typography>

                        {/* 实时指标 */}
                        {trainingProgress[model.model_id]?.metrics &&
                          Object.keys(trainingProgress[model.model_id].metrics).length > 0 && (
                            <Box sx={{ display: 'flex', gap: 1 }}>
                              {Object.entries(trainingProgress[model.model_id].metrics)
                                .slice(0, 2)
                                .map(([key, value]) => (
                ...
                <TableCell>{new Date(model.created_at).toLocaleDateString('zh-CN')}</TableCell>
                <TableCell>
                  <Box sx={{ display: 'flex', gap: 1 }}>
                    {model.status === 'ready' && (
                      <Button
