...
export default function BacktestProgressMonitor({
  ...
  const renderStageProgress = useCallback((stage: BacktestProgressStage) => {
    return (
      <Box key={stage.name} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 1.5, borderRadius: 1, bgcolor: 'grey.50' }}>
        <Box sx={{ flexShrink: 0 }}>
    ...
    return (
      <Card className={className}>
        <CardContent sx={{ textAlign: 'center', py: 4 }}>
          ...
          <Typography variant="body2" color="text.secondary">连接回测进度监控中...</Typography>
        </CardContent>
      </Card>
    );
  }
      ...
      <Card className={className}>
        ...
        <CardContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            {error && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, p: 1.5, bgcolor: 'error.light', border: 1, borderColor: 'error.main', borderRadius: 1 }}>
                <ExclamationTriangleIcon className="w-5 h-5" style={{ color: '#d32f2f', flexShrink: 0 }} />
                <Typography variant="body2" sx={{ color: 'error.dark' }}>{error}</Typography>
              </Box>
            )}
              ...
              <>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Typography variant="body2" sx={{ fontWeight: 500 }}>总体进度</Typography>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2, 1fr)', md: 'repeat(4, 1fr)' }, gap: 2 }}>
                  <Box sx={{ textAlign: 'center' }}>
                    ...
                    <Typography variant="caption" color="text.secondary">已处理天数</Typography>
                  </Box>
                  ...
                  <Box sx={{ textAlign: 'center' }}>
                    ...
                    <Typography variant="caption" color="text.secondary">执行交易</Typography>
                  </Box>
                  ...
                  <Box sx={{ textAlign: 'center' }}>
                    ...
                    <Typography variant="caption" color="text.secondary">生成信号</Typography>
                  </Box>
                  ...
                  <Box sx={{ textAlign: 'center' }}>
                    ...
                    <Typography variant="caption" color="text.secondary">组合价值</Typography>
                  </Box>
                </Box>
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, gap: 2 }}>
                      {progressData.elapsed_time && (
                        <Box>
                          ...
                          <Typography variant="body2" sx={{ fontWeight: 500 }}>{formatDuration(progressData.elapsed_time)}</Typography>
                        </Box>
                      )}
                      
                      {progressData.estimated_completion && (
                        <Box>
                          <Typography variant="caption" color="text.secondary">预计完成: </Typography>
                          <Typography variant="body2" sx={{ fontWeight: 500 }}>{formatDateTime(progressData.estimated_completion)}</Typography>
                        </Box>
                      )}
                      
                      {progressData.processing_speed > 0 && (
                        <Box>
                          <Typography variant="caption" color="text.secondary">处理速度: </Typography>
                          <Typography variant="body2" sx={{ fontWeight: 500 }}>{formatNumber(progressData.processing_speed, 2)} 天/秒</Typography>
                        </Box>
                      )}
                    </Box>
                  </>
                ...
                {progressData.current_date && (
                  <>
                    <Divider />
                    <Box>
                      <Typography variant="caption" color="text.secondary">当前处理日期: </Typography>
                      <Typography variant="body2" sx={{ fontWeight: 500 }}>{progressData.current_date}</Typography>
                    </Box>
                  </>
                )}
                ...
                <Divider />
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                  <Typography variant="subtitle2" sx={{ fontWeight: 500 }}>执行阶段</Typography>
