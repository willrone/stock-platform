...
// 生成完整的任务数据 - 简化版本
const completedTaskArbitrary = fc
  .record({
    task_id: fc.uuid(),
...
describe('前端数据展示完整性属性测试', () => {
  ...
  test('属性 8 边界测试: 空数据处理 - 应正确处理空预测结果', () => {
    fc.assert(
      fc.property(completedTaskArbitrary, task => {
      ...
      fc.property(
        ...
        predictions => {
          for (const prediction of predictions) {
            ...
            if (prediction.predicted_return < -0.25 || prediction.predicted_return > 0.25) {
              // 放宽范围
              return false;
            }
